...
既存の GitHub Issue を入力として、`/create-issue` 手順4の6項目バリデーションをベースに refine 用途向けへ項目構成を再編した観点で受け入れ条件を検証し、コードベース探索によって「Issue が触れていない論点」を洗い出します。検出した曖昧さは trivial / architectural / critical の 3 階層に分類し、階層に応じて Issue body 更新・コメント投稿・ラベル付与のいずれかを実行します。
...
取得した Issue body を以下 6 観点で検証します。`/create-issue` 手順4の6項目（具体的・検証可能・単一責務・曖昧表現なし・ストーリー・GWT+DoD 形式）をベースに、「曖昧表現」を項目 1「具体性」へ統合し、項目 4「受け入れ条件の明示」を追加した構成です:
...
- `/create-issue` コマンド（本プラグイン同梱）: バリデーションのベース（refine-issue は「曖昧表現排除」を「具体性」に統合し、「受け入れ条件の明示」を追加した 6 観点に再構成）
