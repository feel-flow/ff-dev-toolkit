...
> ⚠️ このスクリプトは判定を `echo` でしか表現せず、終了コードは常に 0 です。出力が 0 行のとき「問題なし」と「上流の `git log` が空振りした（対象文書が 1 件も列挙されなかった）」の区別がつかないため、まず対象文書が列挙されていることを確認してから結果を読んでください。**このまま CI 等の自動ゲートにコピーしないこと**（終了コードを捨てるゲートは空振りを合格にする。Issue #154）。ゲート化する場合は「列挙件数 > 0」の検査と `MISSING` 検出時の `exit 1` を追加します。
...
>
> ⚠️ §1 と同様、出力 0 行は「孤立なし」と「`find` / `grep` の空振り（列挙自体が失敗）」を区別しません。中間ファイル（`/tmp/all-md.txt` / `/tmp/linked-md.txt`）が非空であることを確認してから結果を読むこと。自動ゲートにする場合はこのままコピーしない。
