...
> **レビュー起動時の必須規定は [Git Workflow ステップ5](./git-workflow.md#ステップ5-セルフレビューpr作成前重要) が正本**: レビュー用サブエージェントの read-only 起動と、**全エージェントが終端に達するまでの作業ツリー凍結**を含む。本書はレビューの観点と記録様式を扱い、起動時の規定は再掲しない（件数も網羅的な列挙も持たせない — 規定が増えたときに「網羅的に見える不完全な要約」へ腐るため、正本を読ませる）。
