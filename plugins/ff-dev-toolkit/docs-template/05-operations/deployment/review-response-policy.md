# PRレビュー対応ポリシー（Review Response Policy）

> **Parent**: [DEPLOYMENT.md](../DEPLOYMENT.md) | **Related**: [git-workflow.md](./git-workflow.md) | [self-review.md](./self-review.md)

## 概要

PRレビュー結果に対する対応ルールを定義します。本ポリシーは全てのレビューソースに共通で適用されます。

**適用対象**:

- PR Review Toolkit（Claude Code サブエージェント）
- Codex CLI クロスモデルレビュー
- Multi-CLI 分散レビュー
- pre-commit / pre-push hook
- 人間によるレビュー

---

## 重要度別対応ルール

| 重要度                   | 対応             | 確認要否         | 説明                                                   |
| ------------------------ | ---------------- | ---------------- | ------------------------------------------------------ |
| **Critical**             | 必ず修正         | 不要（即対応）   | セキュリティ脆弱性、データ損失リスク、本番障害の可能性 |
| **Warning**              | 必ず修正         | 不要（即対応）   | バグの可能性、パフォーマンス問題、規約違反             |
| **Suggestion**           | 妥当なものは修正 | 不要             | コード改善提案、リファクタリング候補                   |
| **Info / Good Practice** | 確認のみ         | 不要（対応不要） | 参考情報、称賛、学習ポイント                           |

**重大度インフレの抑止**: 重大度は各レビュー観点の自己申告であり、そのまま修正義務へ直結する。対応表を適用する**前に**次を適用する。**新しいガード・抽象・フォールバック・防御コードの追加を求める指摘は、具体的な失敗シナリオ（再現する入力・状態と観測可能な誤動作）の提示がない限り Suggestion として扱う** — Warning 表記であっても本ポリシー上の扱いは Suggestion に落とす。ワークフローの基準は「止まらない・データを壊さない」であり、そこを超える堅牢化は任意の磨き込みであって欠陥ではない。独立した Warning を follow-up Issue へパーキングする運用は採用しない。

**重大度とスコープ判定は別軸**: 重大度は「どれだけ重要か」を表すもので、「その指摘への対応がこの PR に属するか」を決めない。対応が PR の明示的な設計判断・宣言した前提を覆すもの（今のままでは「誰かが望むより狭いものを出荷する」にあたるもの）は、Critical / Warning であっても PR 内の fix commit で処理せず、別 Issue に切り出して現行 PR は元のスコープで収束させる。「今のままでは間違ったものを出荷する」欠陥は従来どおり PR 内で修正する（切り分け基準と例外時のフォールバックは [git-workflow.md §レビュー指摘のスコープ判定](./git-workflow.md#レビュー指摘のスコープ判定欠陥か前提変更か)）。

---

## 対応フロー

```
レビュー結果を受信
    │
    ├─ 重大度インフレの抑止を先に適用（失敗シナリオのない「ガード追加」要求は Suggestion 扱い）
    │
    ├─ スコープ判定を先行（§重大度とスコープ判定は別軸 — PR の宣言した前提を覆す対応は別 Issue へ分離）
    │
    ├─ Critical / Warning → 即座に修正開始（ユーザー確認不要）
    │
    ├─ Suggestion → 実装が妥当か判断
    │   ├─ 妥当 → 修正
    │   └─ 不適切 → スキップ（理由を記録。却下時の検証要件は下記「指摘を却下（スキップ）するとき」）
    │
    └─ Info / Good Practice → 確認のみ（対応不要）

--- 以下は全ケース共通の後続フロー ---

TodoWrite で対応項目をリスト化（進捗可視化）
    ↓
すべての対応完了後、変更をコミット・プッシュ
    ↓
対応内容のサマリーをユーザーに報告
```

### 対応手順

1. レビュー結果を受け取ったら、ユーザーに確認を求めずに上記ポリシーに従って自動的に対応を開始
2. TodoWrite で対応項目をリスト化して進捗を可視化
3. 対応で実装が Issue の AC の前提を超えた／変えた場合は、その場で Issue 本文の該当 AC も更新する（変わった理由を 1 行添える）
4. すべての対応完了後、変更をコミット・プッシュ
5. 対応内容のサマリーをユーザーに報告

### 指摘を却下（スキップ）するとき

指摘を「不適切 → スキップ」へ倒す場合、理由の記録に加えて次の 4 点を守る。誤った却下は正しい指摘より長く生き残り、コードコメントに固定化されると後続レビューの正しい再提案まで拒否させるため、却下する側にも検証責任を置く。

1. **却下理由が技術的制約の主張であるとき（「提案コードはコンパイルしない」「この API では動かない」等）は、その制約を 1 変数ずつ切り分けて実測する。** 「提案コードが落ちた」は原因の特定ではない。再現時に複数の要素を同時に変えていたなら、各要素を単独で戻して確かめる。設計方針・スコープ判断による却下は従来どおり理由の記録のみでよい（実測は不要）
2. **却下理由をコードコメントに残す場合は、実測した範囲だけを書く。** 一般化した主張（「generic は通らない」）ではなく、確認した条件（「readonly 戻り値では通らない」）を書く
3. **将来のレビュアーへの指示形（「提案するな」「使うな」）を書かない。** 事実の記述に留める。指示形は誤っていたとき最も害が大きく、事実の記述なら次の人が再検証できる
4. **指摘の趣旨と、添えられた実装の正しさは別物として扱う。** 提案コードが通らないことは、指摘そのものが誤りである証拠にならない

---

## ESLint Warnings の扱い

| 対象                                 | 対応                                                                          |
| ------------------------------------ | ----------------------------------------------------------------------------- |
| 今回変更したファイルの Warning       | 必ず修正                                                                      |
| 現 diff・AC・既存契約に関わる Warning | ファイル境界によらず現 PR で解消                                              |
| 独立した既存ファイルの Warning       | 重大度が Warning と確定しているため、ファイル境界によらず現 PR で解消         |

変更対象外という理由だけで一律に別 Issue へ送らない。現在の diff が導入・悪化させた回帰、現 Issue の AC、既存契約、必須品質ゲート、レビューで Critical / Warning と確定した指摘は現 PR で解消する。Suggestion 以下の独立した発見だけをスコープ外発見の三分岐へ渡し、Issue 化する場合も先に類似 Issue を検索する。この表にも §重大度とスコープ判定は別軸 が先行する — 対応が PR の宣言した前提を覆す場合は、Warning 確定でも別 Issue へ分離する。

---

## 適用場面

### セルフレビュー時（PR作成後）

```
ステップ6: セルフレビュー（対象は PR の head SHA）
    ├── PR Review Toolkit 実行 → 本ポリシーで対応
    ├── Codex CLI クロスモデルレビュー → 本ポリシーで対応
    └── Multi-CLI 分散レビュー → 本ポリシーで対応
```

### レビュー対応（fix commit 後）

```
ステップ7: レビュー対応
    ├── Claude Code + Codex クロスモデルレビュー → 本ポリシーで対応
    ├── 人間レビュワー → 本ポリシーで対応 + スレッド返信必須
    └── CI/CD チェック → 本ポリシーで対応
```

**注意**: 人間レビュワーからの指摘には、本ポリシーに加えて[レビュー対応のコメント規約](./git-workflow.md#ステップ7-レビュー対応review)に従ったスレッド返信が必須。

---

## 関連ドキュメント

- [git-workflow.md](./git-workflow.md) -- AI駆動Git Workflow全体
- [self-review.md](./self-review.md) -- セルフレビュー詳細
- [multi-cli-review-orchestration.md](./multi-cli-review-orchestration.md) -- Multi-CLI分散レビュー
- [workflow-principles.md](./workflow-principles.md) -- ワークフロー運用原則
