...
| `--model auto`         | モデル自動選択                         | 最適なモデルを自動選択（既定はこれ） |
| `--model <name>`       | モデル指定                             | 特定モデルを使用（env 経由でのみ）   |
| `--output-format text` | テキスト出力                           | 人間可読な結果                       |
| `--force`              | 確認なし実行                           | 自動化用                             |

> **モデル指定の方針**: スクリプトに具体的なモデル slug をベタ書きしないこと。`auto` は「ベンダー側に最適なモデルを選ばせる」という世代交代しない語なので既定値として持ってよいが、特定モデルを使いたい場合は環境変数から組み立て、未設定ならフラグ自体を渡さない。理由と実装パターンは [REVIEW_AGENT_CREATION_GUIDE.md の「モデル指定は『既定値を持たない』」](../../06-reference/REVIEW_AGENT_CREATION_GUIDE.md#モデル指定は既定値を持たない) を参照。
