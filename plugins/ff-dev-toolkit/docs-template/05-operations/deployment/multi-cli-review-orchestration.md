...
主担当のセルフレビューに、主担当以外の最低1つのレビューを加えるパターンです。[レビュー担当の選択と利用制限時の継続](./self-review.md#レビュー担当の選択と利用制限時の継続)を正本とし、他の候補がすべて利用不可なら理由を記録して主担当のみで進めます。以下の Codex コマンドは Claude が主担当の場合の例です。
...
- 資格情報の拒否（🔑）・残高の枯渇（💳）と切り分けられた失敗が出た時点で、**同じ実行内に残っている同一 CLI のタスクはスキップします**（フル setup を払ってから同じ失敗を引かないため）。スキップしたタスクは実行ログで `⏭ Skipped <cli>/<perspective>` として名指しされ、統合レポートにも `⏭ SKIPPED`（`INCOMPLETE` 併記）の節が出ます。**スキップは「失敗」ではなく「未実行」**なので、次回の `--resume` では通常どおり再実行対象になります。引数リスト超過（📏）・切り分け不能な失敗・**タイムアウト（時間を足せば通りうる）ではスキップしません**（推測でカバレッジを落とさないための fail-open）。前回の未解消 Critical がある観点がスキップされた場合、その分類は「解消が証明されていない」ものとして保持されます。
...
別 PR の `.review-results/` が残っていると、未解消 Critical の系列ガードが「別 branch/base/scope の状態」として絞り込み実行を非 0 で止める。案内の「Run an unfiltered full review」は、`--perspective` / `--exclude-perspective` / `--mode cross-model` を付けないフルレビューを指す（`bash scripts/codex-review.sh --base <branch>` で足りる。`--cli` は絞り込みではない）。残存結果を確認せず捨ててやり直すなら `--fresh` を付ける。`--fresh` はライブ出力ディレクトリの中身を隣の `.review-results.prev-<timestamp>/` へ移す（実行中 lock は残す）。この sibling も揮発成果物なので、ライブディレクトリを ignore しているリポジトリでは `.review-results.prev-*/` も ignore する。
