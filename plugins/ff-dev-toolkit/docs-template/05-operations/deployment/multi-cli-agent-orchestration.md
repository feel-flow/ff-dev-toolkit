...
プロジェクト側に `.claude/agent-config.yaml` を置くとプラグイン同梱のデフォルト設定より優先される（環境変数 `MULTI_AGENT_CONFIG=<path>` または `--config <path>` でも上書き可。読み取りは `yq` 依存で、無い環境では設定ファイルは読まれず既定値で動く）。ただし**実際に読まれるのは `version` / `mode` / `parallel` / `review.main` / `review.sub` / `review.critical_nonblock_perspectives` / `review.cross_review` / `review.confidence_threshold` / `exclude_clis` と、`version: "2.0"` のときだけ `tasks.<task>.{mode,cost_strategy,timeout,output_dir}` である**（`version` が `2.0` でない場合は v1 形式とみなされ、トップレベルの `cost_strategy` / `timeout` / `output_dir` が読まれる — `version` を書き忘れると `tasks.*` が黙って無視されるので注意）。`agents:` と `fallback:` はどのバージョンでも読まれず、書いても挙動は変わらない（同梱の既定設定はこの 2 ブロックを持たない）。実行時のレジストリの正本は `scripts/multi-agent.sh` の `get_cli_*` 関数。 `exclude_clis` は空白またはカンマ区切りの **1 文字列**で、そこに挙げた CLI をプランから外す（`--exclude-cli` と和集合。プラン表示に出所が出る）。`review.cross_review` は `auto`（既定）/ `off` の 2 値で、`--print-reviewers` の出力に出所付きで表示されるだけ — プラン・縮退警告・レポートは変えず、消費するのはスキル層（[self-review.md §クロスレビューの単一固定](./self-review.md#クロスレビューの単一固定cross_review)）。環境変数 `MULTI_AGENT_CROSS_REVIEW` とユーザー設定ファイルからも同じ優先順位（env → project config → user config）で解決される。 `review.confidence_threshold` は review の信頼度の報告閾値（0〜100 の整数、既定 80。環境変数 `MULTI_AGENT_REVIEW_CONFIDENCE_THRESHOLD` が優先）で、型付き判定行の confidence がこれ未満の finding を統合レポートの「未確認 › 低信頼の指摘」へ列挙する。範囲外・非整数の値は CLI を起動する前に名指しで止まる（[multi-cli-review-orchestration.md §型付き判定の集計](./multi-cli-review-orchestration.md#型付き判定の集計)）。
...
複数系統のレビュー指摘を fix エージェントへ渡すときは、両レビューの指摘を1本のプロンプトへ束ね、指摘ごとに番号を振り、各項目の先頭に親の裁定を置く（OBS-144）。全項目に4区分すべてを書くのではなく、該当する1区分を選ぶ。

| 裁定 | 使いどころ |
| --- | --- |
| [修正] | 直す。理由と実測の根拠を添える |
| [棄却] | 直さない。既知の反証を添える。具体的な失敗シナリオのない契約テスト要求なども、レビュー対応ポリシーで判断する |
| [文書化] | コードを変えず、限界として明記する |
| [判断委任] | 親の推奨を添えて結論を委ねる。マージ後の状態・実ホストの配布物など、親の裁定が実測で覆りうる項目に使い、fix 側は反証の実測と採用した結論を返す |

裁定は[一次情報確認](#委譲プロンプトへ載せる事実主張の一次情報確認)を済ませて渡し、未解消の Critical / Warning をラベルだけで解消扱いにしない。単一系統の軽い指摘へ新しい分類手順を強制せず、レビュー・再検証の要否は既存のレビュー対応ポリシーに従う。
...
背景（実測 3 セッション。出典は本テンプレートのソースリポジトリの観測台帳 OBS-036）:
...
3 セッション目は到達性を塞いだ後の実測で、**委譲プロンプト自身に規定が書いてあっても**同じ形で
停止し、親の明示的な再指示の直後にも繰り返した。**文言を置く対策は 3 セッション連続で効いて
いない**ので、遵守は文言ではなく機構で担保する（下の 0）。

0. **委譲先の background 起動は PreToolUse hook が機械的に止める**。同梱の
   `hooks/guard-long-gate-background.sh` が、サブエージェントの Bash 呼び出しを
   **抜け道付きの deny** で止める。トリガは 2 つあり、**名簿に縛られるのは片方だけ**である。
   - **明示 background（`run_in_background: true`）は、コマンドを問わず止める。**
     判定材料は構造化フィールド 1 つで、コマンド本文を 1 文字も読まない。所要時間の
     予測が要らないので名簿と無関係に成立し、散文由来の誤検知面も増えない
   - **ハーネスの自動 background 化（Bash ツールの `timeout` が未指定 / foreground 上限
     未満）は、名簿に載った長時間ゲートだけを止める。** こちらは「このコマンドは上限を
     超えるか」の予測が要り、一般化すると `timeout` を書かない短いコマンドが全部 deny に
     なる（変異注入で実測）

   名簿は全件ゲート（`run-all.sh`）と変異マトリクス（`run-mutations.sh`）で、依存
   インストール等の綴りは含めない — 自然文へ紛れ込む綴りは、引用符を跨ぐ近似的な
   コマンド分割と組み合わさって無害なコマンドの誤検知を生む（実測）。ただし `.sh` の
   basename も散文に現れる（本テンプレートのソースリポジトリの docs 自体が
   `bash …/run-all.sh` をコマンド行の形で持つ）ので、散文起因の誤検知は名簿ではなく
   「**引用符を含むコマンドは最初のセグメントだけを見る**」規則が閉じている。委譲先が
   検証報告を PR / Issue 本文へ書く操作（`gh pr comment --body "…"` の複数行引数）が
   最も踏みやすい形だった。止めるのはゲートを**実行する**呼び出しだけで、ゲート名を
   引数として触るだけの操作（`git add` / `git diff` / `chmod` / `bash -n`）は対象外である。

   **名簿へ足せない綴りがある。** 各 suite 共通の入口（`tests/<suite>/verify.sh`）は
   大半が数秒で終わるので、名簿へ足すと「`timeout` を書かずに単体 suite を回す」日常の
   呼び出しが全部 deny になる。名簿は所要時間の予測なので、suite ごとに所要が違う共通
   入口は原理的に載せられない。実際にこの綴りで素通りした回があり（変異を当てながら
   個別 suite を回すループ）、明示 background 側のトリガはその実測を受けて足した。
   自動 background 化でこの綴りが背景へ回る経路は開いたままである。

   以下の 1〜3 は引き続き委譲プロンプトへ常置するが、
   それは hook が届かない環境（hook 未導入のホスト・名簿外の綴り・hook が意図的に
   取りこぼす形）のための冗長であって、遵守の主たる担保ではない。取りこぼす形には
   **引用符を含むコマンドの 2 本目以降のセグメント**も含まれる（`cd "/repo" &&
   bash …/run-all.sh` は素通しする。原因はコマンド置換ではなく引用符の有無で、
   引用符の無い `cd /repo && bash …/run-all.sh` は捕まる）ので、「hook があるから
   委譲プロンプトには書かなくてよい」とはならない。**この取りこぼしは自動
   background 化の経路に限る** — 明示 `run_in_background: true` なら同じ形でも止まる。
   - **親（オーケストレータ）の呼び出しは対象外**。意図的に background でゲートを回す運用を
     壊さないため、発火条件に「呼び出し元がサブエージェントであること」を含めている。判別は
     hook 入力の `agent_type` の有無による（実測: 親の `PreToolUse` には `agent_id` /
     `agent_type` がキーごと無く、サブエージェントには在る。`session_id` / `transcript_path`
     は親子で同一なので判別材料にならない）。
   - **発火は記録される**。deny と抜け道通過は `${TMPDIR}/ff-dev-toolkit-delegation-guard/`
     配下の session_id 別ログへ 1 行ずつ残るので、親は「どの子がゲートを background で
     起こそうとしたか」を一覧でき、対策後の発生回数も数えられる。
