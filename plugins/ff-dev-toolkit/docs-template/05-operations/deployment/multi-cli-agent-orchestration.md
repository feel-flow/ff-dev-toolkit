...
0. **長時間ゲートの background 起動は PreToolUse hook が機械的に止める**。同梱の
   `hooks/guard-long-gate-background.sh` が、サブエージェントの Bash 呼び出しのうち
   全件ゲート（`run-all.sh`）を `run_in_background` で起こすもの、および Bash ツールの
   `timeout` が未指定 / foreground 上限未満のものを **抜け道付きの deny** で止める。
   止めるのはゲートを**実行する**呼び出しだけで、ゲート名を引数として触るだけの操作
   （`git add` / `git diff` / `chmod` / `bash -n`）は対象外である。
   **名簿は `run-all.sh` だけで、依存インストール等の綴りは含めない** — 自然文へ
   紛れ込む綴りは、引用符を跨ぐ近似的なコマンド分割と組み合わさって無害なコマンドの
   誤検知を生む（実測）。ただし `.sh` の basename も散文に現れる（本テンプレートの
   ソースリポジトリの docs 自体が `bash …/run-all.sh` をコマンド行の形で持つ）ので、
   散文起因の誤検知は名簿ではなく「**引用符を含むコマンドは最初のセグメントだけを
   見る**」規則が閉じている。委譲先が検証報告を PR / Issue 本文へ書く操作
   （`gh pr comment --body "…"` の複数行引数）が最も踏みやすい形だった。以下の 1〜3 は引き続き委譲プロンプトへ常置するが、
