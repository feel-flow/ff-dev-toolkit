...
> **適用範囲の注意**: ここでいう「Windows 対象」は、**AI エージェントが Windows 上で行う破壊的操作を止める**という話であり、**ツールキット自身のスクリプトが Windows ネイティブで動くという意味ではありません**。配布物は端から端まで bash なので、ツールキットの実行には WSL2 か Git Bash が要ります（[対応プラットフォーム](./multi-cli-review-orchestration.md#対応プラットフォーム)）。本書の PowerShell 例は、エージェントが Windows 側で叩くコマンドを安全側に倒すためのものです。
