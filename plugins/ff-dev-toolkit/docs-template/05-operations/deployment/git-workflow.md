...
> **運用原則**: 本ワークフローは [ワークフロー運用原則](./workflow-principles.md)（ノンストップフロー（フルオート）・スコープ外発見の YAGNI / インライン / Issue 化・曖昧仕様確認タイミング・定型判断の即決と熟慮の起動条件）に従って運用します。
...
**観測台帳も同じ導線で引く。** `/retrospective` が育てる観測台帳（[OBSERVATIONS.md](../../08-knowledge/OBSERVATIONS.md)）は書く store であると同時に引く store で、読み出し経路が無いと観測は「再発してから数えられる」だけで再発を防ぐ側に回らない（同じセッションで ACE の知見は作業中に届き、台帳の同じ主張は振り返りの事後にしか突き合わされなかった実測が起点）。本節の実行手段は `/knowledge-lookup`（ff-dev-toolkit）で、PLAYBOOK と台帳を同じキーワードで 1 回引き、store ごと・状態ごとに切り分けて返す。記録の契約は台帳側にも同形で適用する（ヒットした OBS ID / `0 件` / `台帳なし` / `読み取り失敗（理由）`。スクリプト末尾の `RECORD` 行がこの形になっている）。台帳の `promoted` / `mitigated` は「対策あり」として読み、`active` だけを未対策の落とし穴として扱う — 対策があるのに独自の回避策を実装へ持ち込むと対策の二重化になる（読み方の正本は `/knowledge-lookup` の SKILL.md）。台帳を参照しても `Count` は動かさない（`Count` は再発の計上であり参照の計上ではない）。
...
**親側の読み取り検査の例外**: [multi-review の「レビュー待ち時間の使い方」](../../../skills/multi-review/SKILL.md#レビュー待ち時間の使い方)が定める baseline ゲートだけは、その条件を満たす場合に凍結中も並走できる。レビュー担当側のビルド・テスト禁止は維持する。
...
**機械的強制の有無は経路で違う**: `multi-review`（`multi-agent.sh` 経由）は起動時にレビュー対象 diff を固定したうえで、実行の**前後のスナップショット差**でリポジトリの変化を検知し、動いていれば結果を DISCARDED にして統合レポートを生成しない。ただし前後の差で見る以上、**途中で変更して元へ戻した場合と `.gitignore` 済みパスへの書き込みは見えない**（機構は凍結の代わりにはならない）。一方**ホストのサブエージェント（Review Toolkit 等）にはこの検査自体が無く、この手順だけが防御になる**（worktree 隔離を使わずに起動した場合。隔離起動の既定は後述）。上の実測はサブエージェント経路で起きている。ただし ff-dev-toolkit を導入した環境では、**非隔離で起動したレビュー用サブエージェントに対して機械側の層が 1 枚入る** — 起動ごとに走行中レーンが 1 本開き、**起動した全レーンが終端するまで**、親の編集系ツールと git の書き込みコマンドが抜け道付き deny で止まる（1 本のレビュアーが返ってきただけでは解けない）。これは結果を DISCARDED にする検査の代わりではなく、この手順を機械的にも支える層である。レーンを取らない範囲 — 隔離起動・ガードの名簿に無い subagent・hook を opt-out している間 — では、従来どおり手順だけが防御になる。
...
**実行中に入力が変わった結果は、緑でも証拠に使わず破棄し、変更を終えてから再実行する。** 途中で元へ戻した場合も同じで、開始・終了時の SHA 一致だけでは無変更を証明できない。長時間ゲートを background へ回すコマンドは、状態を変える短い検証・修正コマンドと連結しない。**状態を変える複数行の手順は `bash -e <<'EOF' … EOF` の形で Bash ツールへ渡し、プロンプト等の特殊文字（バッククォート・`$VAR`・条件展開）を含む文字列はヒアドキュメントで一時ファイルに書いてから渡す。** 対話シェルが zsh だと複数行スクリプト中の `$VAR` 展開や `set -e` の停止が bash と異なる挙動になり、無言に欠落・空振りする（出典は本テンプレートのソースリポジトリの観測台帳 OBS-092）。
...
**並行マージの静止確認と並べて、同じ作業ツリーで動く並行セッションの実測も行う**（出典は本テンプレートのソースリポジトリの観測台帳 OBS-014）: `git status --porcelain --untracked-files=all`（未追跡ファイルを含む）と `git worktree list --porcelain`、`git branch --show-current` で、別セッションが同じ作業ツリーへ untracked ファイルを書き込んでいないか・別ブランチへ checkout していないかを確認する。並行マージの静止（develop の前進）とは別軸の実測で、`gh pr list` は同じ作業ツリーを共有する他セッションの存在を教えない。検出したら削除せず、パス・clean/dirty・最終更新を報告して着手を見送る（処分はユーザー判断。手順の詳細は `.claude/skills/sync-dev-toolkit/SKILL.md` 手順 0a）。
...
ACE 完了後、チェーンの末尾として `/retrospective` を毎回実行する（`/merge-cleanup` → `/ace-curate` → `/retrospective`）。ACE がコード・設計のプロジェクト知見を Playbook へ蓄積するのに対し、`/retrospective` はプロセス/ツール/スキルのメタ知見（そのセッションで**実測した**手戻り・無駄時間）から改善提案を最大 3 件出す（該当なしなら「振り返り: 改善候補なし」の 1 行で終了）。起票は既存確認 → 提案 → 対象 repo へ Issue 作成の順で、既定では承認を待たずに起票して発行番号を振り返り結果で報告する（`RETROSPECTIVE_FILING=ask` で従来の承認待ち式へ戻せる。正本はスキルの「承認と起票」）。提案の提示前に既存知見・既存 Issue との重複を確認し、重複していた提案は新規起票ではなく既存への追記へ切り替える（確認手順の詳細はスキルの「起票前の既存確認」。手順そのものはここへ複製せず参照する）。

観測は起票の前に**作業中リポジトリ**の観測台帳（`docs/08-knowledge/OBSERVATIONS.md`。無ければスキルがテンプレートから作成する）へ蓄積し、同じ観測の再発はエントリの Count +1 に畳む。Issue 起票を提案するのは原則として Count が累計 3 回に到達した再発（または一回でも重大な特急レーン）だけで、起票先は改善対象で分岐する — ツール群（スキル・hook・スクリプト）の改善は到達可能な SSOT（不能なら配布元）、プロジェクト固有のプロセス改善はこのリポジトリ自身。台帳の作成・定型記録は承認不要、Issue の新規起票も既定では承認を待たない（`RETROSPECTIVE_FILING=ask` のときだけ承認後。詳細はスキルの「観測の記録」「承認と起票」。手順そのものはここへ複製せず参照する）。

対応ホストでは UserPromptSubmit で応答前に注入し、Stop hook は実行漏れ時だけ自動継続する。`RETROSPECTIVE_MODE=ask` で実施前確認、`off` で自動発火を無効にできる。起票のスイッチは別で、`RETROSPECTIVE_FILING=ask` が承認待ち式へ戻す。
