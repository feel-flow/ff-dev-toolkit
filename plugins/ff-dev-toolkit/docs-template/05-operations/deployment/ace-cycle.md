...
domainは通常curateの標準収集対象です。PRからの自動収集でも毎回評価し、`--source`は不要です。domain確認は評価済み（候補N件）と未実施（理由）を区別して報告します。

[ACE ドメイン知識契約](./ace-domain.md)が読取許可範囲内にあればGenerate前に読む。未配置・範囲外の場合は、プラグインのcurateは同梱契約、自律収集はace-captureの「domain保存契約」を適用し、補足文書の不在だけでは停止しない。読取範囲は広げない。PR経路と `--source` / `--issue` の資料単独経路は同じ7観点を使う。資料の取得失敗は未確認と報告する。業務知識はdomainで根拠・確認状態・反映先を保持し、設計書変更案はace-refineの別PRへ分離する。
...
**対応**: (1) 旧テーブル形式のエントリを `/ace-refine` でコンパクト正準へ正準化する（原文は `playbook/archive/` へ verbatim 保全）。(2) それでも再超過が常態化するなら [PLAYBOOK.md のファイル分割ルール](../../08-knowledge/PLAYBOOK.md#ファイル分割ルール) に従って、検索語彙が分岐するときだけカテゴリ分割する。総量そのものは件数ゲート（refine 目安 130 件・警告 / ブロック上限 280 件・exit 1）が担当する
...
- 直 push の対象を knowledge 単独コミット（PLAYBOOK・観測台帳の追記）とし、`/ace-curate` と `/retrospective` の両方が書いた回は共通の書き込み口が 1 コミットに畳むことを明記した

### [1.4.0] - 2026-08-31

#### 変更

- 件数ゲートのブロック上限の既定値を更新した
