...
- **実行タイミング**: 月次、または `check-category-size` の 130 件ゲートがブロックしたとき・エントリ密度の警告（導出上限超過）が出たとき
...
**対応**: (1) 旧テーブル形式のエントリを `/ace-refine` でコンパクト正準へ正準化する（原文は `playbook/archive/` へ verbatim 保全）。(2) それでも再超過が常態化するなら [PLAYBOOK.md のファイル分割ルール](../../08-knowledge/PLAYBOOK.md#ファイル分割ルール) に従ってカテゴリ別に分割する。総量そのものは件数ゲート（130 件・exit 1）が担当する
