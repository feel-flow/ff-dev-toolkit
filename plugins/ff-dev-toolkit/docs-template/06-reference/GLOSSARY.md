...
プロジェクト固有の用語をここへ追加する。**形式**: `### ` + 用語名（必要なら丸括弧で英語表記・略称）の見出し 1 行 + 空行 + 定義 1〜3 行。

> **この節に「実在しない用語の見出し」を置かないこと。** 同梱 MCP（spec-docs）の `glossary_lookup` は行頭の `###` を行単位で拾って用語として索引する。閉じたコードフェンス内・閉じた HTML コメント内・`## Changelog` 節以降は除外されるが、**閉じ忘れたフェンス / コメントは除外区間として扱われない**（以降の実在用語を無言で消さないための fail-safe）。したがって記入例は見出し行として書かず、この段落のように散文で説明する。
...
- 「プロジェクト固有用語」節の記入例を、見出し行（`### [プロジェクト固有の用語を追加]`）から散文の説明へ変更。同梱 MCP の用語索引は行頭 `###` を行単位で拾うため、雛形の見出しがそのまま実在しない用語として登録されていた
