...
**新規 ADR の番号は目視 grep で決めない。** 本文中の参照リンク（`[ADR-021](#adr-021)` 等）が見出しと混ざるため、`grep ADR- | sort | tail` の出力は「最後の見出し」を指さない（実測: 参照リンクを 358 個含むファイルで最新が ADR-069 なのに `sort -u | tail -3` は ADR-064 までしか見せず、ADR-065 を採番しかけた）。

1. 採番検証スクリプトがあるリポジトリでは、その出力を根拠にする（例: `npx tsx scripts/check-adr-numbers.ts` → `ADR-001..ADR-0NN` の NN + 1 を採る）
2. スクリプトが無い場合は**見出し行だけ**を対象に数値順で最大値を採り、+1 する:

   ```bash
   grep -oE '^## ADR-[0-9]+' docs/06-reference/DECISIONS.md \
     | grep -oE '[0-9]+' | sort -n | tail -1
   ```

   パターンを数値限定（`^## ADR-[0-9]+`）にするのは、本ファイルの「決定記録テンプレート（AI最適化版）」のフェンス内雛形 `## ADR-XXX` を拾わないため。grep はコードフェンスを解釈しないので、数値を許さない `^## ADR-` だけのパターンでは雛形の行も一致してしまう。
