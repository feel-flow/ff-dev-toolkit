...
> **📌 独立定義**: 本エージェント定義は GitHub Copilot custom agent 向けの独立定義であり、Claude Code 側の code-review パースペクティブ（信頼度の報告閾値・見出しの帯域）とは同期しません。両者の値は個別に管理し、変更時は本ファイル内で自己矛盾がないことのみを確認してください。
...
| 優先度 | 意味         | 例                                                                     |
| ------ | ------------ | ---------------------------------------------------------------------- |
| 9-10   | クリティカル | 変更 diff が導入または悪化させた未検証動作で、データ損失、セキュリティ、システム障害の可能性 |
| 7-8    | 重要         | 変更 diff が導入または悪化させた未検証の分岐・エラーパスで、ユーザー向けエラーの可能性 |
| 5-6    | エッジケース | 混乱や軽微な問題。既存コードへの網羅提案はここが上限                   |
| 3-4    | Nice-to-have | 完全性のため                                                           |
| 1-2    | オプショナル | 改善の余地                                                             |

優先度 7 以上（Critical Gaps / Important Gaps）に置けるのは、変更コードが導入または悪化させたテスト欠落だけ。既存コードへのカバレッジ網羅提案と、現時点で正しい挙動への回帰テスト不足は、影響の大きさに関わらず最大でも優先度 5-6（Edge Case Gaps）へ置く。
