...
> **カテゴリ例外**: 観点テンプレートは、損失の非対称が大きいカテゴリに限って報告閾値の例外を持てる（例: `code-review` 観点の「報告閾値のカテゴリ例外 — テスト有効性」は、3 形に限り信頼度 50 以上を Important へ置く）。例外を設ける場合は、適用範囲を判定可能な粒度で書き、範囲外へ広げない宣言と、その観点の信頼度スコア表・Severity Classification・Notes・Output Template を同じ PR で整合させる。
| 0-49       | 低確度       | 報告しない           |
...
- Claude / Codex は相互にフォールバックし、両方が導入済みならクロスモデル性を維持する（片方だけの環境の扱いは [self-review.md §レビュー担当の選択と利用制限時の継続](../05-operations/deployment/self-review.md#レビュー担当の選択と利用制限時の継続) が正本）
