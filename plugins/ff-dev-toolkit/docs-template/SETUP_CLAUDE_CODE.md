...
- TypeScript strict mode: 新規では推奨。既存では段階導入も比較して合意
- テスト方針: 重要経路から推奨し、数値目標は必要なら合意
...
| ルール                   | 適用方法                                                          |
| ------------------------ | ----------------------------------------------------------------- |
| **意味のある値の定数化（採用時）** | 名前付き定数を使用。単位・有効範囲をコメントに記載                |
| **型安全性**             | TypeScriptではstrictを推奨し、既存構成との互換性を確認して合意 |
| **ファイルサイズ**       | ソフトリミット: 500行, ハードリミット: 800行                      |
| **関数サイズ**           | 30行以下を目標                                                    |
| **テストカバレッジ**     | 必要な場合に数値目標を合意する                                      |
| **エラーハンドリング**   | 言語と既存設計に合う例外／Result型などの採用方針、console.logは本番環境で禁止                   |
...
2. **インライン修正**: 必要かつ軽微（仕様判断・別モジュール波及・独立検証・実装 10 行超（テスト・fixture は数えない）のいずれにも明確に該当しない。近傍の設定ファイルの小変更も含む）なら、現 PR で修正
3. **Issue 化**: 仕様判断・別モジュール波及・独立検証・実装 10 行超のいずれかに明確に該当するなら（仕様判断・波及の 2 軸は不確かでも Issue 化へ倒す）、先に類似 Issue を検索。同じ完了条件なら既定はコメントでまとめ、同一 PR からの複数発見は既定で 1 Issue に束ねる。本文 AC は明示許可と競合確認がある場合だけ最小追記し、独立する場合だけ関連 Issue を作成
...
1. **具体的な要件を明示**
   ...
   - ユーザー認証（JWT使用）
   - パスワードハッシュ化（bcrypt使用）
   - エラーハンドリング（言語と既存設計に合う例外／Result型などの採用方針）
   - ユニットテスト（合意した検証対象を確認）」
   ```
...
- 意味のある値の定数化（採用時）
- any型禁止
- エラーハンドリングは例外／Result型などから合意
- テストの対象と必要な数値目標はリスクに応じて合意
