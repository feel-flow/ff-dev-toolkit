...
### ACE-79-1: 手順に無いステップは実行されない — 手順修正と機械ゲートはセットで入れる
...
**Insight**: 運用手順に書いていないステップは、誰も（AI も含め）実行しない。手順を直すだけでは次の実装者が別理由でスキップして同じ漏れが再発する。同じ不変条件を機械検証（CI / quality gate）に載せて初めて閉じる。

**Context**: `/ace-curate` の 4-c は version / updated / ace_entry_count だけを指示し、PLAYBOOK の `## Changelog` 追記が無かった。結果 ACE-61/66/70 系が Changelog から漏れ、port レビューで「version 1.60.0 なのに Changelog 最新が 1.59.0」と発覚した（Issue #76）。

**Action**:

1. 運用手順を直す PR では、同じ不変条件を `npm run ... --check` 系ゲートに足すかを必ず検討する（「手順だけ」は再発前提）。
2. ゲートは既にある検証スクリプトへ最小差分で足す（本 PR では `ace_entry_count` 検証に version↔Changelog を同居させた）。
3. 手順側にはゲート実行ステップ（本リポジトリでは 4-e）を書き、commit 前に exit 0 を必須にする。
