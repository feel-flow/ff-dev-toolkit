...
| **追記の抽象度の下限**             | 判定は「**適用条件が固有名なしで書けるか**」（ADR-047）。本文（タイトル・適用条件・アクション）の固有名（Issue/PR 番号・ファイルパス・特定コマンド/API/スクリプト名）のうち、**別の名前へ置き換えても主張とアクションが成立するもの**は 1 段上の性質へ書き直す。置き換えると処方が導出できない固有名は残す。**例示としての固有名は常に残してよい**。過度な抽象化は新規性バーを逆向きに使って棄却する（下の「抽象度の下限と棄却基準」を見ること）。機械ゲートではない — `ace-abstraction-report.ts` が候補を出し、判定は `/ace-curate` の新規性バーで行う |
...
**なぜ下限が要るか**: 具体的すぎるエントリは、次の類似インシデントで「取るアクションが違う」と判定されて**新規追加**される。1 段上げておけば推論で隣接ケースを覆えるので、次の類似事象は `Helpful` +1 に落ちる。件数の**流入**を下げられる経路はここだけである（archive の供給は構造的にほぼ 0 件）。

**下限の当て方**: 本文の固有名を 1 つずつ「別の名前に置き換えても主張とアクションが成立するか」で試す。成立するなら 1 段上の性質へ書き直し、固有名は例示として残す。

| 現状の書き方 | 1 段上げた主張 |
| ------------ | -------------- |
| `find \| wc -l` は失敗しても 0 を出す | 件数を出す外部コマンドは失敗しても 0 を出す。件数の入力は生の出力で受け、終了コードと非空を確認してから使う（`grep -c` / `jq length` も同じ） |
| 配布対象 docs-template を変えたら plugin.json を bump する | 配布物の一部を変えたら、その配布物のメタデータを同じコミットで同期する |
| 対話シェル zsh の `for e in $VAR` は単語分割されない | 実行シェルが想定と違う実行単位では、シェル依存構文がエラーを出さずに全滅する。件数を期待値と照合してから本実行する |

**棄却基準（過度な抽象化）**: 新規性バーを**逆向き**に使う — 上位主張から**元エントリそれぞれのアクションが再導出できない**なら、その抽象化は棄却して元の粒度を残す。固有名がアクションの本体である類型は次の 5 つで、これらは抽象化しない。

1. **プラットフォーム実装差** — 同名コマンドの実装ごとの挙動差（BSD / GNU の `mktemp`・`sed` など）。上げると代替形が導出できない
2. **特定 CLI の usage と実挙動の乖離** — 「そのオプションは併用できない」型。固有名そのものが処方
3. **言語ランタイム仕様** — 特定 API の定義された挙動（正規表現フラグの効果、文字列長の単位など）
4. **診断メッセージと原因の対応表** — 診断が原因に見えないことが価値の本体。症状の文字列を落とすと引けなくなる
5. **主張が既に 1 段上** — 本文の固有名が例示にすぎないもの。書き換える対象がない

**検出は機械ゲートではない**: `ace-abstraction-report.ts` は固有名シグナルで候補を挙げるだけで、**候補が何件出ても終了コードは 0**（非 0 になるのは入力そのものが測れないとき — パス不備・ACE エントリ 0 件・未閉フェンス・読み込み失敗）。提供元リポジトリでの実測で機械シグナル単独の精度は 47%（再現率 85%）であり、ゲートにすると過半が誤って止まる。判定は `/ace-curate` の新規性バー、統合の実施は `/ace-refine` R3-c の承認ゲートが行う。**このスクリプトは `/ace-setup` の既定手順では配られない**（`docs-template/scripts/ace/` をディレクトリ単位でコピーする任意手順を通した環境にだけ届く）。無い環境では下限の確認はレビュー時の目視で代替する。

**遡及**: 既存エントリの一括書き換えはしない。レポートが出す統合候補の組を `/ace-refine` R3-c で畳む。**旧テーブル形式のエントリは組の対象から外れている** — 行き先は R3-b（正準化）で、正準化した後に R3-c の対象へ入る。統合先本文を上位主張へ引き上げる書き換えは、上表の**例外 1（意味保存要約。原文の `playbook/archive/` への verbatim 保全が必須条件）**の適用範囲に収まる。
...
- **妥当な形状**（機械ゲート `check-entry-format` が強制する）: `ACE-` + 省略可の `i` + 数字列（`-` 区切りで複数段可）。旧連番形式（`ACE-001` / `ACE-1071` — 3 桁ちょうどは先頭ゼロ可、4 桁以上は先頭ゼロ不可）と 3 段以上（`ACE-1-2-3`）は妥当。**禁止**: 二重ハイフン（`ACE-337--1`）・アンダースコア（`ACE-1_9`）・英字 suffix（`ACE-01a` / `ACE-438-1a`）・連番の無い単段（`ACE-1` / `ACE-01` / `ACE-0001` / `ACE-i425` — 単段を許すのは旧連番形式だけで、PR 由来・Issue 由来とも連番が必須。`ACE-0001` は 4 桁以上で先頭ゼロなので旧連番形式ではない）。枝番が要る場合は suffix ではなく `-<連番>` を 1 段増やす（suffix を許すと、参照走査の単語境界の都合で同じエントリが suffix の有無で 2 通りに参照され、Helpful カウンターと stale 判定が分裂する）。
- **見本用の予約**: `ACE-000-*` は本テンプレート同梱の見本エントリ専用に予約する。対応する PR は存在しないため、実プロジェクトでは採番しない（見本を消せば `ACE-000-*` も消える）。
- **既存 ID の扱い**: 旧 `ACE-{連番}` 形式（`ACE-001`〜。3 桁ちょうど、または 999 を超えて 4 桁以上）のエントリは **改名しない**。旧連番形式と新 PRスコープ式は恒久的に共存する（参照・anchor 互換の維持。旧採番のまま 999 を超えて 4 桁へ到達した導入先も改名しない — 形状ゲートは先頭ゼロ無しの 4 桁以上も通す）。ID にファイル位置の情報は持たせないため、分割後も ID はそのまま維持する。
...
[ACE ドメイン知識契約](../05-operations/deployment/ace-domain.md)に従い、domainへEvidence / Verification / Distill-Toを記録する。主体・状態・例外条件は抽象化で削らない。Distilled-Toは設計書PRの検証とマージ確認後だけ記録する。4メタ行と15行上限を維持する。

## カテゴリ一覧

| カテゴリ                | 説明                                                 | 例                                   |
| ----------------------- | ---------------------------------------------------- | ------------------------------------ |
| `domain` | 根拠付きの業務知識・設計書への橋渡し | 主体別制約、状態遷移、仕様の理由 |
...
- **適用条件は固有名なしで書く**: 適用条件（2 文目）に書く固有名は、別の名前へ置き換えても主張とアクションが成立するなら 1 段上の性質へ書き直す（§運用ルール「抽象度の下限と棄却基準」。ADR-047）。固有名は**例示として**残してよい — 落とすのは「その名前でしか成立しない書き方」であって、具体例そのものではない。逆に、置き換えると処方が導出できなくなる固有名（プラットフォーム実装差・言語ランタイム仕様・診断メッセージの対応表など）は残す。
...
- §エントリID規則「妥当な形状」「既存 ID の扱い」の単段を「旧 3 桁形式」から「旧連番形式（3 桁ちょうどは先頭ゼロ可、4 桁以上は先頭ゼロ不可）」へ揃えた（旧採番のまま 999 を超えた導入先を改名なしで通す。[公開 ff-dev-toolkit の報告](https://github.com/feel-flow/ff-dev-toolkit/issues/120)）。

### [1.79.0] - 2026-09-02

#### 追加

- §運用ルールへ「追記の抽象度の下限」の行と `#### 抽象度の下限と棄却基準（ADR-047）` を追加。判定は「適用条件が固有名なしで書けるか」、過度な抽象化は「上位主張から元エントリのアクションが再導出できるか」で棄却する（固有名がアクションの本体である 5 類型を列挙）。検出は機械ゲートではなく候補提示 + 新規性バーの判定で、遡及は `/ace-refine` R3-c の統合サイクルへ乗せる。`ace-abstraction-report.ts` は `/ace-setup` の既定手順では配られないため、無い環境ではレビュー時の目視で代替することを明示した
- §記述ガイドラインへ「適用条件は固有名なしで書く」を追加（例示としての固有名は残してよいこと、置き換えると処方が導出できない固有名は残すことを明記）
