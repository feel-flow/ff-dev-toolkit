...
| **密度超過時は正準化 → refine → 必要なら分割** | 行数上限は**件数から導出**する（`ヘッダ行数 + 件数 × 16`。16 = 行数バジェット 15 + ブロック間の空行 1）。超過は「ファイルが大きい」ではなく「**1 エントリが太い**」の意味なので、第一対応は旧テーブル形式の正準化、次に `/ace-refine`。件数ゲートは二段（refine 目安 130 件・警告 / ブロック上限 280 件・exit 1）。分割は「同一カテゴリ内で検索語彙が明確に分岐する」ときだけ。ファイル分割だけでは Category 値が合算されるため件数ゲートは解除されない。分割レイアウトの索引 `PLAYBOOK.md` は行数監視対象外 |
...
1. **エントリブロックの内側に置く** — anchor 行〜終端 `---` の範囲内だけが有効（終端 `---` を欠くブロックは、次エントリの始点・次の `##` 見出しの手前を上限に、原文で最後の非空行までが範囲。末尾に置いた宣言マーカー行も内側）。ヘッダ・§運用ルール・Changelog の記述は加算されない
...
- **ID の一意性**（機械ゲート `check-entry-format` が強制する）: 走査対象（索引 `PLAYBOOK.md` + `playbook/*.md` 直下）で同じ ID を 2 回以上使わない。ID は anchor・索引の参照先・allowlist・再利用カウンタのキーであり、重複すると「allowlist 済み ID を再利用した旧形式の新規追記が通る」「参照リンクがどちらか一方にしか飛ばない」形で静かに壊れる。`playbook/archive/` は対象外（`/ace-refine` の compact は live と archive の両方に同じ ID を残すのが正常）。
- **見出し行は正準形で**（機械ゲート `check-entry-format` が強制する）: `### <ID>: <タイトル>` の形（`###` の後に半角スペース 1 個、行頭にインデントを置かない）。この形を外れた見出しはエントリの境界として認識されず、本文が直前のエントリへ**吸収**される（件数から消え、旧形式マーカーの検査も直前のエントリの allowlist 判定に乗る）ため、フェンス外の `### ACE-` 行が正準形と一致しなければ fail-loud に拒否する。
...
| `estimation`            | 工数見積もりの精度・予実の乖離                       | 見積もりの立て方、乖離の原因、補正   |

### `estimation` の抽象度契約（新設カテゴリ限定）

知見層は放っておくと具体的になりすぎる。具体的なエントリは次の類似ケースを「新しい知見」に見せるため、**入口で件数が発散する**。`estimation` は新設カテゴリなので、最初から抽象度の下限を契約として持たせる。

**具体はデータ層（GitHub Issue 本文の工数ブロック）に既にあるため、知見層に具体を書く必要がない。** 層分離がそのまま抽象度の強制装置になる。

| 契約 | 内容 |
| ---- | ---- |
| 固有名の禁止 | 本文に Issue / PR 番号を判断材料として書かない（メタ行の `Origin` は出典なので対象外）。特定の技術スタック名・ファイル名を適用条件に書かない |
| 適用条件の書き方 | 技術非依存の性質で書く（「外部契約の変更を伴う」「既存の安全ゲートを緩める」「並列実行で計測が汚れる」） |
| 書き方の型 | 「〈作業の性質〉を含む見積もりは〈方向〉へ外れる — 〈補正〉せよ」 |
| 件数上限 20 件 | 他カテゴリの件数ゲートとは別枠の、意図的に厳しい値 |

**件数上限 20 件の根拠**: 見積もりを外す原因の**類型**は本質的に少数であり、20 を超えたのは「新しい原因が見つかった」ではなく「同じ原因を別の言葉で書いた」の兆候である。ADR-041 の「上限は詰めてよい容量ではなく**再判断の発火点**」の考え方をそのまま適用する。この値は新しい契約であり、実運用の実測で見直す。

**発火点に注意**: この上限は `/ace-curate` の書き込み時には止まらない（カテゴリ件数ゲート `check-category-size` はカテゴリ共通の上限を持ち、カテゴリ別の上書き機構が無い）。21 件目の追記は成功し、赤くなるのは後から回帰スイートを回したときである。**「ブロックされなかったから 20 件未満だ」とは読めない。** 赤を見た時点で、抽象度を 1 段上げた統合（2 件を 1 件へ畳む）を行う。

**機械検査の所在**: 固有名の禁止（Issue / PR 参照）と件数上限は、**本テンプレの提供元リポジトリ**（ff-dev-toolkit）の回帰スイートが機械検査している。導入先プロジェクトへは検査そのものは同梱されないため、`/ace-setup` でこのテンプレを受け取っただけの環境では**機械では止まらない**。導入先で同じ検査を効かせるには、提供元の該当スイートを移植するか、レビュー時の目視確認で代替すること。書き方の型（文型）は提供元でも検査しない — 文型の強制は表現を壊すだけで、実効は他の 2 つで足りる。
...
- §`estimation` の抽象度契約から、実体の無い主張を除いた。件数上限は ACE の書き込み時には止まらないため「到達時は追加をブロックし」を撤回して発火点を明記し、機械検査は提供元リポジトリにあり本テンプレの導入先へは同梱されないことを明示した

### [1.77.0] - 2026-09-01

#### 追加

- §カテゴリ一覧に `estimation`（工数見積もりの精度・予実の乖離）を追加し、§`estimation` の抽象度契約を新設した。契約は固有名の禁止・技術非依存の適用条件・件数上限 20 件の 3 点で、うち固有名の禁止と件数上限を提供元リポジトリの回帰スイートが機械検査する（導入先へは検査を同梱しない）。具体は工数の実データ側（Issue 本文）に持つため知見層に具体を書く必要がない、という層分離を抽象度の強制装置として使う

### [1.76.0] - 2026-08-31

#### 変更

- §運用ルールの件数ゲート記載を、ブロック上限の既定値の更新に合わせて更新した
...
- §エントリID規則に、機械ゲート `check-entry-format` が強制する 2 規則（走査対象内での ID の一意性。`playbook/archive/` は対象外 / 見出し行が正準形 `### <ID>: <タイトル>` であること）を追記した
- §運用ルール「末尾追記のみ」に例外2（**ID 衝突の修復**に限り、見出し行・anchor 行・索引の参照先の書き換えを許可。本文は無改変で、改番を本 Changelog に記録）を追記した

### [1.72.0] - 2026-08-19

#### 変更（運用ルール）

- 有効条件 1 に、終端 `---` を欠くブロックの範囲規則（次エントリ始点・`##` 見出しの手前を上限に、原文で最後の非空行まで。末尾の宣言マーカー行も内側）を明記した

### [1.71.0] - 2026-08-14

#### 変更（運用ルール）

- 件数ゲートを二段化した（refine 目安 130 件・警告 / ブロック上限 180 件・exit 1）。分割は検索語彙が明確に分岐するときだけとし、判断手順を §ファイル分割ルールへ追加した
