...
**限界**: 判定は markdown を解さない素のテキスト一致です。エントリ本文がコードスパンやコードフェンスの中に `<!-- … -->` の完全形でマーカーを引用していると、1 件として数えます（`ace-refine-report.ts` も同じく数えるので両者はそろっています）。エントリ本文で書式を説明するときは `<!--` を崩して書いてください。
