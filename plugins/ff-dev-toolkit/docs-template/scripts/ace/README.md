...
**保証の範囲**: ファイル全体で対にならない未閉フェンスについては、本スクリプトと `check-category-size.ts` が揃って拒否します（逆向き——本スクリプトが落として check が通る——は存在しません）。ただし走査単位が違うため（check は**エントリセグメント単位**、本スクリプトは**ファイル全体**）、セグメント境界をまたいで別のフェンス行と対になる閉じ忘れは check だけが拒否します。この非対称は未解消です（Issue #353）。
