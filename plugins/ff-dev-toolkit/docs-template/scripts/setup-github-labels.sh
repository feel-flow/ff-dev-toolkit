...
# ラベル（バージョニング / 緊急度 / 優先度 / 分類の補完）のうち、
# 対象リポジトリに**存在しないものだけ**を作成する。既存ラベルには一切触れない
# （色・説明の上書きもしない）— 消費プロジェクトが意図的に変えた色や説明を、
# セットアップの再実行が黙って戻すのを防ぐため。
...
# `priority:` のように名前空間を区切るコロンは許す）。
# github-setup.md の「カスタムラベル（追加が必要）」表と一致させること。
#
# ⚠️ `epic` は作ると挙動が変わるラベルである。`out-of-scope-issue` はこのラベルの
...
#    残る 12 件は分類が増えるだけで、他スキルの分岐を開かない。
...
priority:critical|B60205|優先度 最高（本番で実害が進行中）
priority:high|E99695|優先度 高（利用者に見える不具合・開発をブロック）
priority:medium|F9D0C4|優先度 中（通常対応。実害は未顕在）
priority:low|FEF2C0|優先度 低（保留可・実需要待ち）
follow-up|006B75|PR レビュー・実装から派生した追跡課題
