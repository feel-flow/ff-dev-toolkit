...
| 遷移            | トリガー   | ガード条件（事前条件） | 発行イベント |
| --------------- | ---------- | ---------------------- | ------------ |
| [状態A → 状態B] | [トリガー] | [条件]                 | [イベント]   |
| [状態B → 状態C] | [トリガー] | [条件]                 | [イベント]   |

- 状態を持つエンティティごとに状態遷移図と遷移ルールを定義する
- 許可されない遷移（例: `Shipped → Draft`）はガード条件ではなく型・状態機械で防ぐ（実装パターンは 03-implementation/PATTERNS.md に定義すること）
- 遷移時に発行するドメインイベントは「7. ドメインイベント」のイベント一覧と対応させる
...
- 「9. 状態遷移」セクションを追加し、以降の節番号を繰り下げ（標準の必須セクション適合。プラグイン配布側#82 の上流還元）
