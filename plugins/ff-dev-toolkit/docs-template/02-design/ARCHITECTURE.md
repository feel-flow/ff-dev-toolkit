...
> このテンプレートと下の記述例の番号（`ADR-00X` / `ADR-00Y`）は非実在番号です（ADR記述例の注記を参照）。
...
> §9 の ADRテンプレートとこの記述例（コードブロック内）の番号（`ADR-00X` / `ADR-00Y`）は非実在番号です。ADR を追加していく過程でも、コードブロック内の例示を実番号（`ADR-001` 等）に書き換えないでください。同一ファイル内で番号が衝突します。§8 技術選定一覧の ADR 列の `ADR-00X` は、対応する ADR を起票した時点で実採番に置き換えます。
