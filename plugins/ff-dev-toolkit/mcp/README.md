...
- **前提**: `node` >= 22 が PATH にあること（バンドルは `--target=node22`、top-level await 使用。18/20 は EOL のためサポート外）。本パッケージの開発（vitest 実行・ビルド）は `engines` どおり >= 22.12.0 が必要（Vite 8 の engines 要求に合わせている）
