...
consumer_roster() {
  ...
  for verify in "$TESTS_DIR"/*/verify.sh; do
    ...
    if awk '
      /^[[:space:]]*#/ { next }
      /(build_isolate_env|run_isolated|unset_isolated_vars|unset_prompt_env_vars)/ { found = 1 }
      END { exit found ? 0 : 1 }
    ' "$verify"; then
...
check_consumer_roster() {
  ...
  if [ "$actual" != "$expected" ]; then
    echo "callsite名簿drift: 共通ライブラリconsumerが現行17 suiteと一致しない" >&2
    diff -u <(printf '%s\n' "$expected") <(printf '%s\n' "$actual") >&2 || true
    return 1
  fi
}
...
expect_pass "現callsite名簿はreview-wrapper-shimを含む17 suite" check_consumer_roster
