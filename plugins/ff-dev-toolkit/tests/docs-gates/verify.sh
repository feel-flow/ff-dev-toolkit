...
# --- 04-quality/TESTING.md: 変異注入の結果の読み方と書き戻し先 ---
# 前の針は「変異が当たったか」を確かめる作法だけを固定しており、**当てた結果をどう読むか**
# （赤転しなかった変異の扱い）と**どこへ残すか**（検査自身のヘッダ）は配布側に無かった。
...
must_contain_section "$f" '### 変異注入の結果の読み方と書き戻し先' 'どうしても赤にできない変異は、消さずに理由とともに残す' \
  "TESTING.md の配布側が測れない変異を理由つきで残すよう求めている"
must_contain_section "$f" '### 変異注入の結果の読み方と書き戻し先' '結果は当該検査自身の先頭コメント（ヘッダ）へ書き戻す' \
  "TESTING.md の配布側が結果の書き戻し先を定めている"
must_contain_section "$f" '### 変異注入の結果の読み方と書き戻し先' '同じ対象へ 2 つの変異を**同時に**当てるのも避ける' \
  "TESTING.md の配布側が同一対象への同時変異を避けるよう求めている"
must_contain_section "$f" '### 変異注入の結果の読み方と書き戻し先' 'コミット前・その場で変異を当てる' \
  "TESTING.md の配布側が変異の実施時期（書いた直後）を定めている"
must_contain_section "$f" '### 変異注入の結果の読み方と書き戻し先' 'TODO を残したまま新設の検査をマージしない' \
  "TESTING.md の配布側が未解消 TODO のままのマージを禁じている"
must_contain_section "$f" '### 変異注入の結果の読み方と書き戻し先' '検査を足せば赤にできる変異は前項が優先する' \
  "TESTING.md の配布側が「理由を書けばマージしてよい」の抜け道を塞いでいる"
