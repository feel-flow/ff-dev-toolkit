...
| case 14 | （run-all.sh の登録照合を `FF_RUN_ALL_CHECK_REGISTRATION=1` で実行 + `REQUIRED_SUITES` 配線の静的監査） | 必須名簿が定義され、**実体（各 verify.sh の suite 全体 skip 経路と `run-all-required:` 宣言）から導出した必須集合と双方向で一致**すること。両方向の失敗経路が実装に在ること。skip が終了コード判定（`REQUIRED_SKIPPED`）へ配線されていること。走査の失敗と理由を欠く宣言が fail-closed であること。導出述語の正負の対照（単一引用符・`printf` の行頭 skip は拾い、アサート行の期待値文字列とインデント付き部分 skip は拾わない。理由なしの宣言は `bad` として印が付く。本文付きの `# 空振り検出:` は `probe`、本文なしは `bad`）を `FF_RUN_ALL_DUMP_DECLARATIONS=1` の probe tree で実測する。名簿の 17 名ハードコード・ミラーは逆向き導出の導入で削除した |
| case 14b | （宣言ダンプから起こした複製木 + 合成した犠牲 suite `miss-probe-victim` の名簿・stub を崩して登録照合） | 空振り検出の宣言（`MISS_PROBE_BASELINE` に無い suite は本文付きの `# 空振り検出:` がヘッダに必須）: 名簿外で宣言なしは赤で名指し + 正本（`docs/04-quality/TESTING.md`「針が当たらない入力の既定」）の案内、ヘッダに宣言を足せば通り免除名簿の件数が 1 減る、ヘッダの外の宣言は数えず置き場所を案内、宣言済み suite の名簿残留は赤、名簿の実在しない名前は赤、本文なしの宣言は赤、名簿を空にしても宣言の無い suite は**全件**名指しされる（集合が空で緑へ倒れない。先頭 1 件で打ち切る退行も赤）、名簿が空で全 suite 宣言済みの終端状態は通り 1 本落とすとその suite だけ名指し。犠牲 suite は複製木内で合成するので実体の名簿が空でも成立する |
