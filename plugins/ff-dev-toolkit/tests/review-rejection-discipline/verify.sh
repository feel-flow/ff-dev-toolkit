...
# 禁止列挙だけだと repo 非接触の probe まで自粛され、静的読解で確定できない検証が
# 「未確認」のまま空振りする（Issue #707）。許可行が消えると同じ立ち止まりが再発する。
contains "$MULTI_REVIEW" "probe）は実行して構いません" \
  "委譲プロンプトが repo 非接触 probe の許可を明示（Issue #707）"
