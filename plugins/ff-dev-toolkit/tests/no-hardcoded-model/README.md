...
モデルの明示指定は環境変数で行う（`MULTI_AGENT_MODEL_<CLI>` / `MULTI_AGENT_CODEX_PROFILE`）。Codex の effort だけを単発指定する場合は `MULTI_AGENT_CODEX_REASONING_EFFORT` を使う。詳細は `skills/multi-review/references/model-selection.md` を参照。
