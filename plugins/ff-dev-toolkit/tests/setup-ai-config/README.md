...
├── README.md                 # このファイル
├── verify.sh                 # 5境界パリティ + Cursor 形式の機械検証
└── fixtures/
    ├── input/
...
1. **期待生成物 fixture**（`fixtures/expected/`）— スナップショットの自己一貫性（4ファイル × 5境界）
...
- **Legacy `.cursorrules` の5境界パリティは既定パス外**のため未検証（明示的 opt-in のみ。既定は `.cursor/rules/*.mdc`）。テンプレ本文自体は `docs-template/SETUP_CURSOR.md` で5境界を含む
