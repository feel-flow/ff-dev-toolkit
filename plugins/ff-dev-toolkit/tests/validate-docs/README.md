...
- **出力契約**: 出力例は置かず、`## 出力形式` の 3 行（観点別の結果 / サマリー / 推奨アクション）で書式を規定する。条文ピンは 3 行すべてに完全行で置く（サマリー行は ⚠️ が判定に効かず、判定は ❌ ゼロで達成であることを持つ）。出力例の行へ張っていたピンは、例を削ったときに同時に外した（ピンは規定に従属する）
- **本線のバイト数**: `SKILL.md` は 20,000 B 以下（超えると (A) で FAIL）

同じ条文を `## 重要ルール` に重複して書かない。各規則のピンは正本の節（§1〜§6・`## 出力形式`・`## 重要ルール`）に 1 か所だけ置く。
