...
#
# 変異検出: (A) の条文ピン 30 本は、対象行を 1 行ずつ否定形（末尾に「（ただし適用しない）」）へ変えると各々 ✗ になり rc=1。
#   本線の上限検査は、SKILL.md 末尾へ 3,101 B を足して 20,097 B にすると「本線のバイト数が上限を超えた」で rc=1（2026-09-24 実測）。
# 空振り検出: SKILL.md が無いと「コマンド定義が見つからない」で rc=1。節見出しが変わり節が抽出できないと「節が抽出できない」で rc=1。
#   本線のバイト数が測れない場合も rc=1（0 B や緑へ倒さない）。PATH 先頭に exit 1 する wc を置くと「(wc 失敗)」、
#   数値でない値を返す wc を置くと「"abc" B」で「測れない」になる（2026-09-24 実測）。SKILL.md が読めない（権限 000）回は rc=2 で止まる。
...
GENERATOR_RULES=(
  ...
  "見出しでなく内容の責務で判定|^### 2\. |^### 3\. |- **見出しの文字列一致ではなく、内容の責務で判定する**。同義見出し（上記の対応のほか、番号prefix \`## 9. 状態遷移\` や見出しレベルの違い \`###\` も許容）に該当内容があれば充足とする。ただし見出し名だけで充足と即断せず、**該当内容が実際に書かれているかを確認し、充足と判定した根拠（対応する見出しと内容の要旨）を出力に含める**（例: 「ステークホルダー分析」に対象ユーザーの記述がなければ「対象ユーザー」は未充足）"
  "充足の根拠を出力|^### 2\. |^### 3\. |- **見出しの文字列一致ではなく、内容の責務で判定する**。同義見出し（上記の対応のほか、番号prefix \`## 9. 状態遷移\` や見出しレベルの違い \`###\` も許容）に該当内容があれば充足とする。ただし見出し名だけで充足と即断せず、**該当内容が実際に書かれているかを確認し、充足と判定した根拠（対応する見出しと内容の要旨）を出力に含める**（例: 「ステークホルダー分析」に対象ユーザーの記述がなければ「対象ユーザー」は未充足）"
  ...
  "出力契約:観点別の結果|^## 出力形式|^## 重要ルール|- **観点別の結果**: §1〜§6 の観点ごとの見出しの下に項目を 1 行ずつ「記号 + 対象 + 根拠」で書く。記号は ✅ 充足 / ❌ 不足・違反 / ⚠️ 注意（§4・§5 の別枠報告等）/ ➖ N/A"
  "出力契約:推奨アクション|^## 出力形式|^## 重要ルール|- **推奨アクション**: ❌ と ⚠️ の各項目の改善を番号付きで示す"
  "出力契約:⚠️は判定に効かず判定は❌ゼロ|^## 出力形式|^## 重要ルール|- **サマリー**: 観点ごとの充足数 / 分母（内訳付き）・全体スコア・判定。⚠️ は判定に効かず、判定は ❌ が 1 つも無ければ達成・あれば未達"
  ...
  # --- Issue #518: プレースホルダー免除区分（§4） ---
  # 節スコープは ^### 4\. 〜 ^### 5\. 。免除を散文や別節へ逃がしても FAIL させる。
  "プレースホルダー免除は閉じた区間のみ|^### 4\. |^### 5\. |- **検査対象外（閉じた区間のみ）**: 判定の前に次を検査対象から外す。(1) 閉じたコードフェンス（開始行から対応する閉じ行まで）(2) 閉じた HTML コメント（\`<!--\` から \`-->\` まで）(3) 同一行内で対になった単一バックティックのインラインコードスパン。規則の正本は本節である"
  "プレースホルダー閉じ忘れは除外区間にしない|^### 4\. |^### 5\. |- **閉じ忘れは除外区間にしない**: 閉じていないフェンス / コメントは除外区間にしない。閉じマーカー探索だけを打ち切り、開始行の本文は検査対象に残して後続の走査を続ける。除外範囲を文書末尾まで広げない。表セル・通常本文のプレースホルダーは検出対象のまま"
  "プレースホルダー記入雛形|^### 4\. |^### 5\. |- **記入雛形**: 新規エントリの追加形式を示すブロックは、閉じたコードフェンス内または閉じた HTML コメント内に置く場合に限り残してよい"
  "プレースホルダー例示内スロット|^### 4\. |^### 5\. |- **例示内の変数スロット**: 表セルなどフェンスに入れられない位置の可変部分は具体例へ置換する（残さない）"
  "プレースホルダー未記入骨組み|^### 4\. |^### 5\. |- **未記入のテンプレート骨組み**: 実体を書く。実体が無い項目は「該当なし（理由）」と明記する"
  "プレースホルダー規則説明の引用|^### 4\. |^### 5\. |- **規則説明のための引用**: プレースホルダー自体を説明対象として引用する場合は、同一行内で対になった単一バックティックのインラインコードスパンで囲む"
)
...
if [[ ! -f "$CMD" ]]; then
  ...
  # 本線の上限。畳んだ本線が再び膨らむのを止める。wc が数値を返さない場合も赤にする（fail-closed）。
  if ! skill_bytes="$(wc -c < "$CMD" | tr -d '[:space:]')"; then
    skill_bytes="(wc 失敗)"
  fi
  if [[ "$skill_bytes" =~ ^[0-9]+$ ]] && (( skill_bytes <= SKILL_MAX_BYTES )); then
    echo "  ✓ 本線のバイト数: ${skill_bytes} B（上限 ${SKILL_MAX_BYTES} B）"
  else
    echo "  ✗ 本線のバイト数が上限を超えた、または測れない: \"${skill_bytes}\" B（上限 ${SKILL_MAX_BYTES} B）"
    fail=1
  fi
fi
echo
