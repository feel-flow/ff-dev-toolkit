...
GENERATOR_RULES=(
  ...
  "重要ルール:プレースホルダー免除|^## 重要ルール|^## |- **プレースホルダー免除は閉じた区間のみ**: 閉じたコードフェンス・閉じた HTML コメント・同一行内で対になった単一バックティックのインラインコードスパンの中は検査対象外。閉じ忘れは除外区間にしない（閉じマーカー探索だけを打ち切り、開始行の本文は残して走査を続ける）。表セル・通常本文は検出対象のまま。規則の正本は §4"
)
