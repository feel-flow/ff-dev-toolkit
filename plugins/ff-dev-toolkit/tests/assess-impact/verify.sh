...
GENERATOR_RULES=(
  ...
  "HIGH:ADR採番規則|^### 5\. |^## 重要ルール|- 新規 ADR の番号は目視 grep で決めず、DECISIONS.md の §新規 ADR の採番（同梱テンプレート \`docs-template/06-reference/DECISIONS.md\` が正本）に従う — 採番検証スクリプトの出力、無ければ見出し行限定・数値順の最大値 +1 を根拠にする"
