...
CLAIMS=(
  ...
  # 除外する 2 行は `refine 目安 130` も併記しているため、この claim も同じ 2 行を跨ぐ。
  # 今は warn 既定が 130 のままなので偶然一致しているだけで、warn を変えた瞬間に歴史行が
  # 赤くなり「同じ除外を足す」か「歴史行の表記を変える」の二択になる。後者は ACE-539-3 が
  # 禁じる回避なので、対称に先へ入れておく（値が変わらない今日は挙動不変）。
  "ACE refine 目安|${D_ACE_WARN}|refine 目安 %N%|## ADR-029:;M4.4: 件数ゲートの二段化|"
  # ブロック上限は ADR-041 で 180 → 280 へ更新した。除外する 2 行はどちらも
  # **決定時点の記録**で、現在値の主張ではない: ADR-029 の見出しはその ADR が
  # 何を決めたかを記録しており、ROADMAP の M4.4 行は完了済みマイルストーンの
  # 実績値である。歴史行を現在値ゲートから外すのは表記を変える回避ではなく
  # **この除外列**で行う（ACE-539-3 / `調査時点` と同じ扱い）。ニードルは
