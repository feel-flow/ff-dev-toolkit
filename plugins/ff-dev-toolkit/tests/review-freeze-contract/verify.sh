...
# 別 CLI が無いとき、主担当のみで完了する前に Toolkit エージェントを任意で挟む段
# （Toolkit 段の導入時に追加。基準線が主担当のセルフレビューへ反転し、この段は必須から
# 任意になった）。正本（self-review.md step 4）だけ直して消費側スキルを放置すると、
# 実行経路は段の存在を知らないまま進む。両者を 1 本の針で固定する。
doc_has "$MULTI_REVIEW" "multi-review/SKILL.md" \
  "別 CLI が無く、ホストが Claude Code で Toolkit のレビューエージェントが使えるなら" \
  "multi-review が別 CLI 不在時の Toolkit エージェント段（任意）を指示している"
doc_has "$MULTI_REVIEW" "multi-review/SKILL.md" \
  "同一モデルの追加観点" \
...
doc_has "$MULTI_REVIEW" "multi-review/SKILL.md" \
  "その規定（隔離時の凍結の扱いを含む）は [Git Workflow ステップ5](../../docs-template/05-operations/deployment/git-workflow.md#ステップ5-セルフレビューpr作成前重要) を正本とする（本書は再掲しない）" \
  "multi-review が worktree 隔離既定を再掲せず正本（ステップ5）へリンクしている"
