...
4. `## Changelog` 先頭へ整理ブロックを追記する:
   ...
   **そのどちらにも ID を裸で書けない**（列挙の続きと区別できないため。`（ACE-238-1 のみ …）` の
   ように括弧の中へ入れる）。括弧の閉じ忘れと閉じ括弧の余りは部分採用せず違反になる
   （余った閉じ括弧は前後の ID を `ACE-1-1ACE-2-1` という 1 つの偽 ID へ融合させ、実在する 2 件を
   丸ごと未検証にする）。
   ...
   無い括弧は本検査の対象外（`Compacted:` は既存の unbalanced 検査が拒否する。他ラベルの非対称は
   別 Issue）。操作対象の ID は**括弧の外**に
