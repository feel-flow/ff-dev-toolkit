...
| `/retrospective`（本スキル） | プロセス/ツール/スキルのメタ知見（手戻り・無駄時間・Keep・過剰動作） | 作業中リポジトリの観測台帳への記録 → 閾値到達で承認後に Issue 昇格 |
...
- **read-only**: 振り返り工程ではファイル編集・コミット・Issue 作成を行わない（記録・起票は振り返りを終えてからの別工程）。書き込みが発生するのは、観測台帳への定型記録（作業中リポジトリでの台帳の作成・エントリ追記・Count 更新・旧経路 Issue の取り込み。下記「観測の記録」）と、提案をユーザーが承認して起票する段だけ
...
1. **起票先の確定（SSOT 確認）**: 改善対象がスキル・hook・スクリプトなどツール群そのものである場合、その開発元（SSOT）リポジトリで作業しているなら起票先は SSOT にし、配布用の同期ミラーへは起票しない。**これは開発元へ到達できる場合の規則**であり、SSOT が非公開などで到達できない利用者は、手元の配布元リポジトリの Issue へ起票してよい（受け取ったメンテナが SSOT へ transfer する）。**手順 3 の既存 Issue 検索**は、ここで確定した起票先 repo に対して行う（後の手順で起票先を変えた場合は手順 3 をやり直す — ミラー側だけ検索して「重複なし」と判定する誤りを防ぐ）。一方、手順 2 の知見ストアは**手戻りを実測した導入先 repo**（いま作業しているプロジェクト）が対象で、起票先とは別の repo になりうる
   ...
   **起票先 repo の解決（手順 1 の具体手順）**:
   ...
   - **公開配布物だけの利用者**: 開発元が非公開・非開示で SSOT 関係を確認できない場合も、存在確認済みの配布元へ起票する fallback は維持する。配布元を SSOT と断定せず、`既存確認:` に「SSOT は確認不能・配布元へ受け渡し」と明記する。起票のユーザー承認は必要。
...
4. 承認された（または手順 2 の包括指示例外に該当する）提案のうち**既存への追記へ切り替えた**もの: 新規起票はしない。既存 Issue へのコメント追記は `gh issue comment "<Issue番号>" --repo "<owner>/<repo>" --body "<再発の実測・経緯 + 元提案の内容>"` で行う。既存 ACE エントリのカウンター更新は本スキルでは行わず、ACE 側の手順（`/ace-curate`、または導入 repo の Playbook 記述ガイドライン）へ回す
