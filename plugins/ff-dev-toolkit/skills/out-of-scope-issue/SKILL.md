...
>
> なお本スキルは §3.2 の `gh issue create` と §3.3 の実在確認が別のブロックに分かれている。スキルの bash ブロックは呼び出しごとに別のシェルで走りうるため、**両者は同じシェルで続けて実行すること**（`label_args` を失っても `${label_args[@]+...}` は空へ展開して exit 0 になり、ラベル 0 個の起票が成功したように見える）。
