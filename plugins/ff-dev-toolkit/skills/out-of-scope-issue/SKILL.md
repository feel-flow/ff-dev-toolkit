...
**起票は PR 作成より前に行う。** GitHub は Issue と PR で採番列を共有するため、PR 作成前に「次に発行されるはずの番号」を推測して PR 本文へ書くと、その番号を PR 自身が取ってしまう（実測: 「`#181` として切り出した」と書いてから `gh pr create` した PR が `#181` を取得し、直後に起票した follow-up Issue は `#182` になった — PR 本文が自分自身を「別 Issue」として指し、本文の取得 → 置換 → `gh pr edit` の 1 往復を要した）。番号が確定するのは `gh issue create` / `gh pr create` を実行した後だけで、事前に推測する余地はない。やむを得ず起票を PR 作成の後に回す場合は、PR 本文へ番号を書かず `<!-- follow-up issue: TBD -->` などのプレースホルダを置き、起票直後に `gh pr edit --body-file` で埋める（番号を書かないまま放置する＝宣言だけ残る失敗とも両立する対処）。
...
> **重複について**: このラベル照合〜起票の手順は `create-issue` 手順 6 と意図的に同じ内容を持つ（違いは候補の系統 — 本スキルは `follow-up` を足した 3 系統、`create-issue` は type / priority の 2 系統 — と `--assignee` の既定だけ）。参照ファイルへ切り出さないのは、照合と報告の判定規則が起票コマンドの直前で読まれないと、実在確認ごと飛ばされる経路ができるから。ドリフトは `tests/issue-label-contract/verify.sh` が両ファイルを突き合わせて検出する。**fixture に載せた契約行を片方だけ書き換える／削除すると red になる**（fixture 未収載の複製文までは検出しない）ので、変更は必ず両方へ入れること。
