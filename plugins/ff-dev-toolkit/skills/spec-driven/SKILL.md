...
4. ゲート進行表を作成する（保存先: ユーザー指定。指定がなければカレント作業ディレクトリ）。進行表は中間成果物のためコミットしない。Git リポジトリ内に保存する場合、`spec-driven-gates-*.md` が ignore されていなければ `.gitignore` へのパターン追加を提案する（リポジトリ側にコミットする方針が明示されている場合を除く）
