...
G1 仕様ゲート（SKILL.md Step 3）で使う。**「この変更はどの文書に書くべきか」を迷わず決める**ための地図。文書が未整備のプロジェクトでは `${FF_DEV_TOOLKIT_ROOT}/docs-template/` から最小構成で始める。
...
- テンプレートから新設した文書は、frontmatter のプレースホルダー（owner / created 等）を実際の値で埋める。`changeImpact` キーが無いテンプレート由来の文書には追記し、初期値が大文字なら小文字へ正規化する
...
- G1 で「対象文書が存在しない」場合、**その場で最小の文書を `${FF_DEV_TOOLKIT_ROOT}/docs-template/` の対応テンプレートから起こす**（数行でよい。壮大な整備プロジェクトにしない）
