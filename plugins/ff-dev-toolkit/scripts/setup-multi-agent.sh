...
# 中断コードは review 系 3 本（multi-agent.sh / multi-review.sh / 本スクリプト）で共通の 2。
# docs-template の multi-CLI review orchestration 節と、契約検査の除外理由が、この族の
# plugin-root 停止を status 2 と名指ししている。本スクリプトは 2 に別の意味を割り当てて
# いないので、族の契約側へ寄せる（1 は一般の致命的エラーで、族の停止と区別できない）。
