...
# provenance の 1 行は、ガード block 内の basename allowlist で抑止している。本スクリプトは
# 「exit 0 は無出力」を契約にしており（一致は何も言わないことで表す）、起動時の情報行はその
# 契約を破る。停止の判定と案内は抑止されない。
...
# 中断コードが契約既定の 2 ではなく 3 なのは、本スクリプトの 2 が「判定不能（マージは
# 止めない）」だから。ガードの停止は検査不成立なので、停止を意味する 3 を使う。
