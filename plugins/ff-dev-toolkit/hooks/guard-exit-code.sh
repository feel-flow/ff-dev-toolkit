...
#
# 止めるのは検出器が出す 4 タグすべてである。いずれも OBS-003 の同じクラス
...
#   gate-exit-dropped    ゲート起動の**後続の行**が rc を伝播させずに終端している
#                        （`<ゲート> > log 2>&1`⏎`rc=$?`⏎`echo "EXIT=$rc"` — 推奨形から
#                        `exit $rc` の 1 語を落とした形。区切り子が改行なので上のタグには
#                        載らなかった。Issue `#1748`、実測 2026-09-18: failed=1 の赤を
#                        「exit code 0」と通知した）
...
#     直し、当たるようになった
#   - **末尾 `&` の background 起動の後で `wait` を読まない形**（`<ゲート> > log 2>&1 &`⏎
#     `echo started`）: rc は `$?` ではなく `wait` が運ぶので、検出器は末尾 `&` の行から
#     持ち越しを追わない（`wait $pid; rc=$?` の正当な形を止めないため）
#   - （解消済み）改行で区切った 2 行目以降: `<ゲート> > log 2>&1`⏎`echo done` と
#     `…`⏎`rc=$?`⏎`echo "EXIT=$rc"` は Issue `#1748` で検出器が論理行をまたいで rc の
#     持ち越しを追うようになり、`gate-exit-dropped` で当たる。`pipe-exit-read` は
#     以前から行をまたぐ（`prev_pipe` を空行・コメント行を挟んでも保持する）
...
#      `gate-exit-dropped` は逆にちょうど 0 個 — 検出行はゲート起動行そのもの（`<ゲート> > log 2>&1`）
#      で、直す場所は「その後ろの行」なので起動行を丸ごと案内できるが、区切りが在ればそれは
#      前置コマンドで、切り出すとゲートが案内から消える
...
launch_ok=1
case "$first_tag" in
  # dropped の検出行はゲート起動行そのもの。区切りが在ればそれは前置コマンド（`cd x;` /
  # `true ||`）で、`%%;*` の切り出しはゲートを落として前置だけを案内する（grok-cli の指摘）
  gate-exit-dropped) [ "$sep_n" -eq 0 ] || launch_ok=0 ;;
...
case "$first_tag" in
  gate-exit-swallowed)
    ...
    ;;
  gate-exit-dropped)
    what="ゲートを起動した後の行が、その終了コードを伝播させずに終端しています。\`rc=\$?\` で受けても \`exit \$rc\` まで書かなければ、**このコマンドのプロセス終了コードは末尾の行のもの**になります。バックグラウンド実行の完了通知や CI ステップは、その 0 を「exit code 0」として**断定**で報告します（実測 2026-09-18: 実際は failed=1 の赤を「exit code 0」と通知した）。"
    ;;
  pipe-exit-read)
