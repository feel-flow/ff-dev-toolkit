...
- Jev の評価テストはロック済みのローカル TypeScript runner を使い、各ケースでの実行環境探索を省略する。テストの実行前に同梱 MCP の依存を npm ci で導入する。
- 検査の自己テストは独立した fixture を維持しつつ準備処理と反復実行を軽量化し、変異の検出と正常系の確認を保ったまま待ち時間を短縮する。
- 日次リリースの footer 追従では、同じ実行内の初回成功と変更内容を証明できる場合だけ公開配置の検査結果を再利用する。週次 CI の確認と通常の検査は再実行する。
- 全件ゲートの既定の同時実行数を最大 5 にし、重い検査を先に投入する。モード切替の自己テストは逐次で回し、並列動作は専用ケースで検証する。

## [0.133.1] - 2026-09-26

### 修正

- docs-template の MASTER が「必要時にコピー」と案内する文書（初心者・新規プロジェクト向けガイド、企画書テンプレート、Quickstart、Copilot Agents / Issue テンプレ設計パターンの参考資料、組織展開ガイド一式、品質・セキュリティの 2 文書）を消費プロジェクトの docs/ へコピーしたとき、コピーされない文書へのリンク・展開前パス（docs-template 配下）・古いディレクトリ名（01-business / 07-quality）が残って解決しなかった問題を修正。コピーされない文書はコピー元パス付きの案内テキストで示し、コピー先の docs/ を基準にした手順へ揃えた。
- `MASTER.md` の「必要時にコピー」の案内を、一緒にコピーしないとリンクが切れる文書の組（コピー単位）で示すよう整理した。組織展開ガイドは索引の `05-operations/ORGANIZATIONAL_ROLLOUT.md` と詳細 4 文書のディレクトリを 1 つの指示でコピーさせ、本文で索引や詳細文書を単体でコピーさせていた箇所も一式を指す形へ揃えた。品質・セキュリティ節の 2 文書も、相互に参照するため一緒にコピーさせる。
- 同梱テスト docs-template-portability が、MASTER の「必要時にコピー」の指示からコピー単位を導き、各単位の文書を「初期セット + 同じ単位」の中で解決できるかを検査するようにした。対象は手書きのリストを持たず MASTER の指示から導くので、箇条へ文書を足すと検査も追随する。「必要時にコピー」節の箇条から指示を読み取れない場合、本文の指示が複数ファイルの単位の一部だけをコピーさせている場合、節が見つからない場合は検査不成立として失敗する。展開前のディレクトリは 「find docs-template docs」のようなコマンド引数の形も検出し、案内テキストに書いたコピー元パスの実在も確かめる。
...
- multi-agent.sh が codex-cli を起動する前に、PATH 上の codex の版と `~/.codex/models_cache.json` の `client_version`（アカウント側のモデル一覧を書いたクライアントの版）を比べ、古ければインストール元の手段（npm なら `npm install -g @openai/codex@latest`、Homebrew なら `brew upgrade codex`）で自動更新してからレビューを回すようにした。実測では codex-cli 0.153.4 が ChatGPT アカウントで gpt-6 系を 400 で拒み、0.157.0 では完走した。
- `--dry-run` では更新せず「更新が必要」を表示するだけにする。インストール元を判定できない回は更新を試みず手動の手順を案内し、更新が失敗した回は古い版で走ることを明示して続行する。版を判定できない回（キャッシュが無い・読めない、`codex --version` が版を返さない）は判定しなかったことを 1 行で示す。
- 新しい環境変数 `FF_DEV_TOOLKIT_SKIP_CODEX_AUTO_UPDATE=1` で自動更新を止められる。

### 変更

- マージ直前の鮮度照合が、直近の全件ゲートの赤がすべて base の先行による鮮度赤（run-all が stale と分類した suite）だけで説明できる回を区別するようにした。これまでは赤い記録を一律に「ゲートを通してから記録を更新すること」と案内していたため、並行マージが続く間は全件ゲートを回し直すたびに同じ鮮度赤を踏み続けていた。該当する回は「base を取り込み、赤だった suite だけを verify.sh のパスで名指しして回し直す」を案内し、緑なら部分実行の記録と PR の checks でゲート全体を回し直さずに進める。
- run-all は、赤の原因がすべて鮮度分類の suite で、起動できなかった suite と必須 skip が無い回に限り、その suite 名をゲート記録（`STALE_ONLY_FAILED`）へ残す。記録器は緑の記録にこの値を受けない。照合器は suite 名の文字集合だけを受け、行の無い旧記録や読めない値は従来の案内へ倒す。鮮度の分類は suite 単位の「含む」判定なので、この値は案内の材料に留め、一致の根拠にはしない（照合は判定不能のまま）。案内を出すのは、記録がリモート先端そのものを clean な木で測った回に限る。記録器も、汚れた木・走行中に HEAD が動いた回の赤い記録では分類を空で書く（別のツリーの赤を根拠に部分実行の経路へ誘導しない）。

### 修正

- multi-agent の `--dry-run` で、grok の sandbox が働いていない環境（Windows 等）を検出できず、grok-cli が警告なしでプランに載ったまま毎回 INCOMPLETE になる問題を修正。sandbox probe は従来、CLI が stderr に出す拒否の目印だけで判定していたため、sandbox の指定をエラーも警告も出さずに受け流す形（rc=0・stderr 空）を拾えなかった。報告: https://github.com/feel-flow/ff-dev-toolkit/issues/111
- probe は rc=0 で拒否の目印が無いとき、起動の前後で sandbox のイベントログ（`GROK_HOME/sessions/sandbox-events.jsonl`（既定は `~/.grok`） と旧置き場 `GROK_HOME/sandbox-events.jsonl`）の ProfileApplied 行数を数え、1 行も増えなければ「不活性」として警告する。過去の記録がログに残っていても、今回の起動が書いたかどうかで判定する。grok-cli はプランに残し（警告を出してもプランから外さない従来の方針のまま）、恒久的に外す設定行 `exclude_clis: "grok-cli"` を案内する。
- イベントログを数えられない場合（置き場が通常ファイルでない・読めない・probe の間に行数が減った・`GROK_HOME` も `HOME` も無い・probe の一時ファイルを作れない）は、「問題なし」として黙らず「sandbox が効くかを起動前に判定できませんでした」と理由付きで表示する。
- sandbox が適用される環境では、probe の起動で ProfileApplied が 1 行増えるため従来どおり何も表示しない（grok 1.0.41 / macOS で実測。Linux は同じ記録を書く前提で未実測）。Windows 実機での確認は報告者へ依頼している。
- multi-agent の失敗理由の記録が拒否された回に、INCOMPLETE 成果物が古い段階の分類（実行開始時の初期値）で原因を断定したまま注記を出さなかった問題を修正。記録が拒否された回だけ、成果物のバナー直下に「失敗理由を更新できなかったため、この分類は古い可能性がある」旨と拒否理由・読み戻した値を 1 行で書く。拒否を知らせる警告はオーケストレーターの標準エラー（複数アダプタの出力が交錯し、保存されない）にしか出ていなかったため、成果物の読み手には誤った原因だけが届いていた。
- 注記の条件は 2 経路で判定する。同じシェルでの記録失敗はフラグで成果物生成まで運び、時間切れの判定のようにサブシェル内で拒否された記録は、成果物を書く直前に理由ファイルの実体を同じ条件で再判定して拾う。記録が正常に行われた回は理由ファイルが自分所有・リンク数 1 のままなので、注記は付かない。再判定だけで分かった回は、記録が実際に試みられたかまでは見えないため「更新されていない可能性がある」と弱めた文面にする。
- 注記に写す読み戻し値は、理由コードの許可値に一致するときだけ表示する。注記が出る回の理由ファイルはハードリンクや他人所有など自分が書いたとは限らない実体であり、中身をそのまま写すと無関係なファイルの内容が成果物へ漏れ、改行を含めば統合レポートが読む理由コード行を偽装できるため。
- リンク数を取得できない環境（`stat` が PATH に無い・壊れている）での拒否理由の文面を、「ファイルのリンク数が不明個ある」から「リンク数を判定できなかった（`stat` は使えるか）」へ改めた。原因がファイルではなく道具の側にあることと、次に確かめるものを示す。実際にハードリンクが複数ある場合は従来どおりリンク数を実数で名指しする。
- multi-agent-timeout の suite に、注記が出る 4 経路（`stat` 不在・サブシェル内の拒否・理由ファイルを消せない回・既存のハードリンク）、ハードリンク先の内容が成果物へ写らないこと、正常な回と理由ファイルが無い回に注記が出ないことの検査を加えた。
- multi-review の 2 本目の CLI が「利用不可」（認証切れ・クレジットや利用枠の上限・モデル非対応）で落ちたとき、統合レポートが Critical ありへ倒して主担当のみで完了できなかった問題を修正。pair モード（`--route full` など）で主担当が全観点を完走した回は、実行全体を 0 終了にし、その観点を INCOMPLETE / Critical ではなく「主担当のみで完了（codex-cli: モデル非対応）」の形で 1 行記録する。主担当そのものが利用不可・途中失敗の回と、単一 CLI の起動は従来どおり非 0 のまま。
- 失敗分類に「モデル非対応」（例: `The 'gpt-6-sol' model is not supported when using Codex with a ChatGPT account.`）を追加し、🚫 の案内で原因の候補（CLI の版・アカウント種別）と対処（CLI の更新・`MULTI_AGENT_MODEL_CODEX_CLI` などによるモデルの一時上書き）を名指しする。認証・残高と同じく同一 CLI の残りタスクはスキップする。クレジット上限の語彙に `spending limit` / `all available credits` を加えた。分類できた失敗には汎用の「時間では直らない失敗」の文を重ねない。
- INCOMPLETE 成果物の CLI stderr 抜粋を、抜粋中の最長のバッククォート列より長いフェンスで囲むようにした。codex は stderr へプロンプトをエコーし、プロンプトに含まれる ``` がフェンスを閉じていたため、型付き判定が「フェンス未閉」として安全側 Critical へ倒れ、失敗分類は末尾の API エラー行に届いていなかった。
- stderr 抜粋がバイト数で UTF-8 の多バイト文字の途中から始まる回に、先頭の切れ端の行を捨てるようにした。不正な UTF-8 が残ると下流の awk が失敗し、判定器の実行失敗として安全側 Critical へ倒れていた。
- 失敗分類は、エコーされたプロンプトの終端行より後ろの stderr だけを読むようにした。レビュー対象の diff に含まれる語（`unauthorized` や `credits` など）を CLI 自身の失敗理由と誤読しない。
...
- `/multi-review` のレーン数が変更クラスで決まるようになった。新しい `scripts/review-route.sh` が決定木の根（v0.129.0 で追加した決定木ルータ）の答えを読み、`fast`（docs のみ、または 10 行以下）は親の直読 + Codex 1 レーンで Claude の 4 観点を起動せず、`full` は従来の 5 レーン、`none` はホストが判定する。完了報告用の 1 行（例: `経路: fast（根拠: docs のみ / 12 行）`）を `REVIEW_REPORT_LINE=` で出す。判定は根に任せ、根の `DT_*` 行はそのまま通す。`scripts/multi-review.sh` は `--route fast|full|none` を受け、`fast` では Codex 1 レーンだけ、`full` では 5 レーン（claude-code の 4 観点 + codex-cli の comprehensive-review）の実計画を、既定の pair や利用者設定のレビュアーに依らず組む（レーン構成を決める `--mode` / `--cli` / `--perspective` 等との併用は exit 2）
- レビュー→修正ループの上限を 3 回転から 2 巡へ変え、機械的に止めるようにした。同じブランチで異なる HEAD に対するレビュー起動（レビュー用サブエージェント・隔離起動を含む、`multi-review.sh` / `codex-review.sh` / `multi-agent.sh`〈`--task` が review 以外の起動は除く〉）を巡として数え、3 巡目の起動を `hooks/guard-review-in-flight.sh` が deny し、hook が発火しないホストでも `scripts/multi-agent.sh` の review 本体（`multi-review.sh` 経由も直接起動も）が exit 4 で止めて、残件を `/out-of-scope-issue` の bundle 統合へ流す案内を出す。記録は git common dir 配下（`ff-review-rounds/`）に置くので、worktree 間で共有され、`--fresh` の退避やレーン置き場の削除では消えない。PR 作成前のレビューも巡を消費する。dirty 検査の確認（ask）で起動した回は仮に記録し、起動が拒否されたら取り消す。1 巡だけ通すには Bash は起動コマンドの先頭へ `FF_REVIEW_ROUND_ACK=1`、Agent は prompt の行頭にその 1 行を置く（通した巡も記録され、次の巡でまた止まる）。上限は `FF_REVIEW_ROUND_LIMIT`（既定 2、0 で無効）。記録が無い・読めない・detached HEAD・統合ブランチ（`develop` / `main` / リモートの既定ブランチ）上のときは 0 巡と見なさず、警告を出して通す。2 巡目の fix の確認は親の直読と対象 suite の再実行で行い、レビュアーを再起動しない
- 変異ハーネス `scripts/mutation-harness.sh` を追加した。タブ区切りの変異表を 1 件ずつ作業ツリーの写しへ当て、適用の前後で内容ハッシュが変わったことを確かめてから対象 suite を回し、Markdown の結果表（適用の成否・期待・rc・判定・suite の要約行）を出す。当たらなかった変異は `NOT-APPLIED`、赤にならなかった変異は `SURVIVED` として非 0 で終わり、変異表が空なら緑にしない。写しの中の symlink を辿る変異は当てず、作業領域は `--work-dir` 配下に作った一意な子ディレクトリだけを消し、対照（変異なし）も専用の写しで回す。plugin root の handoff（`FF_DEV_TOOLKIT_ROOT` 等）は写し側へ張り替えて suite へ渡し、`delete-line` は他の行のバイトを変えず、`create` は内容をそのまま書く。Git Workflow ステップ5 が、契約針の差し替え・文言更新が中心の PR でこの表を `gh pr create` の前に回し、本文ファイルへ組み込んでから PR を作る手順を持つ
- knowledge コミットの共通の書き込み口 `scripts/knowledge-commit.sh` を追加した。`/ace-curate` の Playbook 追記と `/retrospective` の観測台帳追記は、書いたファイルを `add`（ID・要約・カテゴリ・パスを保留記録へ足す）してから `commit` で 1 つのコミットにする。同じセッションで両方が書いた回は `knowledge: ACE-… / OBS-…` の 1 コミットに畳み、要約とカテゴリは本文へ置く。片方だけの回は従来と同じ件名（`knowledge:` の後に ID と要約、カテゴリは本文の `Categories:` 行）の単独コミットになる。commit は記録したパスへ固定し（索引に載った無関係な変更を巻き込まない）、push はしない。`--type` で件名の prefix だけを差し替えられる（commitlint が `knowledge` を許さない導入先向け）。保留記録は最初の `add` のときのブランチ・基点の HEAD・時刻を持ち、`commit` はブランチが違う・基点が今の HEAD の祖先でない・12 時間を超えた保留を拒否する（中断した別セッションの古い追記を今回の追記と畳まない）。`add` も既存の保留が古ければ足さずに止まる。`status` で確かめ、今回のものなら `commit --force`、古いものなら `discard` で捨てる。`add` は stage した時点の内容の hash を記録し、`commit` は作業ツリーの内容が一致するときだけ commit する（`add` の後に同じファイルへ入った別の編集を吸い込まない）。
- 書き込み口は commit の直前に、実効の git identity が検査用の合成 identity（`user.name=fixture` / `user.email` が `@example.invalid` で終わる）でないことを確かめ、合成 identity なら commit せず復旧手順を出して止まる（保留記録は残す）。PreToolUse のガードが届かない script の内側の commit や、Claude Code 以外のホストの経路でも同じ条件で止まる。意図的に通すときは `FF_DEV_TOOLKIT_SKIP_COMMIT_IDENTITY_GUARD=1` を付ける。
- `/ace-curate` 手順 5 と `/retrospective` の「書き込み（定型コミット）」節を書き込み口の呼び出しへ置き換えた。`/ace-curate` は既定ではその場で commit と push を行い、チェーンの起動元（振り返りの事前注入 hook の文面）が `ACE_DEFER_TO_RETRO=1` を指示した回だけ `add` までで止めて、`/retrospective` が観測台帳と合わせて 1 回で commit と push を行う（commit type は `add --type` で保留へ記録され、合流先が引き継ぐ）。`/retrospective` はモードを判定した直後に保留を確かめ、振り返りを実施しない回でも回収する。

### 変更

- `scripts/finish.sh` を新設し、PR の尾の固定手順を 3 サブコマンドへ機械化した: `precheck`（PR 番号を受け、closing keyword 抵触検査・工数の実測・checks の有無分岐・ゲート実測鮮度の照合・merge コマンド生成を行う。base / head ブランチを別 worktree が保持していれば `--delete-branch` を付けず、merge の成功時だけ走る lease 付きのリモートブランチ削除を `&&` で繋ぐ。対象 Issue が無ければ `PRECHECK=no-target` で以降を行わず、checks の待機後に PR を読み直して判定に使った全フィールドが変わっていれば止めて最初からやり直させ、checks の取得が通信・認証で失敗した回は check の失敗と区別して `gh` 不通を名指しする）/ `cleanup`（PR の実在と `gh` の到達を確かめてから `merge-cleanup.sh` へ委譲する。未コミット変更ガードの除外指定は従来どおり環境変数で届く）/ `knowledge-commit`（共通の書き込み口 `knowledge-commit.sh` を呼び、claim の再生成・保護判定・直 push・PR 経由を同じ入口で行う。直 push と PR 経由のどちらも、送る範囲が knowledge コミット 1 つだけであることを push より前に確かめ、`--claim` の検証で stage するのは文書・claim・`--` 以降の明示パスだけにする）。前提崩れ（PR 不在・`gh` 不通・実測の記録不在）は黙って 0 で終わらず、判定不能を名指しした非 0 と復帰手段を出す
- `/close-issue` / `/merge-cleanup` / `/ace-curate` / `/retrospective` の本線を判断点と `finish.sh` の呼び出しだけに畳み、各 20,000 バイト以下にした。削った規則（AC 判定・マージ直前のゲート・工数の書き戻し・curate の詳細規則・観測台帳の記録規則）は各スキルの `references/` へ移し、散文に張っていた契約針は `finish.sh` の挙動針へ移した（`tests/finish` を新設）
- 振り返りの事前注入 hook と `git-workflow.md` のマージ・クリーンアップ・マージ後の節が `finish.sh` を指すようにした
- `knowledge-commit.sh add` の `--id` を繰り返し指定で累積するようにした（従来は最後の値だけが残り、複数エントリを 1 コミットへ畳むと件名の ID 部から落ちた）
- `/out-of-scope-issue` の本線（SKILL.md）を 20,134 B から 17,237 B へ畳んだ。判定の上位原則（`workflow-principles.md` 原則2）・fix ループの収束判定（`multi-cli-review-orchestration.md`）・bundle の定義（`git-workflow.md`）と同じ文面は正本へのリンクに置き換え、gh の既定 30 件の打ち切り・fail-closed の検索・切った本文で判定しないこと・10 行閾値の数え方など、推論で導出できない事実だけを本線に残した。見出し・節番号・frontmatter の `description` は変えていない
- bundle を新設してその sub-issue として起票する手順（`/create-issue --bundle`、カテゴリ Epic があれば `--parent`）を、本線の類似度表から `references/filing.md` の「Epic への紐付け」節へ移した。類似度表には振り分けの結論だけを残した
- `out-of-scope-routing` suite に本線のバイト上限（20,000 B）の検査を足した。束ねの分割条件は 1 行の列挙に畳み、同じ行に張っていた針 3 本を 1 本にした
- `setup-ai-config` の SKILL.md を 23,797 B から約 17 KB へ畳んだ。3 つの生成テンプレート（CLAUDE.md / copilot-instructions.md / AGENTS.md）が同じ文面で持っていた「実測の記録先」「Out-of-Scope Finding Routing」「Secrets Exposure Prevention」の 3 節を新設の手順 3「3 ファイル共通ブロック」へ 1 本にまとめ、各テンプレートにはその本文で置き換える差し込み行を 1 行だけ置く。導入先へ書き出される 3 ファイルの文面は変わらない（組み立て前後でバイト同一）。5 境界の再掲・手順 2 の選択肢の重複・`setup-multi-agent.sh` の処理内容の説明は削り、手順番号は 1 つずつ繰り下がった（Multi-CLI のセットアップは手順 7）
- `tests/setup-ai-config/verify.sh` は、SKILL.md の文字列ではなく、差し込み行を共通ブロックで置き換えた組み立て後の生成器出力に対して 5 境界と実測の記録先のアンカーを照合するようになった。差し込み行が 1 行でない、または共通ブロックのフェンスが抽出できない場合は赤になる
- `spec-driven` の本線を約 22.9 KB から約 15 KB へ縮めた。G4 の共有版の再確定手順（最新 default branch からの version・Changelog・claim の再生成と、push 済み commit を上書きしない収束規則）は新設の `references/version-convergence.md` へそのまま移し、本線には読む条件つきの 1 行だけを残した。bundle 判定・Playbook 参照・tier の暫定判定と確定判定・G0 の通過条件・ゲート運用の原則は、正本の節へのリンクに置き換えた
- `spec-driven` の `references/gate-criteria.md` に残っていた「G0 で軽量モードを宣言する」記述を、tier 判定で決まる軽量モードの説明へ直した。references 2 本のプラグインルート表記を本線と同じ `FF_DEV_TOOLKIT_ROOT` に揃え、テンプレート由来文書への `changeImpact` 追記の規定を `references/spec-docs-map.md` へ移した
- `/multi-review` の本線（SKILL.md）を 71,224 B から 19,729 B へ畳んだ。`multi-review.sh --help` と配布ドキュメントの正本節（Perspective の絞り込みと単一 CLI 縮退・クロスレビューの単一固定・結果の退避規則・重要度別対応ルール・部分再検証・fix ループの収束判定・ホスト委譲・grok の docker.sock 回避）と同じ文面は正本へのリンクに置き換え、新ブランチ初回の `--fresh`、既定 900 秒と `--resume --timeout 1800`、Monitor を armed してから止まること、凍結の終端条件と worktree 隔離の適用範囲、待ち時間に通る書き込み先 3 つ、分析用サブエージェントへの委譲プロンプトなど、推論で導出できない事実だけを本線に残した。frontmatter の `description` と手順の見出しは変えていない
- 「モデル選択」節を `skills/multi-review/references/model-selection.md` へ、「ツリー変化判定のパス除外」節を `skills/multi-review/references/ignore-paths.md` へ移した。本線には、モデルを明示指定するとき・常駐ツールの書き込みでリビジョンガードが毎回発火するときにそれぞれを読む 1 行だけを残した
- `review-rejection-discipline` suite に multi-review 本線のバイト上限（20,000 B）と、移した 2 つの reference を読む条件の検査を足した。本線から外した文に張っていた針（凍結の非適用ケースの列挙・上限到達前の原因の問い・追加上限・worktree 実走の説明文など 7 本）は、hook の振る舞い検査・正本側の針・ADR との相互照合が担保するので外した
- `create-issue` の本線 `SKILL.md` を約 52 KB から約 18 KB へ畳んだ。工数推定の目安表・過去実績の 3 層参照・補正手順は `references/estimation.md` へ、ラベルの照合規則・priority の判定基準・報告の書き分け・ISSUE_TEMPLATE の pre-flight・起票 3 ステップは `references/filing.md` へ移し（照合規則と起票コマンドは同じファイル内で隣接させたまま）、本線には手順 1〜7 の骨格と、実行時に読まれる事実（非対話モードの非対称・手順 4 の項目名・手順 5 の系統表・工数ブロックの契約・完了報告の必須項目）だけを残した。記入例・手順の再掲・理由の段落は削り、手順 4 の項目を番号でなく項目名で指す形に直した。
- `issue-label-contract` は起票側の検査と共有 fixture の照合を `create-issue/references/filing.md` へ当て直し、本線と `references/estimation.md` に `gh issue create` / `gh label list` の実行行が無いことを新たに検査する。
- `refine-issue` の本線 `SKILL.md` を約 24 KB から約 19 KB へ畳んだ。手順本文の再掲だった「重要ルール」「使用する gh CLI コマンド一覧」「関連」の節を削り、そこにしか書かれていなかった `--repo` の必須化は手順 1 へ、Bot 識別プレフィックスは手順 7 冒頭へ 1 文ずつ移した。手順 3 の参照文書表は docs-template の `git-workflow.md`「タスク種別×参照文書」へのリンクに置き換え、手順 4・7 の記入例、非継承の理由散文、手順 8 の再掲、範囲外の列挙を短くした。6 観点のチェックリスト、`/create-issue` 手順 4 との対応表、skip 条件（6 観点違反 0 件かつ SubAgent 論点 0 件）、`needs-spec` の冪等作成、SubAgent テンプレートの返却フィールド、`[bot]` の mention スキップは変えていない。
- `refine-issue` の Trivial 手順で、注記コメントは body 用の一時ファイルと別に `mktemp` で作った一時ファイルから投稿すると明記した（body 用ファイルを渡して本文全体をコメントする事故を防ぐ）。`refine-issue-skip-contract` は本線 `SKILL.md` が 20,000 B を超えると赤になり、バイト数が測れない場合も赤にする。
- `ace-refine` の SKILL.md を 64,743 B から約 19 KB へ畳んだ。承認された操作の種類ごとにしか読まない R3-a〜R3-d（アーカイブ・圧縮・統合・昇格）の手順を `references/operations.md` へ、引数が `domain` のときだけ読む反映経路を `references/domain.md` へ、R3-e のゲートが exit 0 になった後に読む claim の最終生成とコミット（PR / 直 push・再試行）を `references/commit.md` へ移した。本線には毎回通る骨格（R1 → R2 → 承認前の base 照合 → R3 開始 → R3-0 → R3-e のゲート）と、推論では導出できない実測事実だけを残す。R3-x のラベルは移設先でも変えない
- 正本と同じ内容の写しを削った: Changelog の ID 列パーサ規則（`scripts/ace/README.md` の「check-refine-invariants.ts の実行」と検査スクリプトが正本）、domain の分類・方針（`ace-domain.md` の「refine と反映」）、Frontmatter の更新規則（`ace-cycle.md` の「3. Frontmatter の更新」）、圧縮ルール（PLAYBOOK.md のエントリテンプレート）、レポート出力の列挙。R3 開始前ガードは承認前の照合フェンスと同じ内容だったので、フェンスを 1 本にして「R3 開始の直前にもう一度実行する」へ置き換えた
- R3-e の Changelog 手順に、統合が 0 組なら `- Merged:` の行ごと書かない（`- Merged: なし` は解析できずゲートが exit 1 になる）を足した
- 契約検査 `tests/ace-refine` は本線のバイト数（上限 20,000 B）を検査するようになり、操作別手順の針は `references/operations.md` の節へ付け替えた。`tests/shared-version-convergence` の ace-refine 側の claim 契約の針は `references/commit.md` へ付け替え、base 先行ガードのフェンス数の期待値を 3 へ下げた。`tests/plugin-root-contract` の handoff 検査は `skills/*/references/*.md` の実行行も対象にする
...
- `/retrospective` の SKILL.md を、毎回通る本線（モード判定・自動発火の実行契約・観察チェックリスト・観測台帳への記録・提案の閾値・出力形式）と、条件に当たったときだけ読む `references/` 5 本（`filing.md` 起票フロー / `promotion.md` 昇格・見送り・再発 / `effort.md` 見積もり乖離と集計 / `legacy-intake.md` 旧経路の取り込み / `auto-trigger.md` hook の判定規則）へ分けた。SKILL.md は 74,550 バイトから 37,256 バイトへ半減し、チェーン末尾で毎回読み込まれる量が減る。規定の意味は変えておらず、SKILL.md 冒頭の振り分け表が各ファイルを読む条件を名指しする。外部から参照される節名（「承認と起票」「起票前の既存確認（必須）」「昇格閾値と特急レーン」「集計レポートの実行」）はそのまま残した。
- 回帰スイートを整理した。`retrospective-contract` は針が対象ファイル（節照合なら節内）で**ちょうど 1 行**に出現することを要求するようになり、写しが残って規定の本体を消しても緑のまま通る形を 1 回の実行で検出する。これで検出情報を持たなかった針ごとの行削除変異の `retrospective-contract-selftest` を撤去した（全件モードで実測約 11 分かかっていた）。共通ライブラリ `tests/lib/section-scope.sh` のフェンス状態機械を直接叩く検査は新しい高速スイート `section-scope-lib` へ移し、既定モードで毎回走る。経緯の叙述や別スイートが振る舞いで覆っている文言への重複した針も外した。`skill-bash-blocks` は `skills/*/references/*.md` の bash ブロックも走査するようになった（切り出した手順が検査から外れないようにするため）。
- `/out-of-scope-issue` の SKILL.md を、発見のたびに通る本線（発火の境界・YAGNI / インライン / Issue 化の判定・インライン修正・類似 Issue の検索と束ね）と、条件に当たったときだけ読む `references/` 3 本（`filing.md` 新規起票のラベル決定から戻り値まで / `consolidation.md` 既存 Issue への統合 / `examples.md` 判定例）へ分けた。SKILL.md は 40,332 バイトから 20,134 バイトへ半減し、YAGNI やインライン修正で終わる発見では起票手順を読み込まなくなる。規定の意味は変えておらず、SKILL.md の振り分け表が各ファイルを読む条件を名指しする。ラベルの実在確認と起票コマンドは `filing.md` の中で隣接させたままにしてあり、互いに分断していない（`create-issue` と意図的に同じ手順を持つ点も変わらない）。
- 回帰スイートを整理した。`out-of-scope-routing` は、スキルの規定の針が置かれたファイルの**ちょうど 1 行**に出現することを要求するようになった。あわせて、本線の針が置かれた節の中でもちょうど 1 行であること、振り分け表と `references/` の実体の双方向一致、スキル内の相対リンクの解決（プラグインの外を指さないこと）、他文書やファイル間で参照される見出しが期待するファイルにだけあることも見る。インラインの上限（実装 10 行）と、束ねたときの種別の優先順位は、値ごと 1 行の針にした。これで、SKILL.md から抽出した値を直書きの期待値と照合していた `out-of-scope-decision` と、針ごとの変異で検出力を測っていた `out-of-scope-routing-selftest` を撤去した。`issue-label-contract` は起票手順の検査を `references/filing.md` へ向け直し、filing.md 以外のファイルに起票コマンドが現れないこと（散文中の素の言及を除くすべての出現を数える）も見るようになった。`issue-label-supply` は本線と `references/` を合わせて参照ラベルを抽出し、`github-labels-setup` は Epic 分岐の参照先を `references/filing.md` へ向け直した。

## [0.127.0] - 2026-09-23

### 追加

- `/assess-impact` の影響度（LOW / MEDIUM / HIGH）と `/close-issue` の AC 判定（達成 / 未達 / 対象外）を Jev の Score / Choice で offline 評価するための評価セット生成器 `scripts/jev/build-judgment-eval-sets.ts` を追加した。assess-impact は同梱 fixture の `input.md` / `expected.md` から Score(3) を、close-issue は閉じた Issue の完了報告コメント（AC 検証結果の表）から Choice(3) を決定的に生成する。`gh` に触るのは `--fetch-closed-issues` だけで cache（Issue ごとに 1 つの JSON）にだけ書き、生成は cache だけを読む。閉じた Issue の判定はほぼ全件「達成」で緩い側を測る負例が無いため、同じ Issue 内で根拠をずらした合成負例（group `shuffled`）を足す。`--summarize` で過剰 / 過少（assess-impact）、厳しい側 / 緩い側（close-issue）を分けて数える。
- `tests/jev-adapter` に生成器の検査（I 節、偽 `gh` を含む 44 件）を足した（検査 106 → 150）。判定不能（期待影響度の欠落・cache 空・判定セルが読めない・全 Issue が読めずセット 0 行・gh 失敗）は exit 2 で止め、セットを小さくして成功にはしない。途中で `gh` が失敗した回は部分 cache を残さない。
- Jev の判定点切替 `FF_JEV_MODE=off|on`（既定 `off`）と共通入口 `scripts/jev/jev-decide.sh` を追加した。`off` は Jev を呼ばず記録も書かず、従来経路と同一。`on` は判定点ごとに Jev を先に 1 回呼び、全質問の confidence が閾値（`FF_JEV_MIN_CONFIDENCE`。共通既定 0.99、Noul の判定点 `novelty` / `retro` は組み込み既定 0.6。判定点別に `FF_JEV_MIN_CONFIDENCE_` に判定点名（大文字）を続けた変数）以上のときだけ判定を採用（exit 0）し、閾値未満（exit 10）と Jev の失敗・記録先へ書けない回（exit 11）はその判定点だけ従来経路へ落ちる二段構え。shadow の二重走行はしない。`FF_JEV_POINTS` で判定点別に外せ（設定済みの空値は 0 件）、`TYPESAFE_MODEL` 未指定なら閾値を較正した `jev-1.13.0` に固定する。採用 / fallback は作業ツリーの外の追記専用 JSONL（既定 `${XDG_STATE_HOME:-~/.local/state}/ff-dev-toolkit/jev-decisions/`。`--status` が実効パスを出す）に残り、`--summarize` で判定点別の採用率・帯別件数・後で人が覆した率（`--overturn`）を表にする。
- `/ace-curate` の新規性判定（読者が取るアクションが既存エントリと同一か）と `/retrospective` の観測同一性判定に、同じ質問 fixture `scripts/jev/questions/same-action.json` で `jev-decide.sh novelty` / `jev-decide.sh retro` を当てる切替節を足した。既定 `off` では手順・出力は従来どおり。state は日本語のまま渡す（英訳 A/B で差が検出されず、英訳コストが節約分を上回った実測に基づく）。
- `tests/jev-adapter` に K 節（切替入口の契約: off は通信も記録もしない・on だけでは課金経路が開かない・閾値未満と失敗は非 0 で従来経路へ・記録先へ書けなければ採用しない・置換されないプレースホルダは送らない・集計は空の記録を表にしない）を足した。
- 「Jev へ渡す判定対象を英語にすると精度が変わるか」を配線も質問文も変えずに測るため、既存の評価セットから「言語だけが違う」対を作る後処理 `scripts/jev/build-lang-ab-sets.ts` を追加した。`--extract`（母集団から決定的に N 件）/ `--template`（英訳対象の列挙）/ `--translate`（翻訳 fixture の適用）/ `--compare`（一致率・group 別・confidence 帯別・不一致 id の差集合）の 4 モード。英訳するのは判定対象の日本語データ（`state` の各フィールドと、state と対で判定される候補文 `questions.*.instructions.candidate`）だけで、識別子（ファイルパス・ASCII の id・質問文と criteria）は訳さない。翻訳 fixture は原文の内容ハッシュをキーにするので、同じ日本語には必ず同じ英訳が当たり、生成元が更新されて原文が変わればキーを引けず「翻訳の無い原文」で止まる（古い英訳を黙って使い続けない）。
- 判定不能を緑へ倒さない: 許可パス外に日本語が残る（混在言語）・翻訳の無い原文・空の訳・訳のコードスパン外に残った日本語・バッククォートが閉じずコードスパンを確定できない・使われない fixture 項目・重複キー・`_meta` に翻訳日 / 翻訳者が無い・母集団が要求件数に足りない・元セットや結果ファイルに重複 id がある・比較する結果の id 集合が違う・同じ id の `group` / `expected` / 質問 id が ja・en で違う・結果行に `match` / `predicted` / `usage.input_tokens` が無い・1 行に質問が 2 件以上ある・`--out` へ書けない・同じフラグを 2 回渡した、はすべて exit 2。
- `tests/jev-adapter` に J 節（39 件）を足した（検査 150 → 189）。

### 修正

- 終了コード誤読ガード（`guard-exit-code.sh`）と静的検出器 `tests/lib/exit-code-guard.sh` に `gate-exit-dropped` タグを追加。全件ゲートの起動行の終了コードを `$?` または変数として論理行をまたいで持ち越し、`exit $rc` で伝播も制御構文などで消費もされないまま終端する形（ログへリダイレクトした `run-all.sh` の起動行の次行に `rc=$?` と `echo` だけを置き、末尾の `exit $rc` を落とした形）を実行前に止め、起動行に `rc=$?; echo "EXIT=$rc"; exit $rc` を足す書き換え案を実値で示す。
- 同検出器の引用符処理を 3 点改善。二重引用符の中の `$(…)` は対応する `)` まで不透明に伏せ（内側の `"` を外側の閉じ引用符と誤認しない）、閉じていない引用符を持つ物理行は閉じるまで（最大 20 行）1 論理行に結合する（複数行の引用文字列の中間行を起動と誤認しない）、heredoc の本文は opener から終端行まで読み飛ばす（本文の対にならない引用符が後続の実行行を伏せない。未終端は誤検出とみなし読み飛ばした行を流し直す）。
- 既知の限界として、引数なしの `exit` / `return` はゲートの終了コードの伝播と見なさない（`run-all.sh` をログへリダイレクトした起動の直後に裸の `exit` を置く形は赤になる。`rc=$?; echo "EXIT=$rc"; exit $rc` へ書き換えると通る）。裸の `exit` が運ぶのは直前のコマンドの終了コードで、ゲートがパイプの手前段や background 起動の背後にあるときそれはゲートのものではないため、伝播として扱うと偽の緑が 5 形開く。
- ブロック境界の判定を先頭語で行う。`fi` / `done` / `esac` / `else` / `elif` / `then` / `do` / `;;` と閉じ括弧で始まる行は、条件やリダイレクトが付いていても構文の境界として扱い、終了コードの追跡をそこで閉じる（`elif` を挟む分岐や、リダイレクトの付いた `done` で閉じるループの最終コマンドがゲートである形を、正しい形として通す）。
- 失敗理由の一時ファイルに対するハードリンク数チェックが GNU coreutils 環境で常に「取得不能」へ倒れ、既存ファイルへの上書きが必ず拒否される問題を修正。GNU の `stat -f` は書式指定ではなくファイルシステム情報の表示であり、書式に見える引数を実在しない対象として扱いながら、実在する対象のファイルシステム情報は標準出力へ出す。そのため 2 つの呼び出しをコマンドとして繋ぐと両者の出力が混ざり、リンク数として読めない文字列になっていた。繋ぐ対象を代入そのものに変え、成功した側の値だけを採る形にした。
- この不具合により Linux 上では multi-agent の失敗分類が実行開始時の初期値のまま固定され、時間切れ・空出力・サンドボックス未確認のいずれもが「CLI が異常終了した」として案内されていた。分類が本来の理由へ更新されるようになり、時間切れに対する再実行案内と、時間を足しても直らない失敗に対する原因確認の案内が正しく出し分けられる。
- 回帰ガードを multi-agent-timeout の suite へ追加した。GNU と BSD の両方の意味論を再現する `stat` を順に PATH の先頭へ置き、どちらでも理由ファイルの再記録が呼び出し側へ届くこと、ハードリンク経由の上書きが拒否されリンク数が実数で名指しされることを検査する。開発環境と週次 CI は一方の方言しか踏めないため、両方を置いて初めて双方の分岐が両環境で測れる。
- 併せて、リンク数を一切取得できない環境（`stat` が無い・壊れている）でも上書きせず既定の文言で拒否理由を名指しすることを検査に加えた。この分岐はこれまでどの検査も踏んでおらず、取得失敗を許可側へ倒す変更が検査を素通りできる状態だった。
- 検査自身の空振りも塞いだ。置いた `stat` が PATH 解決で勝たなければ、検査は宿主の実体を触るだけになり本体が壊れていても緑になる。解決先が置いた実体であることを事前と各検査内の両方で確認し、外れた回は赤にする。
- `check-entry-format` の ID 形状ゲートが、旧 3 桁連番のまま 999 を超えて 4 桁へ到達した導入先の既存エントリ（`ACE-1000`〜）を弾いていた問題を修正（[公開 ff-dev-toolkit の報告](https://github.com/feel-flow/ff-dev-toolkit/issues/120)）。`ACE_ENTRY_ID_SHAPE` の単段を「3 桁ちょうど」から「3 桁ちょうど、または先頭ゼロ無しの 4 桁以上」へ広げた。採番規則は既存 ID の改名を禁じているため、桁数を 3 に固定すると改名以外に通す手段が無かった。
- 先頭ゼロの `ACE-0001`・2 桁以下の `ACE-1` / `ACE-01`・連番の無い Issue 由来 `ACE-i425` は従来どおり拒否する（判定が変わるのは 4 桁以上の旧連番だけ）。形状違反の誘導文と、同梱 PLAYBOOK テンプレート §エントリID規則の「妥当な形状」「既存 ID の扱い」を「旧連番形式（3 桁ちょうどは先頭ゼロ可、4 桁以上は先頭ゼロ不可）」へ揃えた。
...
- `docs-template/05-operations/deployment/workflow-principles.md` に原則4「定型判断の即決と熟慮の起動条件」を追加した。規定は推論で導出できる上位視点で書き、導出できない事実だけを具体に残し、繰り返し外れる判断は機械ゲートへ移す。定型判断は既存の表を機械適用して即決し、熟慮を起こす検知可能なトリガを既存 5 規定への索引として 1 箇所にまとめた（見出し「3つの運用原則」は「運用原則」に改名）
- `/retrospective` の「昇格閾値と特急レーン」に、closed 済み対策が再発したときは前の対策と同じ型（規定文言・参照経路・needle・名簿の追加）を再提案せず、構造を変える案だけを候補にする規定を追加した。`tests/retrospective-contract` が禁止と許可を対の針で固定する

## [0.121.1] - 2026-09-18

### 修正

- レビュー重大度パーサーが、ラベルとコロンをまとめて markdown 強調で囲んだ件数行（`- **Critical:** 0` / `- **Critical:** なし` / `- *Critical:* 0`、全角コロンの `- **Critical：** 0` を含む）をゼロ件として認識せず、Critical 0 件のレビューでも `CRITICAL_BLOCK` マーカーを立てていた問題を修正した。強調記号がコロンの後ろへ来るため、ラベル側の強調許容でも件数値側の強調許容でも吸収できず、該当行が件数行ではなくラベル付き指摘行として分類されていた。強調で囲まれた件数ゼロ（`- Critical: **0**`）の修正では届かない別経路で、レビュー本文の書式は実行ごとに揺れるため、同じ差分・同じ 0 件でもレビューゲートが通ったり落ちたりする状態が残っていた
- コロン形は受理判定（レビュー本文が実体を持つかの判定）と Critical 検出の両方、および critical / warning / suggestion の全ラベルで共有されるため、今回の緩和は両判定・全ラベルへ同時に効く。強調コロン跨ぎの非ゼロ件数（`- **Critical:** 1`）と実際の指摘は従来どおり Critical として検出し、critical 以外のラベル（`- **Warning:** 0` / `- **Suggestion:** 0`）が Critical 検出へ漏れることもない
- あわせて、bullet を持たない強調コロン跨ぎの件数行（`**Critical:** 0`）が受理ゲートで弾かれ、その観点の結果が欠測として扱われる問題も解消した。件数行は値を行内に持つ自己完結行として bullet を必須にしていないため、この形も受理する
- あわせて、ラベルとコロンをまとめて強調した bullet なしの指摘行（`**Critical:** 認証チェックの欠落`）が受理も検出もされなかった問題を修正した。行頭強調の指摘行はラベル直後に強調記号を必須としていたため、強調がコロンの後ろへ回るこの形はどの分類にも該当せず、同じ本文に件数行が 1 行でもあると「本文は受理されたが Critical は 0 件」という無音の見落としになっていた
- critical 見出し配下でゼロ宣言の後ろの箇条書きを実所見に数えない抑止について、抑止のきっかけを**ラベルを見て**判定するようにした。これまでは重大度ラベルを問わずゼロ件数行であれば抑止が掛かっていたため、critical 節に `- Warning: 0`（および強調形の `- **Warning:** 0`、集約形の `- **Suggestion:** 0 / **Critical:** 2`）が置かれていると、その後ろに書かれた本物の Critical 指摘が検出されなかった。ラベルを持たないゼロ件報告行と散文のゼロ宣言は従来どおり抑止のきっかけになる

## [0.121.0] - 2026-09-18

### 変更

- レビュー走行中の編集抑止ガードが、書き込み先ではなくツール経路で止めていたため、レビュー結果の破棄を決めるリビジョン指紋に映らない書き込みまで拒否していた。gitignore 済みのパスと作業ツリーの外への書き込みを、`Edit` / `Write` / `MultiEdit` / `NotebookEdit` でも Bash 経由でも通すようにした。レビュー待ち時間に勧めている GitHub 側の作業（`gh` の出力をファイルへ受ける形）がそのまま実行できる
- 緩めたのは判定できた書き込み先だけで、判定できない形（変数展開・コマンド置換・読めないスクリプト・未終端 heredoc）と `git check-ignore` を起動できない回は従来どおり走行中は拒否する。書き込み先が別の場所を指す記号リンクのときは、リンク先で判定し直す
- あわせて、ノートブックの編集ツールが渡す `notebook_path` を読むようにした。以前は書き込み先を取り出せず、レビュー出力先への書き込みまで拒否していた

### 修正

- レビュー重大度パーサーが、markdown 強調で囲まれた件数ゼロ（`**0**` / `*0*` / `__0__`）をゼロ件として認識せず、Critical 0 件のレビューでも `CRITICAL_BLOCK` マーカーを立てていた問題を修正した。ラベル側の強調は以前から許容していたが、件数値側の強調は許容しておらず、該当行が件数行ではなくラベル付き指摘行として分類されていた。レビュー本文の書式は実行ごとに揺れるため、同じ差分・同じ 0 件でもレビューゲートが通ったり落ちたりする状態になっていた
- ゼロ語（`なし` / `none` / `指摘なし` / `指摘 0 件` など）についても同様に、強調で囲まれた形をゼロ件として扱うようにした。件数値側だけを直すとゼロ語側に同根の非対称が残り、`- **指摘なし**` / `- **なし**` が偽の Critical として発火し続ける。あわせて、本文が強調付きゼロ語だけのレビュー応答が受理ゲートで弾かれる（その観点の結果が欠測として扱われる）問題も解消した
- 強調付きの非ゼロ件数（`**1**` など）と、強調された実際の指摘は従来どおり Critical として検出する。ただし**同一の重大度スコープにゼロ宣言が先行する場合は従来どおり抑止される** — 強調形のゼロ宣言（`- **なし**` 等）もゼロ宣言として扱うようになったため、「ゼロ宣言の後ろに実所見を続けた矛盾レポートは見出しスコープ判定では数えない」という既存の限界が、強調形にも及ぶ。件数行を併記していれば従来どおり件数行の判定で検出される
- 起票ラベル契約ガードが優先度ラベルを `priority:` という綴りでしか判定しておらず、別の命名規則（例 `P1-High`）を採るプロジェクトでは優先度の欠落を検出できないうえ、その優先度ラベル自身が種別として充足していた。優先度の水準語は推奨ラベル構成の正本から、綴りは対象リポジトリのラベル一覧から引くようにして、別命名でも優先度系統を判定する。欠落を伝える文面の例示もそのリポジトリで実在する綴りになる
- 別命名の判定は「正本の綴りの優先度ラベルが対象リポジトリに 1 件も無い」ときにだけ働き、さらに同じ命名のまとまりが推奨ラベル構成の水準の過半を覆う場合に限る。順序を持つ別の軸（見積もり規模など 2 水準だけのもの）を優先度と読み違えて正しい起票を止めないため。既定の `priority:` 命名のプロジェクトでは判定は従来どおり
- あわせてラベル照合の内部実装から 1 件あたり 2 つのプロセス起動を除いた。上限に近い数のラベルを持つリポジトリで、起票 1 回あたりの所要が実測 2281 ミリ秒から 723 ミリ秒へ短くなる

## [0.120.0] - 2026-09-18

### 変更

- 委譲先の background 実行を止める PreToolUse ガードに、名簿（長時間ゲートの綴り）に依らない一般トリガを追加した。サブエージェントが任意のコマンドを `run_in_background: true` で明示的に起動した場合は、コマンド名に関わらず deny する。判定材料は構造化フィールドだけでコマンド本文を解析しないため、散文由来の誤検知面は増えない。親（メインセッション）の意図的な background 運用は従来どおり対象外。
- ハーネスの自動 background 化（Bash ツールの `timeout` 未指定・過少）に対する検出は従来どおり名簿に限定される。名簿の綴りはガード本体の `LONG_GATE_ROSTER` 1 行が正本になり、3 段の前置フィルタと判定がすべてそこから作られる。綴りを 1 つ足せば全段に効くため、一部の段だけが広がって新しい綴りが判定に到達しない状態が起きなくなった。
- 名簿に変異マトリクスの実行スクリプト（`run-mutations.sh`）の綴りを追加した。全件ゲートの既定一覧に載らない設計のため、自分の綴りで起動するしかないスクリプトである（スクリプト自体は本版で変更していない）。
- deny の本文と発火ログが、ラッパ（`bash`）ではなく実際に走るコマンド名を名乗るようになった。
- 委譲オーケストレーションの運用文書を、上記のトリガ 2 系統（名簿に縛られない明示 background / 名簿に縛られる自動 background 化）に追随させた。名簿へ載せられない綴り（suite 共通の入口）が在ることと、その経路が開いたままであることも明記した。
- 同期手順の契約検査 sync-sha-contract に、同期サイクルの in-flight 判定と Release 反映遅れの吸収について 15 件の針を追加した。いずれも SSOT 専用の検査で、公開チェックアウトでは従来どおりスキップされる
- 同 suite のヘッダへ、針が当たらない入力（検査対象の不在・空ファイル）で赤になることの実測と、15 変異それぞれの検出結果を記録した
- 上記の宣言を足したことに伴い、全件ゲートの登録照合が使う免除名簿から sync-sha-contract を外した。宣言を後から足した suite が名簿に残ると「未実測」を主張し続ける形になり登録照合が赤になる

### ドキュメント

- クロスモデルレビューのスキル手順に、新しいブランチでの初回レビューには `--fresh` が要るという前提を明記した。未解消 Critical の系列ガードが持つ「unfiltered full review なら新シリーズを自動開始する」逃げ道は、`--mode cross-model` と `--perspective` を使う標準の起動形では構造的に発火しないため、新ブランチの初回は必ず一度中断していた。前のブランチがすでにマージ済みでも同じで、系列 ID はダイジェストのためガード側に到達可能性を判定する材料が無い。同じブランチの 2 回目以降に `--fresh` を付けるとラウンド間の Critical 持ち越しが失われる点も併記した。
- クロスモデルレビューの運用文書にも同じ前提を追記した。「フルレビューなら手動退避は不要」という既存の記述が成立するのはフルレビューだけで、観点を絞る標準の起動形は自動開始に構造的に入らない。
...
- `tests/run-all.sh` の登録照合が、新設 suite の `verify.sh` に `# 空振り検出:` 宣言（検査の針が当たらない入力で赤になる実測の記録。本文必須）を要求するようになった。既存 suite は `MISS_PROBE_BASELINE` で免除し、名簿は縮める方向にしか動かさない（宣言を足した suite が名簿に残る・名簿に実在しない名前がある・本文の無い宣言は、いずれも赤）。宣言ダンプ `FF_RUN_ALL_DUMP_DECLARATIONS=1` の 1 行は末尾に probe 列（本文付き宣言の有無）が増え、`名前:skip:yes:no:bad:probe` の 6 列になる
...
- 週次の公開 CI が、全スイート通過にもかかわらず赤になっていた問題を修正した。ソースリポジトリ専用のスイート 3 件が除外名簿から漏れていたため、それらが実行対象へ入って環境都合でスキップし、「許容名簿に無いスキップ」の検査で失敗していた。該当スイートを除外名簿へ追加し、除外理由をコメントに明記した。あわせて、ルート直下の指示文書の一致を検査する新しいスイートも同じ理由で除外名簿へ入れた

## [0.116.0] - 2026-09-16

### 追加

- ルート指示文書が複数ある構成（Claude Code が読む `CLAUDE.md` と、他ホストが読む `AGENTS.md`）で、2 文書の記載一致を検査する回帰 suite `tests/root-instructions-parity/` を追加した。対象ファイルはホスト経路の正本表から導出し、ホスト固有の差は文書側へ `host-specific` マーカーで宣言した区間だけを比較から外す。マーカー外の本文は行単位の完全一致を要求し、宣言した区間の中身が 2 文書で同じ場合（マーカーの腐り・一方を verbatim コピーで置き換えた状態）や、一方が symlink / hard link になっている場合も赤にする。外部コマンドに依存せず、正本表を持たないチェックアウトでは suite 全体をスキップする

### 変更

- ホスト経路パリティ検査（`tests/host-route-parity/`）の契約表の読み方を「1 経路につき正本ファイル 1 件」から「経路の届け先 1 つにつき正本ファイル 1 件」へ変えた。1 経路が複数の届け先を持つ場合、従来は片方のファイルに検出語があれば緑になり、もう片方の届け先へ規定が 1 行も届いていない状態が機械的に許容されていた。届け先と正本ファイルの対応づけは並び順ではなく配下関係で決め、同じファイルが 2 つの届け先へ対応づいた場合と、どの届け先にも対応づかない正本ファイルがある場合を名指しで赤にする。複数の届け先を持つ経路を宣言している正本表は、契約行の正本ファイル列を届け先ごとの列挙へ更新する必要がある
- 正本表の 4 表を読む実装（sentinel の切り出し・行と列の分解・経路 ID からの引き当て）を `tests/lib/host-parity-tables.sh` へ切り出し、`host-route-parity` と `root-instructions-parity` で共有するようにした。既存検査の出力は変わらない

## [0.115.2] - 2026-09-16

### 修正

- 配布先 checkout（公開リポジトリ単体）で回帰スイートの `run-all` が awk の fatal で中断していた問題を修正した。走査対象にリポジトリ直下の `scripts/*.sh` を無条件で渡しており、そのディレクトリを持たない配布先では glob が展開されずリテラル文字列が awk へ渡っていた。配置を判定してから走査対象を決める形へ変更し、ソースリポジトリなのに当該ディレクトリが無い場合と、配置の標識が食い違う場合はいずれも失敗として報告する（配布先扱いへ倒さない）。ソースリポジトリ側の走査件数は変更していない
...
- Bash ツールの実行前に終了コードの誤読・握り潰しを止める PreToolUse ガードを追加した。測りたい処理の終了コードを運ばない 3 つの形（全件ゲートの起動を含む行が診断・出力整形で終端している / head・tail・less・more・cat・tee・wc で終わるパイプラインの直後で終了コード変数を読んでいる / PIPESTATUS を参照している）を実行前に停止する。切り出しが一意に決まる形では、検出した実コマンドを書き換えた案を示す（決まらない形では一般形だけを出し、誤った案内をしない）。
- 判定は同梱の静的検出器 tests/lib/exit-code-guard.sh をそのまま再利用し、ガード側は判定規則を持たない。静的検出器の入口はいずれもファイルを入力に取るため、エージェントがその場で組み立てた Bash 呼び出しには原理的に届かなかった。このガードはその走査面だけを足す。
- 単体起動・短絡する連結・推奨形（終了コードを変数へ取ってから伝播させる形）・単一引用符の内側に同じ綴りが現れるだけのコマンド・heredoc 本文は素通しする。どうしても止まる形で実行したい場合はコマンドの先頭に FF_EXIT_CODE_ACK=1 を付ける。ガードごと無効化する場合は環境変数 FF_DEV_TOOLKIT_SKIP_EXIT_CODE_GUARD=1 を設定する。
- このガードだけは判定を完了できないときに素通しせず、判定不能である旨を出して停止する（検出器の欠落・破損、走査 awk の失敗、jq フィルタの実行失敗、検出器出力の解釈不能）。停止する面は終了コード変数・PIPESTATUS・全件ゲートの綴りを含む候補コマンドに限るため、検出器が壊れても無関係な Bash 呼び出しは止まらない。停止用の JSON を組み立てられない場合も黙って許可へ落ちず、終了コード 2 と標準エラー出力でブロックする。
- run-all のサマリーが、実行した suite の経過秒（elapsed-sec）と 1 秒以上の遅い順（slowest、最大 8 件）を出すようになった。未実行の suite には秒を付けず、時計が取れない環境では計測行を出さずに完走する。

## [0.113.0] - 2026-09-15

### 追加

- GitHub の sub-issues API で親子 Issue を紐付けるとき、`gh api -f sub_issue_id=` が値を文字列で送って全件 422 になる取り違えを、呼び出し口のヘルパと PreToolUse ガードで止めるようになった。ヘルパは Issue 番号から database ID を解決して型付きの `-F` で送り、失敗時は HTTP 本文を出して 1 件目で止まる。ガードは `-f sub_issue_id=` と、同じ API の integer フィールド `after_id` / `before_id` を実行前に抜け道付き deny で止め、`-F` とヘルパを案内する。対象を `gh api -f` 全般へは広げない（GraphQL の query や REST の base、per_page は文字列として正しい）
- レビュー凍結レーンの名簿がホストのレビュアー実体へ追随しているかを、セッション開始時に自動で突き合わせる `SessionStart` hook（`hooks/check-review-roster-drift.sh`）を追加した。名簿から落ちたレビュアーが増えたこと・実体から消えたことを通知する。無音で終わるのはレビュアー実体が 1 つも見つからない環境（当該プラグイン未導入）と一致した場合だけで、走査が成立しない回（ディレクトリを列挙できない・`agents/` が無い・名簿を抽出できない等）は「一致」ではなく「判定不能」として通知する。同じプラグインの版違いの写しが併存する環境では、最新版（版を読めないときは写し全体の和集合）を 1 つの判定へ畳むので、名簿を直したあとに古い写しが通知を出し続けることはない。`FF_DEV_TOOLKIT_SKIP_REVIEW_ROSTER_CHECK=1` で無効化できる
- 公開面の一覧 `plugins/ff-dev-toolkit/PUBLIC-SURFACE.md` を配布物へ同梱した。skill の起動名・hooks・導入先へ配置されるパス・`FF_*` 環境変数について、互換性を約束する「契約」と予告なく変わる「内部」を区別して列挙している。公開 README からも導線を張った
- 配布物に現れる `FF_*` 環境変数をすべて契約と内部のどちらかへ分類した。未分類が残っていないことは回帰ゲートが両方向の集合一致で検査するため、新しい環境変数を足して一覧へ載せ忘れると赤になる
- CHANGELOG へ破壊的変更を記録する規約を決めた。契約側の要素を削除・改名・意味変更したときは、各版の先頭に置かれる `### 破壊的変更` 節へ、何が変わったかと導入先の復旧手順を書く。内部の変更は従来どおり変更・削除の節に書く

### 変更

- ace-curate の索引行の書き方を「索引テーブルのヘッダ行の列順を読んでから値を並べる」手順に変えた。ヘッダは導入先ごとに列順が違うため、固定の列順の雛形を写すとカテゴリ列にタイトルが入った索引行が量産される。ヘッダがタイトル先の Playbook では従来どおりタイトル先で入る
- check-category-size が PLAYBOOK.md のエントリ一覧節の索引テーブルを検査するようになった。ID 列とカテゴリ列の位置をヘッダ行から読み、各行のカテゴリ列をエントリ本文の Category 値と突き合わせて、食い違い・列数不一致・空欄を警告する（終了コードは変えない）
- 索引行のセル区切りを GFM に合わせ、インラインコードスパンの中の素の縦棒も列を増やすものとして扱うようにした。GitHub 上で列がずれて描画される索引行を検出できる

### 修正

- `/ace-refine` の archive リンク検査（`check-archive-links`）が、YAML frontmatter で始まるアーカイブファイルの冒頭注記を認識するようになりました。従来は 1 行目の frontmatter 区切りで冒頭走査が打ち切られ、Parent ブロック内に「保全本文内の相対リンクは live 基準」注記があっても「冒頭注記 なし」と判定して exit 1 になっていました。docs の frontmatter が必須なプロジェクトでは、注記を追加しても refine の完了条件（全ゲート exit 0）を満たせない状態でした。
- Claude Code セットアップ手順書（docs-template/SETUP_CLAUDE_CODE.md）が CLAUDE.md 生成ブロックの外で案内していたパスを、手順書をコピーした先のプロジェクトから解決できる表記へ統一した。配布元にしか無い docs-template/ 起点の参照、階層を落とした実装パターン文書の参照、リンク文言とリンク先の食い違い、実体の無い文書への参照を解消している
- 同手順書で、初期展開の対象外にある文書への Markdown リンクを、コピー元パス付きの案内テキストへ変更した。初期展開の直後にリンク切れにならない
- docs-template-portability の検査を、生成ブロックの中だけでなく外の参照にも広げた。コピー後に解決しないパスや、初期展開の対象外へのリンクが混入した時点で赤くなる
- 週次 public run-all が checkout 直後に `origin/HEAD` を remote の既定ブランチへ向けるようになりました。`actions/checkout` は `git clone` と違い `refs/remotes/origin/HEAD` を置かないため、既定ブランチの解決に `origin/HEAD` を使う ace-curate の保護判定が公開 CI で全シナリオ失敗していました。
- 週次 CI の checkout 直後に置いたランナー特性への対策が開発元側 workflow だけに入る非対称を、既存の changelog-fragments ゲートが step 名の突き合わせで検出するようになりました。
- ホスト経路パリティ検査（`tests/host-route-parity/`）の README の変異表へ、「経路表が届け先として宣言している直下の指示文書を削除する」変異を追加した。検査ロジックは変更していない。同検査は届け先の宣言と実体を集合一致で照合するため、1 経路に複数の届け先を宣言すると「宣言に無い文書の新設を拒否する」向きに加えて「宣言した文書の不在を拒否する」向きにも働く。開発元リポジトリでの実測では、宣言だけを足して実体を置かない状態は失敗 2 件・終了コード 1 になる

## [0.112.1] - 2026-09-15

### 修正

- `/retrospective` が観測台帳へ記録する前に、デフォルト統合ブランチの先行を照合するようになった。台帳は統合ブランチの共有文書で、これまでは並行セッションが先に更新していても押し切って記録内容を作り、直 push が non-fast-forward で弾かれて初めて気づく形だった。弾かれた時点では同一性の判定・`Count` の加算・新規エントリの連番採番がすべて古い台帳から決まっているため、同じ観測が別セッションで既に記録済みでも件数が二重に増えうる
- 先行を検出したときの復帰手順を規定した。取り込んでから記録手順の最初へ戻って作り直すのが正で、並行セッションが同じ主張を既に記録していれば新規エントリではなく既存エントリの件数加算へ落ちる。新規追記になる場合の連番は取り込んだ後の台帳から採り直す。取り込みは fast-forward のみとし、rebase や merge で作りかけの記録内容をローカルに残さない。取り込めないまま停止した場合は台帳へ書かず、観測を振り返り結果の報告に残す。先行セッションを force で上書きすることはしない
- 台帳への書き込みを、台帳のパスを明示したコミットに固定した。従来は索引に載った無関係な変更が知見コミットへ紛れ込み、統合ブランチへ直接反映されうる形だった
- 照合の向きは ACE Playbook の追記が持つ既存ガードと揃えた一方、未コミット変更の検査は台帳のパスへ絞ってある。台帳への書き込みは 1 ファイルの単独コミットなので、作業ツリー全体を対象にすると統合ブランチ上で作業中に単独実行した振り返りが記録できなくなるため。絞り込みによってパス指定が実行ディレクトリ依存になる問題は、リポジトリ先頭からの指定に固定して塞いだ
- 照合フェンスを実際に実行して検証する回帰スイートを追加した。文字列の一致だけでは、比較の向きの反転・実行ディレクトリ依存の見落とし・停止の握り潰しを検出できないため、隔離したリポジトリを組んで通す回と止まる回の双方を実測し、止まる回は停止理由まで照合する
- 自動振り返りの発火判定で、1 つの応答メッセージが「振り返りの結果」と「次のチェーン末尾コマンドの実行」を同時に含む場合の扱いを修正した。判定はメッセージ内の位置で決まるようになり、末尾コマンドが振り返りより後なら振り返りを要求し、振り返りが後なら実施済みとして扱う。従来は先に見つかった方を採用していたため、Epic の一括対応のように「ある PR の振り返りを書いてから次の PR のクリーンアップへ進む」流れで、後続分の振り返りが黙って要求されなくなっていた
- コマンド位置の `gh pr merge` の検出に、バッククォートによるコマンド置換、ブレースグループ、`!` による否定、`if` の条件部、`else` の直後、そして引数なしで行末に来る形を追加した。いずれも従来は実行として数えられず、チェーン末尾に到達したターンが見落とされていた。引用された文字列を実行と誤認しないための判定はそのままで、`sed` の区切り文字に挟まれた同じ語句は引き続き実行として数えない
- 変異試験の前段にある「変異の対象が想定した件数だけ存在するか」を確かめるガードが、対象が 1 件も無いときに限り黙って素通りしていた問題を修正した。対象が消えた変異は空振りするため、検出力の不足と区別できないまま緑で通りうる状態だった
...
- `/retrospective` の観測台帳 `Count` を「計上する観測メモ 1 行 = 1 回」と定義した。1 行へ複数回を畳んでも乗数にせず、同一セッションの同一原因は 1 回とし、既存の `Count` は遡及しない。昇格閾値の入力が行数と事象の多重度で割れないようにする。
- 開発元リポジトリでスキルを実行するとき、インストール済みプラグインのスナップショットより作業ツリーのスキル本文を正本とする旨を開発入口へ書いた。docs-gates はその一文を開発元 checkout でのみ検査する（公開 checkout では適用外）。
...
- 配布するテスト方針の文書に「変異注入の結果の読み方と書き戻し先」を追加した。これまでは「変異が実際に当たったかを確かめる」作法だけがあり、**当てた結果をどう読むか**（検出できなかった変異の扱い）と**どこへ残すか**が無かった。この 2 つが欠けている場所で、検出力ゼロの検査が「緑だから守られている」と読まれたまま残る。検査を書いた直後にその場で測ること、検出できなかった変異はそのまま検査追加の宿題であること（緑は守られている証拠ではなく、誰も測っていないことの実測）、どうしても赤にできない変異は、どの形（状態を作れない / 書き換えても振る舞いが変わらない / 別の層が吸収している）に当たるかまで書いて残すこと、結果は当該検査自身の先頭コメントへ書き戻すことを明記し、記述を検査で固定した。

### 変更

- レビュー凍結レーンの適用範囲を 2 方向へ広げた。(1) レビューをホストのセッションへ委譲する経路は、汎用の subagent 種別で起動されるため名簿に載らず、レビューが走っている間も編集が凍結されなかった。名簿へ汎用種別を足すとレビュー以外の委譲まで凍結してしまうので、識別を「何の種別で起動したか」から「何を渡されたか」（委譲プロンプトのパス、または引き渡し文が載せるマーカー）へ移した。識別は「行頭のマーカー行 + 引き渡されたプロンプトが実在すること」をすべて要求する（語がただ含まれるだけでは凍結しない）。(2) 委譲経路のレーンは専用の種別名で記帳する。汎用の種別名のまま記帳すると、無関係な同種のサブエージェントの開始・終了が「同じ種別で対応づいていない最も古いレーン」を引き受ける仕組みに拾われ、レビューが走っている最中に凍結が解ける。この経路のレーンは終了イベントでは閉じない（終了イベントは相関する識別子を持たず、汎用の種別では正しい委譲先を見分けられないため、早く解ける側へ倒さない）。出口は 3 つで、起動が拒否されたときの解放、委譲結果の回収（回収の到達が「委譲したレビューは終わった」の宣言で、正常完了の確定的な出口）、どちらも来なければ寿命上限。あわせて、委譲先が自分の結果ファイル（レビュー出力先の配下）を書くことは凍結の対象外にした — ここを止めると引き渡しの手順そのものが自分のレーンで自分を止める。識別子の正規化が記号を削るだけで別物を同じ形へ潰していたのも直した（潰れた識別子で対応づけると、無関係な終了が別のレーンを閉じる）。
- レーンを取る種別の名簿は別プラグインのレビュアー群を列挙しており、そちらが増えても追随する手段が無かった。実体のディレクトリと名簿を突き合わせる検査を追加し、増えた側・消えた側・意図的な除外・判定不能（ディレクトリ不在・実体 0 件・名簿を抽出できない）を区別して返すようにした。検査は実体を模した入力で全分岐を常時実行するので、当該プラグインが導入されていない環境でも検出力が落ちない。名簿の件数そのものは、実体と同じ源から導かない独立した数として別に固定した（両方を同じ源から導くと、源が縮んだときに一致したまま通る）。
- 未解消 Critical のガードが中断したときの案内を、原因ごとに具体化した。系列の識別に失敗する原因は「プロジェクトルートが消えた・読めない」と「git が HEAD を読めない」に分かれるが、いずれも同じ終了コードへ潰れていたため、案内は「作業ツリーの中で、ルートが在って HEAD が解決できる状態で実行する」という一般化した言い方にせざるを得なかった。後者はさらに「作業ツリーの外で走っている」と「作業ツリーの管理データが壊れている」の 2 つを含むが、これは git の側から区別できない（どちらも同じ終了コードで、作業ツリー判定も同じ値を返す）ので、案内は両方を名乗って両方の直し方を出す。原因ごとに直す場所が違うので、これでは読み手が自分の状況に当てはまるかを判断できない。失敗の理由を別チャネルで運び、原因を名指しする案内に変えた。理由を変数ではなくファイルで運ぶのは、この識別子が常にコマンド置換（サブシェル）の中で計算されるためで、同じ制約をアダプタ側の失敗理由がすでに同じ方法で解いている。
- 理由を読めなかった回（記録に失敗した・別の実行が残した行だった）にも、原因は名乗れないが復帰手段だけは出すようにした。統合レポートを書く側の中断にも同じ案内を出す。あわせて、理由の記録に失敗したことを黙って捨てず警告するようにし、受け付ける理由の値を入口で検証するようにした（綴りを間違えた理由が、どの分岐にも当たらないまま一般案内へ静かに落ちるのを防ぐ）。中断して終わった実行が理由ファイルを残し続けていたのも直した。
- あわせて、説明文書がこのガードの中断を 2 分岐でしか説明しておらず、分岐によって退避オプションが正解だったり有害だったりする実装と食い違っていたのを直した。3 分類（残骸は読める / ファイルを読めない / 系列を作れない）と、退避オプションを一律の復帰手段として使ってはいけないことを明記し、文書側の追随を検査で固定した。原因の名乗り方についても、区別できない 2 つを併記する実装に合わせて書き換えた。
- 委譲経路で「成果物は所定のパスへ書けたが、読み戻した内容が受理した応答と一致しない」失敗に、専用の分類と文言を与えた。従来はこの失敗が汎用の内部エラーへ落ちており、(1) 壊れている対象を名指しせず「ホストへ渡せなかった」という誤った理由が残り、(2) 失敗の記録として同じパスへ未完成の成果物を書き直すため、**内容が違うと分かったばかりの成果物をその場で上書きして食い違いの証拠を消して**いた。書き込み失敗（「そこに成果物は残らなかった」）の文言へ寄せることもしない — 事実と食い違ううえ、調査の向きが逆になる（無い側は空き容量・権限・マウント、在るが違う側はそのファイル自身・別の書き手・部分書き込み）。新しい報告は、**いま何が残っているか**を見てから名乗る — 内容だけが違うのか、空になったのか、読めなくなったのか、通常ファイルでなくなったのか。決め打ちで「在るが内容が違う」と言うと、次の一手（そのファイルの中身を見る）が空振りする回に嘘をつく。あわせて、どちらの側も上書きしていないことと、ホストの応答の在り処を名指しする。なお成果物が丸ごと消えた場合は、書き込み側の publish 後検査が先に捕まえて書き込み失敗として名乗る（この経路へは来ない）。
- レビュー凍結レーンの hook で「レーンの取得が作業ツリーの状態確認より前に来る」ことを検査で固定した。この順序は、状態確認が hook の持ち時間を使い切る遅いリポジトリで「レーンを 1 本も作らないままレビューが起動する」ことを防ぐためにあるが、順序を元へ戻す変更が検査で赤にならず、根拠がコメントだけで守られていた。検査は**時間ではなく因果**で測る — 状態確認を「解放の合図が来るまで返らない」形に差し替えると、順序が正しければレーンは状態確認へ入る前に存在し、逆なら状態確認から返れないので永久に現れない。順序の判定を固定の待ち時間へ依存させないので、赤を「またフレーキーか」と読ませない（プロセスの起動と取得到達には依存するため、そこには上限つきの生存確認を置き、超過は診断として出す）。観測はこの検査が開いた記録だけに限り、開始前に残りが無いことも確かめる。解放したあとは期限つきで刈り取り、終わらない回・異常終了した回を緑にしない。差し替えた側が実際に呼ばれたことも併せて確かめる（呼ばれないまま緑になる形を残さない）。

### 修正

- セットアップ手順が生成する `CLAUDE.md` が、消費プロジェクトには作られないディレクトリを参照していたのを直した。手順書は 1 つのファイルの中でパスの基準が切り替わる構造で、生成ブロックの外では「テンプレート側から見た位置」、中では「生成先から見た位置」で書くのが正しい。外側の書き方が中へ流れ込み、生成された `CLAUDE.md` の冒頭が「存在しないパスの文書を必ず読め」と指示する状態になっていた。セットアップ直後の `CLAUDE.md` はそのプロジェクトで AI が最初に読む文書なので、解決できない参照は初回体験をそのまま壊す。
- 基準を揃えるだけでは足りなかった箇所もある — 元の参照が階層を落としていると、揃えた結果も解決できないままになる。あわせて、生成ブロックの範囲を行番号ではなく開始・終端のマーカーで特定し、その中の参照を**配布物の実体と突き合わせて**解決できなければ落ちる検査を足した。ブロックを特定できない回は「違反 0 件」ではなく検査不成立として落とす。検査そのものが消される退化に備えて、実行された検査の件数も下限で固定した。
- codex が未導入で他の AI CLI が入っている環境で、Codex レビューのシムに `--dry-run` を渡してもプランが表示されなかったのを直した。シムは委譲先へ「この CLI だけを使う」指定を固定で足しており、codex が不在だとその観点の再割り当て先が同じ指定で除外されて実行対象が 0 件になり、プラン確認の経路がエラーで止まっていた。`--dry-run` かつ codex 不在のときだけこの固定指定を外して委譲するようにした。この指定は委譲先では分散モードのフィルタなので、外すとレビューの既定である 2 者モードへ戻り、主レビュアーが観点すべてを担当する形のプランが出る（主レビュアーが未導入の構成では、その旨のエラーで止まる — 帰結が 3 通りに分かれることを警告と終了コード表の両方へ書いた）。実行モードと、codex が導入済みの `--dry-run` は引数も終了コードも従来どおり。AI CLI が 1 本も無い環境の振る舞いも変えていない（委譲先の CLI 検出で止まる）。表示されたプランはシムを実行したときに走るものではない（実行には codex が要る）ため、その食い違いを警告と終了コード表の両方へ明記した。
- この経路は、常に成功する疑似コマンドを使ったテストに対してだけ合格していた。検査を実体の委譲先へ通すよう配線し直し、疑似コマンドへ戻す変異で赤くなることを実測した（合格の根拠が実体にあることの確認）。

## [0.107.0] - 2026-09-13

### 変更

- 公開ミラー対象の文書へ書かれた「公開側から辿れないリポジトリ識別子」の検査に、綴りを変えるだけで抜けられた 3 形を足した。(1) ホスト名を書かず `gh api` の引数としてリポジトリ識別子を渡す形、(2) 末尾ドットの完全修飾ホスト名とパーセントエンコードされた識別子、(3) 生ファイル配信ホスト（ホスト名が既存の入口条件の綴りと一致しないため、走査に一度も乗っていなかった）。(2)(3) は抽出の前段に正規化層を 1 箇所置いて畳んだので、ウェブ形と API 形の両方へ同時に効く。(1) は誤検出を抑えるため `gh` のサブコマンドを含む行に限定した別の検出器にした — 限定しないと公開対象で 39 箇所に当たるところ、限定すると 4 箇所（すべて外部所有者かプレースホルダで、組織スコープの判定が落とす）。入口を広げても既存ツリーの検出は 0 件のまま変わらない（公開対象 612 ファイルで実測）。差分限定の入口でも同じ 3 形が止まる。
- hook が受け取る標準入力の扱いを契約として明文化し、登録済みの全 hook を「読み切る」側へ揃えた。従来の契約文は「ペイロードを持つ hook はゲートより先に読め」としか定めておらず、(1) そもそも読まない hook を許すのか、(2) 上限付きで読んでいる hook が読み切れなかったときどうするのか、が未定義のままだった。セッション開始時に走る 3 本は実際に一度も読んでおらず、パイプバッファ（64KiB）を超える入力を書くと書き手が SIGPIPE を受ける状態だった（実測: 200KB の入力で 3 本とも書き手が異常終了 → 修正後は正常終了）。読まない hook を免除しなかったのは、免除が実質 denylist で、後から追加される hook を無審査で通し、しかも根拠がプラグイン側で制御できないホストのペイロードサイズに乗るため。上限付きで読む hook は上限を維持する（撤廃しても解決しない — 待ち続けた hook はホストに強制終了され、その強制終了が同じ症状を書き手へ渡したうえで hook の持ち時間を使い切る）。かわりに、上限は hook 自身の登録タイムアウトから導出することを契約として要求し、読み切れない入力では症状が残りうることを明記した。
- あわせて、登録された全 hook がこの契約を満たすことを機械的に検査する横断ガードを追加した。検査対象は hook の登録一覧から導出するので、契約に反する hook を新しく足すとゲートが赤くなる（名簿に載せ忘れても素通りしない）。
- 読み切りには上限を付ける。上限のない読み取りは入力の終わりが来るまで戻らないため、対話端末やパイプを開いたまま書かないホストでは hook が固まり、ホスト側の強制終了まで持ち時間を使い切る（実測: 上限なしでは 6 秒経っても戻らず、上限ありでは 0.1 秒台）。上限は各 hook の登録タイムアウトから、その hook 自身の処理時間を引いた残りに収まる値へ置く（1 本は自分の処理だけで持ち時間の 8 割を使うため、他の 2 本より短い上限にしている）。
- 規定の所在を固定する検査を、契約ごとの宣言データとして置ける場所を用意した。従来は 1 つの検査ファイルへ全契約が積み上がっており（1,674 行）、別々の規定を固定する変更どうしが同じファイルの同じ領域を触るため、並行作業のたびに競合解決が要った。回避は「末尾へ追加する」という書き手の注意に依存していて、構造は守っていない。契約ごとにファイルを分ければ、新しい規定を固定する変更は新しいファイルを 1 つ足すだけで済み、既存ファイルを 1 行も編集しないので、別の規定を足す変更とは競合しない。宣言で書ける検査（対象文書・節・固定文字列・ラベル）はデータ側へ、手続きが要る検査（複数文書の突き合わせ、抽出したパスの実測）は従来どおり検査ファイル側へ置く。データが読めない・1 行も解釈できない・種別が未知、はいずれも失敗として扱う（データ駆動の検査は「読めなければ検査 0 件で緑」に倒れやすく、その状態は契約が消えたのと区別できない）。同じ理由で、行の形（種別ごとの欄数・必須欄の非空）とファイル単位の状態（空・コメントだけ・拡張子の付け替え・一覧と実体の食い違い）も実行前に検める — 欄がずれた行は隣の欄を検査パターンとして実行し、空のパターンはどの文書にも一致し、ファイルごと消えた契約は残りのファイルが 1 行でも返す限り件数側の検査に掛からない。いずれも「検査を書いたのに何も検査していない」状態になる。ディレクトリの検分は隠し名・symlink・名前に制御文字を含む要素も拒む（隠し名は走査の展開に載らず、symlink は実体を外へ逃がせ、名前に改行を含むファイルは一覧との突合で 1 ファイルが 2 行に数えられて実在しない契約を在ることにできる）。
- あわせて、参照行の到達点を測る同型の検査が 2 箇所に重複していたのを 1 実装へ寄せた。義務本文の照合は本数が呼び出し側で異なるため可変長で受ける — 固定長にすると「本数が違う側だけ別実装」という今回の重複がそのまま再発する。
- Bash の cwd ガードが出す警告のうち、**linked worktree があり、かつコマンドの先頭が相対パスの `cd`** である場合を、エージェントへ届くチャネル（再実行可能な拒否）へ切り替えた。従来の出力は利用者向けの表示チャネルだけで、行動を変えるべきエージェントのコンテキストには入っていなかった — 実測では、発火条件を満たした呼び出しが相対 `cd` で始まり、続く 2 呼び出しが「そのようなファイルはありません」に化けたが、エージェント側には警告が 1 行も現れなかった。切り替えたのはこの 1 形だけで、`cd` を含まない呼び出しと、モノレポ + background だけに該当する呼び出しは従来どおりブロックしない警告のままなので、誤警告の面は広がらない。拒否の本文には先頭を絶対パスの `cd` へ書き換えて再実行する手順と、ガードを止める環境変数を併記する。あわせて、同じ仕組みを使う 4 本のガードのヘッダーに残っていた「エージェントへ届くチャネルは無い」という記述を、実体（拒否の理由文は届く）に合わせて揃えた。
- 拒否の理由文は、警告用の締め（「そのまま実行して構いません／この警告はブロックしません」）を流用しない。エージェントへ届く唯一のチャネルの中で「止めていない」と自称すると、同じ呼び出しの再実行と抜け道探しを誘発するため、拒否側は「この呼び出しはブロックしました」と効果を名乗り、書き換えて再実行する手順と抜け道を併記する。判定はコマンドの区切り・改行を越えず、オプション付きの絶対 `cd`（`cd -P /abs`）や引数なしの `cd`、直前のディレクトリへ戻る `cd -` は拒否の対象にしない — 誤検知のコストが警告より桁違いに高い側なので、迷う形はすべて警告のまま残す。
- 振り返りスキルの事前注入文へ、スキル本文の絶対パスを実値で載せるようにした。`Skill` ツールを持たないサブエージェントは注入文を受けるたびにスキル本文の所在を自力で探すしかなく、導入先の実測では 5 回とも `find` の探索に 3〜6 回のツール呼び出しを費やしていた（プラグインの配置は深く、かつ版ごとに分かれるため一度で当たらない）。hook は既にプラグインルートを受け取って起動されるので、経路は増やさず値だけを足した。変数名は他ホスト向けに既存の契約が定めているものを使う。パスは JSON 文字列リテラルへ差し込まれるため引用符とバックスラッシュをエスケープし、それで表せない制御文字を含む場合とプラグインルートを解決できない場合は、パスの一節だけを落として注入そのものは従来どおり成立させる（値を足したつもりで注入契約を失う向きへ倒さない）。

## [0.106.0] - 2026-09-13

### 変更

- 変異 self-test（retrospective Stop hook）の変異検査を並列実行にした。隔離された fixture ごとに独立して回せるため、検査の一覧・期待診断・判定条件を一切変えずに実行の仕方だけを変えている。判定は登録順のままなので出力の並びも変わらない。同時実行数は論理 CPU 数（上限 8）で、`FF_RETRO_SELFTEST_JOBS` で上書きでき、`1` で従来どおりの逐次実行に戻る。同時実行数の制限が壊れても検査結果は変わらないため、制限が一度も効かなかった回は実行時に検出して赤にする。
- テストランナー `tests/run-all.sh` は、同梱 MCP の依存（`mcp/node_modules`）が入っていない状態で引数なしの既定一覧を起動したとき、suite を 1 つも実行せずに停止し、「環境が未整備である」ことと復旧コマンド `npm ci --prefix plugins/ff-dev-toolkit/mcp` を案内するようになった。従来はこの状態でも十数分かけて最後まで走り、環境都合の赤が変更起因の赤と同じ形で出ていた。判定は同梱 MCP の package.json が在るのに `node_modules` が無いときだけで、同梱 MCP を含まない checkout では従来どおり停止しない。依存を入れずに回す場合は `FF_RUN_ALL_ALLOW_MISSING_MCP_DEPS=1` を付ける。
- テストランナー `tests/run-all.sh` は、base ブランチが先行しているために落ちた suite を、変更起因の赤とは別行（`✗ stale`）で名指しし、取り込み手段（merge / rebase）と「取り込んでも赤なら変更起因である」ことを案内するようになった。赤であることと終了コードは従来どおりで、判定を skip へ緩めてはいない。あわせて version claim の検査は base 先行を機械可読マーカーで名乗るようになり、`shared-version-convergence` が base の古さを「claim が不足・stale・orphan」という内容の欠陥として報告していた誤りを解消した。
- `node --test` を起動するテストスクリプトへ、reporter の明示（`--test-reporter=spec`）を横断で要求する静的ガードを追加した。既定の reporter は標準出力が端末かどうかと Node の版で変わるため、出力を文字列で照合する検査は手元と CI で結果が食い違い、「手元で緑・CI で赤」と「CI で偽の緑」の両方が起こりうる。出力を照合していない起動も対象にする（照合の有無は静的に決められず、次に照合を足したときに同じ穴が開くため）。同梱テストの未固定だった 1 件もあわせて固定した。
- 知見追記スキルに「追記前にカテゴリ件数の上限超過を予測して停止する」手順を追加した。このスキルは Playbook を増やす操作しか持たないのに、増やした結果がブロックゲートに当たりうる（整理スキルはユーザー承認を必須とするため、自動で呼んで直すこともできない）。超えてから直すのではなく超える前に止め、停止時には先に必要な整理の範囲を名指しして、ユーザーの往復を 1 回で済ませる。あわせて「機械的検証が非 0 を返したまま完了と報告しない」契約を明記した — 非 0 が残る回は完了ではなく停止として報告し、どの検証が赤で、その結果どのゲートが赤になるかを名指しする。
- 件数チェックが判定に使うブロック上限と refine 目安を、件数の状態に関わらず 1 行で出すようにした。従来この値は「目安を超えたカテゴリの警告行」にしか現れず、目安以下のカテゴリへ追記する回は読む対象そのものが存在しなかった — 上の予測手順がその行を読む形だと、読めなかった回が静かに「超過なし」に化ける（fail-open）。予測手順は常に出るこの 1 行だけを読み、終了コードも握り潰さない（既に超過している回・判定が成立していない回は、どちらも追記せず停止する）。
- 予測の停止条件を「上限以上」から「上限を超える」へ直した。件数ゲートは上限ちょうどでは落ちないため、「以上」では 1 件早く止まり、不要な承認の往復を求めたうえで「何件超えるか」の報告が 0 件になる。あわせて件数の取得先を、検証ゲートと同じ順（プロジェクト側の実体があればそれ、無ければ同梱）に揃えた — 同梱固定だと、プロジェクトが環境変数で上限を変えている場合に予測だけが既定値を読んで緑になる。
- 完了報告の契約が挙げる「機械的検証」から行数バジェットを外した。行数の超過は警告として出るだけで終了コードを動かさないため、そこに混ぜると 15 行を超えたエントリが「全ゲート 0」を満たしてしまう。行数バジェットは自分で数える自己チェックとして、完了条件を別に書いた。
- 週次フルゲートの健全性確認に `CRON_NOTE=` を追加した。定期実行（schedule）が発火しているのに結果が赤い回は、総合判定の直後にその事実を 1 行で名乗る。判定そのもの（成功実績を trigger 非依存で測る既存の決定）は変えていない — 実害は「赤い定期実行が出力の 20 行に埋もれて 2 週間気づかれなかった」ことであり、判定の緩さではなく可視性の問題として扱う。
- ACE Playbook 整理スキルに、承認を求める前と承認を受け取った直後の 2 点で base ブランチの先行を照合する規定を追加した。従来の照合は適用の直前 1 点だけで、ユーザーの応答時間だけ開く承認待ちの窓を覆えず、実測では承認を挟んでいる間に並行して入った整理と重なり、適用済みの成果を作り直すことになっていた。照合は適用直前のものと同じ述語を共有する。あわせて、適用後に 2 度目の先行が起きたときの復帰手順（適用済みの成果を捨てず、重なった分を差し引いた集合へ作り直す）も明記した。

### 修正

- 終了コード誤読の静的ガードに `gate-exit-swallowed` を追加した。テストランナーの起動を含む行を診断（`echo` / `printf`）や出力整形フィルタ（`head` / `tail` ほか）で終端すると、起動したプロセス全体の終了コードがその診断のものになり、バックグラウンド実行の完了通知や CI ステップが「赤いゲートを緑」と断定する。判定はゲートをインタプリタ経由または直接起動する形だけを対象にし、ファイルを読むだけの参照は起動と数えない。診断には正しい起動形（終了コードの伝播まで書く形）と、同じ穴になる抜け道を併記する。

## [0.105.0] - 2026-09-13

### 追加

- multi-review に `--delegate-to-host` を追加した。review 実行の `claude-code` レーンだけ CLI を起動せず、プロンプトと書き戻し先をホストへ返す。ホストが自分のセッション内エージェントでそれを実行し、結果を所定のパス（`.review-results/claude-code/` 配下の観点別ファイル）へ書いて同じコマンドを再実行すると、その結果が CLI 起動経路と同じヘッダーで受理され統合レポートが生成される。`claude` CLI がログインしているアカウントの利用枠だけが尽きて、ホストのセッションは別アカウントで動作している状況を想定している（`claude auth` に呼び出し単位のアカウント選択が無く、CLI 側では解決できないため）
- 委譲した結果の受理はプロンプト本文の digest 一致を条件とし、別の入力に対する応答とレビュー本文として成立しない応答は受理しない。受理しなかった応答は削除せず退避先を名指しする。委譲待ちの観点は統合レポートで DELEGATED かつ INCOMPLETE として報告され、実行の終了コードは委譲待ちを表す 3 になる
- 既定の挙動は変えていない。フラグを指定しない実行は従来どおり CLI を起動し、委譲は `claude-code` レーンと review タスクにだけ適用される

### 変更

- 公開ミラー対象の到達不能参照検査が、GitHub REST API のエンドポイント形（API ホスト配下の `/repos/OWNER/REPO`）も見るようになった。これまでホスト判定が Web の GitHub ホストだけに厳密に限られていたため、同じリポジトリ識別子を API の綴りで書くと 1 つのゲートも止まらなかった。判定に使う allowlist は従来と同じ（同期スクリプトの org / リポジトリ定義）で、掲載の無い org 配下のリポジトリを指す API 形 URL は、同期前の全文検査と追加行限定の検査の両方で file:line を名指しして止まる
- ホスト判定そのものは緩めていない。API ホストだけを明示的に扱い、docs / gist / cli などのサブドメインと、ホスト名の末尾が一致するだけの別ホストは従来どおり対象外のままにしている。API 形はパスの文法が違うため、リポジトリ識別子を含む `/repos/OWNER/REPO` だけを見る。`/user` や `/rate_limit` のようなリポジトリを指さないエンドポイントと、allowlist の org 配下でない外部 owner の API URL は 1 件も検出しない
- ホスト名の大小文字違いと明示ポート付きの表記も同じ URL として扱う（綴りを変えるだけの迂回形を作らないため）
- Multi-CLI Agent Orchestration に「委譲先に長時間コマンドを foreground で待たせる契約」と「生存中の委譲先の worktree を回収しない」の 2 節を追加した。前者は委譲プロンプトへタイムアウトの実値と background 実行禁止を常置させる規定、後者は完了通知が来ていない委譲先の worktree を回収しないための規定で、未コミット差分を根拠にしないことと、lock の読み取り・プロセスの実測による生存判定の手順を含む
- 上記 2 節を正本のまま、multi-implement スキルの重要ルールと Git Workflow の並列委譲手順の両方から参照できるようにした。どちらの委譲経路から読んでも同じ規定へ到達する（経路ごとに別の規定を持たない）
- 上記 2 節の到達点検査を、参照側の文書全体ではなく**その手順の節スコープ**で行うようにした。文書全体の grep だと、手順からリンク・実値を外して同じ文書内の無関係な位置へ移すだけで緑のまま通り、手順を読む委譲元には何も届かない。あわせてタイムアウトの実値（Bash ツールの `timeout` に与えるミリ秒）を Git Workflow の並列委譲手順と multi-implement スキルの重要ルールにも実値で置いた（正本を読まない消費地点にはリンクでは届かないため）
- `docs/04-quality/TESTING.md` の「新規 suite 追加の随伴先」の直後に「新規検査を書いた直後の変異注入バッテリー」節を新設した。新規 suite / 新規検査を書いたらコミット前・その場で変異注入バッテリーを回して検出力を実測する手順を明記し、(1) 変異は検査 1 つにつき 1 つの粒度で並べる、(2) 退避・変異・suite 実行・復元を 1 コマンドにまとめて手作業の退避・復元に依存させない、(3) 赤転しなかった変異はそのまま検査追加の TODO にする（緑 = 誰も測っていない）、(4) 結果は当該 suite 自身の `verify.sh` ヘッダの「変異検出:」節へ書き戻す、の 4 点を規範化した。実行がここまで書き手の記憶に依存しており、新規に書いたばかりのゲートが検出力ゼロのまま緑で残る事例が複数回実測されていたことが動機
- 上記の規範を機械検査する `docs-gates` の針を、要求ごとに 1 本の bullet 行へ共起させて当てる形へ強化した（節全体への固定文字列検査だと、要求の一部語句しか見ていない針が条件を削っても生き残る）。併せて、正本の文書が見つからないときに検査群が黙って skip する fail-open を塞ぎ（開発元 checkout では赤、公開 checkout では適用外として 1 行報告）、`docs-gates-runtime` には隔離した偽リポジトリ root で正本を変異させて `docs-gates` の検出器が実際に赤になることを確かめるケースを追加した

### 修正

- 自動振り返りの Stop / UserPromptSubmit hook が、ASDD の任意 Hook ゲートで停止するときに標準入力を読み切らずに終了していた。ペイロードがパイプバッファを超えるとホスト側が EPIPE / SIGPIPE を受けるため、ゲートの前置きを drain の後ろへ移した
- 事前注入 hook は標準入力の読み取りを Node.js に委譲していたため、Node.js が PATH に無い環境ではどの経路でも標準入力が読まれなかった。Node.js 不在時はシェル組み込みの read で読み切るようにし、他の hook と契約を揃えた
- シェル組み込みの read は入力上限（2 秒）で打ち切られると残りを読まないため、約 6 MB を超えるペイロードや、上限を過ぎてから書き始めるホストでは依然として書き手が SIGPIPE を受けていた。上限に達したあとは内容を保持せずに EOF まで捨てて読み切る段を足し、その段にも hook 登録の timeout から導いた上限を置いた
- `RETROSPECTIVE_MODE=off` のとき、事前注入 hook がホスト判別用の Node.js プロセスを毎プロンプト起動していた。kill switch の判定を判別より前へ戻し、無効時はシェル組み込みの read だけで標準入力を読み切るようにした
- 未解消 Critical ガードが中断するすべての分岐で、次に何をすればよいかを stderr に示すようにした。これまでは同ガードの 10 分岐のうち 4 分岐だけが復帰手段を案内しており、残る 6 分岐は中断理由しか出さないため `--help` を読み直すまで次の一手が分からなかった
- 復帰手段は分岐の性質で変える。残骸レポートを読める分岐は従来どおり `--fresh` による退避を案内し、機械状態が壊れていて未解消観点を名指しできない分岐には「先にレポートの Critical セクションを手で読む」を前置きする。レポートファイル自体を読めない分岐と、現在のレビュー系列を特定できない分岐では `--fresh` を提示せず、可読性やリポジトリ状態を先に直すよう案内する（誰も読んでいない Critical 状態を退避させないため）
- 実際に起きる「レポートファイルを読めない」（権限を落とした残骸・I/O エラー）は、機械状態を解釈できない分岐と同じ入口へ落ちて `--fresh` を案内していた。読み取り失敗を専用の戻り値で区別し、可読性を先に直す案内へ振り分けた。この分離で中断分岐は 11 になった
- 前回レポートの Critical マーカー検査は、`grep` 自体のエラーや `grep` / `tail` の不在まで「レポートファイルを読めなかった」と断定していた。戻り値を正規化し、読み取り失敗専用の戻り値と混ざらないようにした
- レビュー系列を特定できない中断の案内が、原因にならない条件（コミットが 1 件以上あるリポジトリであること）を挙げていた。実測した原因（worktree の外・プロジェクトルートの消失・HEAD が解決不能）だけを挙げるようにした
- 中断の条件そのもの（fail-closed の判定）と `--fresh` の実装は変えていない。変更したのは中断時の案内だけ
- multi-review テンプレートの `codex-review.sh` シムで、codex CLI が PATH に無い環境の `--dry-run` 案内を実態に合わせた。従来は「codex が PATH に無くても `--dry-run` はプラン表示を続行する」とだけ告知していたが、このシムは委譲先（`multi-agent.sh`）へ `--cli codex-cli` を固定で足すため、codex 不在の `--dry-run` は他の AI CLI の有無に関わらず委譲先が rc=1 で停止し、プランは表示されない。原因は 2 通りあり、AI CLI が 1 本も PATH に無い場合は委譲先の CLI 検出が `ERROR: No AI CLIs are installed` で止まり、codex だけが不在で他の AI CLI がある場合は codex-cli の fallback 再割り当てが `--cli` フィルタで除外されて実行対象 0 件となり `ERROR: Execution plan is empty` で止まる。警告文と終了コード表の双方をこの読み分けに直した。挙動そのもの（委譲先の CLI 検出・プラン構築・終了コード）は変更していない
- multi-agent の 4 アダプタ共通の成果物書き込み（`adapter-common.sh` の `write_output`）が、書き込みの失敗を戻り値へ載せるようにした。以前は関数の最後のコマンドが表示行だったため、出力先が書き込み不可・容量不足でも戻り値 0 を返し、しかも「Review saved」と成功を名乗っていた。失敗した実行はこれで即座に止まり、成功を名乗る行も出さない
- 書き込みは同じディレクトリの一時ファイルへ行い、全量が書けたことをバイト数で確かめてから所定のパスへ移す方式にした。容量不足が本文の途中で起きた回に、切り詰められた成果物が所定のパスへ残ってオーケストレータの存在検査を通過し、「完成した結果」として resume キャッシュと統合レポートへ INCOMPLETE の印なしで載る経路を塞いだ。同じ場所に前回の完成した成果物が在る場合も、上書きに失敗した実行がそれを壊さない
- 成果物を書けなかったタスクの終了コードはオーケストレータ起因を表す 125 に固定した。CLI 自身は完走しており壊れているのは出力先なので、CLI のクラッシュ・認証切れと同じ番号へ混ぜない。オーケストレータ側の報告もこの番号を「CLI ではなくこちら側の失敗」と読み分けて 1 行で名指しする
- 成果物の宛先に既にディレクトリが在る場合を拒否するようにした。`mv` は一時ファイルをそのディレクトリの**中へ**移して 0 を返すため、所定のパスに成果物が無いまま「Review saved」と名乗る経路がここだけ残っていた。併せて publish の直後に通常ファイルの存在を確かめ、`mv` が 0 を返しても宛先に成果物が無い回は非 0 を返す
- 一時ファイル名は mktemp で作るようにした（使えない環境では従来どおり pid 由来の固定名へ落ち、その場合は書き込み前にそのパスが symlink でないことを確かめる）。予測可能な固定名は、先に置かれた symlink を追跡してその指し先を切り詰める余地を残していた
- ホスト委譲経路で成果物を書けなかった回に、壊れている対象（成果物の出力先）を名指しするようにした。以前は「ホストへ渡せなかった」という誤った理由で報告し、しかも書けない出力先へ失敗の記録を書きに行っていた
- レビュー走行中の作業ツリー編集ガードが、ホストのサブエージェントでレビューする経路でも発火するようになった。これまでは外部 CLI のオーケストレータが走っている間だけロックが置かれていたため、レビュー用サブエージェントを並列起動する経路ではガードが 1 度も発火しなかった。
- レビュー用サブエージェントの起動ごとに `.review-results/.review-in-flight.d/` へレーンが 1 本開く。取得は `tool_use_id` に対して冪等で、`SubagentStart` が来た時点でそのレーンへ `agent_id` を書き込み、`SubagentStop` は **`agent_id` が一致するレーンだけ**を閉じる（同じ `agent_id` の 2 回目以降は何も閉じない）。レーンが 1 本でも残っている間は編集系ツールと git の書き込みコマンドが止まるので、一部のレビュアーが終端しただけでは凍結が解けない。
- レーンを取る subagent の名簿は `FF_REVIEW_SUBAGENT_LOCK_TYPES` で差し替えられる（既定は read-only なレビュアーのみ）。`isolation: "worktree"` の隔離起動は親と作業ツリーを共有しないためレーンを取らない。**名簿の型で動いているレビュアー自身の編集も止めない**（自分の起動で開いたレーンで自分が止まる形を避ける）。
- 取りこぼしたレーンは、対応づいたもので `FF_REVIEW_SUBAGENT_LOCK_MAX_AGE_SECONDS`（既定 14400 秒 = 走行中ロックと同値）、まだ走り始めていないもので `FF_REVIEW_SUBAGENT_LOCK_PENDING_SECONDS`（既定 1800 秒）を過ぎると走査時に自動回収される。**回収したことは systemMessage で必ず通知される**ので、凍結が黙って解けることは無い。手動の回収手順は拒否理由に引用付きの実パスで出る（パスに空白や glob が含まれても安全な形）。
- レーン置き場が外部を指す symlink であるか、物理パスがリポジトリの外へ出る場合は、レーンを書かず・走査せず・削除しない（走査が上限超過レーンを削除するため、リポジトリ外のファイルを消しうる経路を塞いだ）。
- レビューエージェント起動時の dirty 確認が、レビュー結果の出力先を判定から除外するようになった。ガード自身の記帳で作業ツリーが dirty と判定され、以後の起動が毎回確認を出す形を避ける。判定順も「レーン取得 → `git status`」へ変え、`git status` に時間がかかった回でもレーンが必ず立つようにした。

### ドキュメント

- docs-template の CLAUDE.md セットアップ例と、setup-ai-config が生成する 3 ファイル（CLAUDE.md / AGENTS.md / copilot-instructions.md）へ「実測の記録先」節を追加した。実体数・版の一覧・時刻表・スコープ差の注記のような時間で腐る記述は正本へ書き足さず、`docs/08-knowledge/` の日付付き証跡文書（日付・スラッグ・`evidence.md` から成るファイル名）へ置く規範を、本文を複製せず `docs/MASTER.md` の「実測の記録先」を参照する 1 項目として、Claude Code / Codex / Copilot のどの入口からも等しく読めるようにした

## [0.104.0] - 2026-09-12

### 追加

- `merge-cleanup.sh` に `--dry-run` を追加した。1 つも削除せず、「実行した場合に削除されるリモートブランチ・ローカルブランチ・worktree・リモート取り残し」を一覧表示して終わる。保護ブランチ・dirty worktree・ガード情報の取得失敗による fail-closed な縮退の判定は通常実行と同じで、dry-run だけ緩むことはない。見送った `fetch --prune` とリモート削除の連鎖（削除 → prune → 新たに `[gone]` になるローカルブランチ → その worktree）は read-only の `git ls-remote` で予測して一覧へ含めるため、予告が実際の削除より少なくならない。リモートブランチの予告は実行時の `--force-with-lease` と同じ証拠基準（origin 上の ref が実在し、その PR の head と OID 一致）で出す
- `merge-cleanup.sh` に `--help` を追加した。使い方と破壊的処理の一覧を表示して終了し、PR 番号としては解釈しない。解析と早期終了は `mktemp` と `gh` / `jq` の存在確認より前にあるため、依存が入っていない環境でも usage が出る。引数なしの実行も、黙って終わらずに使い方を表示して非 0 で終わるようにした

### 変更

- 公開同期手順の契約検査 `tests/sync-sha-contract` に、定期実行点ゲートが `run-all.sh` のサマリー行 `checks-skipped` を非 0 のまま通すときの許容条件を固定する検査を追加した。手順側は「構造的な部分 skip（設計上の免除・上流仕様により照合が原理的に不可能・データ未到来・同期サイクル内で解消するもの）だけを続行させ、環境都合の部分 skip（外部 CLI や perl / yq / jq / iconv の不在、一時領域不可、root 実行）は 1 件でも名指しして中断する」を、理由文の許容表と実行可能な判定分岐の両方で持つ。全 suite を対象にするゲートでは恒久的な部分 skip が残るため `checks-skipped: total=0` は達成不能であり、達成不能な基準は必ず無視されるようになる。対象を 2 suite に限定した CHANGELOG footer 追従ゲートの側は `total=0` の要求を従来どおり維持し、その非対称が崩れないことも同じ検査で固定する
- 判定コード片の終了コード捕捉を `|| GATE_RC=$?` の形にした。手順書のコード片は `bash -e` で渡す規約に従って set -e 下で走るため、`GATE_RC=$?` を直後に書くと run-all が非 0 の時点でシェルごと落ち、中断理由と次の一手が表示されないまま終わる。一時ファイルは trap で後始末する
- `create-issue` スキルの粒度チェックへ「他リポジトリの台帳 ID を修飾しているか」を追加した。観測台帳・プレイブック・ADR の ID（`OBS-NNN` / `ACE-NNN-N` / `ADR-NNN`）を引用するとき、どのリポジトリの台帳かが文面だけで一意になる形を求める（本リポジトリの台帳を指す場合は修飾不要）。同じ番号の別エントリを掴む誤読を防ぐため
- 改善提案の出力例へ修飾漏れの 1 ブロックを追加し、`refine-issue` スキル手順 4 の対応表へ同名の行（非継承）を追加した
- 委譲プロンプトへ載せる事実主張を一次情報で確認する規定の正本を、multi-implement スキルから Multi-CLI Agent Orchestration の「委譲プロンプトへ載せる事実主張の一次情報確認」節へ移した。同梱スクリプト経由の CLI 委譲だけでなく、ホストが提供する Agent / Task ツールで直接起こしたサブエージェントへの委譲も対象であることを明記している
- 同じ規定へ「サブエージェントの完了報告を Issue コメント・PR 本文・別の委譲プロンプトへ転記する場面も同じ確認義務の対象」を追加した
- multi-implement スキルと Git Workflow の並列委譲手順、およびリポジトリ入口の開発ルールから同じ正本節を参照する形にし、規定の複製を作らずにどの委譲経路からも到達できるようにした
- 公開ミラー前の参照検査が、SSOT の組織配下にありながら公開が明示されていないリポジトリの GitHub URL（clone 用の SSH 形式、ホスト名の大文字小文字違い、明示ポート付きの形を含む）も検出するようになった。外部の公開リポジトリ URL と、リポジトリを指さない GitHub のパスは従来どおり対象外で、誤検出は増えない
- `create-issue` スキルの AI 工数の目安表へ「静的検出器・ゲートの新設」の行を追加した。実装分に加えて、既存資産へ当てて偽陽性クラスを潰す 1 巡とクロスモデルの回避形指摘への対応 1 巡を別項目で積むよう明記する（実装分の 1.0〜1.5 倍）
- 同スキルの「実装分」「レビュー対応分」の分割規定に 1 文を追加し、検出器・ゲートの新設では偽陽性クラスを潰す 1 巡をレビュー対応分とは別項目で積むことを明記した。クロスモデル指摘への対応（レビュー対応分）に畳むと、指摘に基づかない偽陽性潰しの工数が消えてしまうため
- あわせて `effort_ai_planned` の組み立て規定と工数ブロックのテンプレートを更新し、静的検出器・ゲートの新設では「実装分 + 偽陽性潰し分 + レビュー対応分」の 3 項目の和になることと、`effort_basis` の内訳にも 3 項目を書くことを明記した。2 項目固定のままだと、追加した要求が記入できる場所を持たず合計へ入らないため
- `/pre-commit-check` に「staged ファイル種別 → 単体で回す静的 suite」の固定表と実行手順を追加した。配布 Markdown（`docs-template/**/*.md` / `skills/**/*.md`）は `docs-gates` と `plugin-root-contract`、公開同期対象と changelog 断片は `sync-forbidden-patterns`、shell は `run-all` と `shellcheck` を commit 前に単体で回す。表は監視 glob を持たない固定表で、全件ゲートを置き換えない（全件ゲートは従来どおり全件で回し、プリフライトはその手前に置く安い先行検査）。対象種別が無ければ「対象なし」を 1 行で報告し、suite 不在・依存不足・suite 自身の skip は「実行できなかった」として「違反なし」と区別し緑扱いにしない。あわせて、staged パスから対象を選びながら実体は作業ツリーを読む構造で staged 側の違反が「違反なし」と報告される経路を塞ぐため、プリフライトの前提ゲート（staged パスについて index と作業ツリーの内容が一致することの確認）を追加した。不一致なら「違反なし」ではなく「判定不能」として fail-closed で止め、不一致のファイルを名指しして stage し直す案内を出す

### 修正

- `scripts/templates/codex-review.sh` の降格案内の見出しを原因非依存の文言にした。toolkit（`multi-agent.sh`）を解決できない経路でも固定で「Codex 不在のため」と名乗っていたため、codex が導入済みの環境で原因を誤って名指ししていた
- 同じ案内の手順 1 を経路ごとに出し分けるようにした。`multi-agent.sh` が見つからなかった経路では、その `multi-agent.sh` の実行ではなく `setup-multi-agent.sh` の再実行を案内する
- さらに同じ手順 1 を toolkit 解決の結果ごとに分けた。`FF_DEV_TOOLKIT_ROOT` が設定されているのに使えない場合は、`setup-multi-agent.sh` の再実行ではなく環境変数の修正 / 解除を案内する。探索順はこの環境変数が最優先なので、setup がサイドカーを直しても同じ指定が次回も先に勝ち、案内が次の一手にならなかった
- 案内が挙げる代替 CLI 候補を `multi-agent.sh` の CLI registry から導出するようにした。cost tier が metered の CLI は委譲先の既定ラインナップと同じ根拠で候補から外れる（従来は候補名が案内側に直書きされ、metered な CLI を勧めていた）
- codex CLI の実在検査を `--dry-run` 非指定時に限定した。`--dry-run` は CLI を 1 本も起動しないため、codex 不在でも警告に留めてプランを表示する（実行モードでは従来どおり exit 4 で停止する）
- `resolve_base_ref_for_size` が toolkit そのものを解決できなかった経路でも、生の値で diff サイズを測る旨を WARNING に残すようにした。歯止めによる skip は委譲前に exit 0 で終わるため、従来はどの基準で測って skip したかが記録に残らなかった
- 同関数の pipefail コメントの条件反転を修正した。exit code のマスクが起きるのは pipefail が無いときで、pipefail 下では非 0 が保たれる
- multi-agent.sh: 残存レポートの機械状態を読めずレビューを中断したとき、他の中断分岐と同じく `--fresh` での復帰手段を案内するようにした。この分岐だけ案内が無く、`--help` を読み直さないと再開できなかった
- multi-implement の観点プロンプト 5 件（documentation / feature-implementation / migration / refactoring / test-writing）で、出力フォーマット例を囲む外側のコードフェンスが閉じず、以降の節が丸ごとコードブロックとして描画されていた問題を修正した。入れ子の 3 連フェンスを含む例は外側を 4 連フェンスへ引き上げて開閉を揃えた
- 観点プロンプトのコードフェンス開閉を固定する検査を既存のテストスイートへ追加した。閉じフェンスが 1 つでも欠けると検査が失敗する
- PreToolUse ガード 6 本（checkout/restore・PR フォローアップ・background cwd・レビュー走行中・工数実績・起票ラベル）が、ASDD の任意 Hook ゲートで早期終了する経路でも stdin ペイロードを読み切ってから終了するようになった。ゲートの前置きを drain より後ろへ移した修正で、`.asdd` 設定があって Node.js が無い環境や、Hook 機能を無効にしている環境でも、ホストが EPIPE / SIGPIPE を受けなくなる
- 同梱の Node.js エントリ（`scripts/asdd/cli.mjs` / `scripts/asdd/work.mjs`）にも plugin root 固定ガードを追加した。shell スクリプトと同じ判定・同じ案内文言で、plugin root が消失・不整合のまま起動された場合に別のインストール領域へ切り替えず、案内付きで終了コード 2 で停止する。handoff が無い直接起動（端末・テスト・CI の pin 実行）と、ライブラリとして import した経路は従来どおり停止しない。

### ドキュメント

- 公開 README の「Bash ガード（PreToolUse）」節が実体（hooks.json の `PreToolUse`・`Bash` matcher）とずれていたのを直し、本数と各ガードの説明を現在の登録内容に合わせた。README の本数・実体名が実体からずれた場合に赤くなる検査を `tests/skill-count-consistency/verify.sh` へ追加した
- docs-template の MASTER.md 更新ルールに「実測の記録先」を追加し、`08-knowledge/README.md` を新設して日付付き証跡文書（例: `2026-09-12-example-evidence.md`）の置き場を定義した。正本に残すのは判断手順・判断に直結する境界値・測り直しコマンド・証跡へのリンクまでとし、実体数・版の一覧・時刻表のような時間で腐る記述は証跡文書へ置く規範を明文化した。
- `merge-cleanup` スキルと Git Workflow ガイドのマージ手順に、`gh pr merge --delete-branch` がローカルブランチ削除だけ失敗する場合の読み方を追記した。`failed to delete local branch ... used by worktree at ...` はマージやリモートブランチ削除の失敗を意味しないため、`gh pr view --json state` と `git ls-remote --heads` の実測で判定する手順を明示した
- 同じ箇所に、残ったリモートブランチを `gh api -X DELETE` で個別削除するときの注意も追記した。ブランチ名に含まれる `#` をそのまま渡すと URL のフラグメント区切りとして解釈され削除リクエストが失敗するため、URL エンコードしてから渡す必要がある
- あわせてリモート個別削除の手順を fail-closed にした。残存判定は `git ls-remote --exit-code` の終了コードで行い（通信・認証失敗で出力が空になる経路を「削除済み」と読まない）、削除前に残っている ref の OID が対象 PR の head と一致することを確認する（マージ後に同名ブランチが再利用されていると別作業を消すため）

## [0.103.0] - 2026-09-12

### 追加

- 新規 suite `host-route-parity` を追加した。ホスト経路（ルート指示文書 / repo-local スキル入口 / 配布スキル / 配布ドキュメント）と届け先ファイルの対応表を正本として読み、規定が 1 ホスト経路だけに入っている状態を検出する。repo-local スキル入口の集合一致（壊れた symlink も列挙に残す）、リポジトリ直下のホスト固有指示文書の逆向き導出、宣言した契約が経路ごとの正本ファイルに実在するかを双方向で照合する。外部ツールに依存せず、対応表を持たない checkout では suite 全体をスキップする
- `multi-review` スキルの「toolkit 変更 PR の制約」節へ、契約文・スキル・hook を変更した PR のセルフレビューで「その変更が全ホスト経路へ届いているか」を確認する導線を追加した。届け先の 3 系統（ルート指示文書 / 配布スキルの SKILL.md / 配布ドキュメントの正本節）と、非対称の正本表がある場合の参照先を明示する
- 同じ導線を配布ドキュメントの「セルフレビュー（PR作成前）」へも常置した。消費プロジェクト側もホストごとに別のルート指示文書（CLAUDE.md / AGENTS.md / copilot-instructions.md）と 2 つの repo-local スキル入口を持つため、1 経路だけに書いた規定が他ホストへ届かない失敗は同型に起きる
- 配布ドキュメントの「Implement の書き込み境界」節を実装に合わせて訂正した。staging だけへの限定が機械的に効くのは Codex の implement だけで、Grok / Claude Code / Copilot では sandbox の境界は repository root のままプロンプト契約であることを明記した

### 修正

- docs-template の 08-knowledge（TROUBLESHOOTING / LESSONS_LEARNED）に残っていた JWT 検証例を修正: `verifyToken` の戻り値と `validateTokenPayload` の引数が `any` で検証済みペイロードの形を表明していなかった。`sub` / `exp` / `iat` を持つ具体的なインタフェースへ変え、`jwt.verify` の戻り値は実行時の型ガード（`exp` / `iat` は NumericDate として `Number.isFinite` で検証）を通してから返すようにした
- 同じ例の catch 節が `unknown` な error から直接 `.message` を読んでいた点を、JWT 専用のエラーマッパー経由に改めた。HTTP ステータス境界向けの `normalizeExternalError` は通さない — JWT 検証エラーはステータスを持たないため、あの関数を通すと一時障害（再試行・本番フォールバックの対象）に分類され、署名不正や期限切れのトークンが握りつぶされる。専用マッパーは期限・時刻条件を認証エラーへ、署名不一致と不正形式をセキュリティエラーへ写し、いずれもフォールバック禁止カテゴリに倒す
...
- 公開ミラー同期の自己検査 suite に、文書中のリポジトリ間参照が公開読者から到達できるかを allowlist で判定する検査の契約を追加した。同期スクリプトを持たない公開 checkout ではこれまでどおり skip する

## [0.100.0] - 2026-09-11

### 追加

- 委譲先のエージェントが完了報告を返したあとも、そのエージェントがバックグラウンドで起こした子プロセスだけが走り続けることがある。完了後にその取り残しを確認して回収する契約を Multi-CLI Agent Orchestration の手順へ追加した
- 探し方は待機ループに限定せず、経過時間で絞る形にした。待機ループはシェル snapshot ラッパー越しに起動されるためコマンドラインに待機構文が現れず、内容で検索すると空振りする。実測では検索コマンドの取り残しもあり、孤児の形は任意の長時間バックグラウンド子プロセスである
- 検出コマンドは実値で載せ、回帰スイートがフェンス内の構文検査まで行う。エージェントホストの PID 一覧をプロセス一覧から引くのは、パターン検索では引数の長いプロセス（チェックを実行している当のセッションを含む）を取りこぼすため

### ドキュメント

- レビュー指摘を「箇所」でなく「系統」で分類し、原因を問う step を追加した。`out-of-scope-issue` の判定へ原因の軸（§1.0）を足し、トリガーは「発見の対象ファイル / 行が前巡の fix commit の差分に含まれる」「同型の発見が 2 回目」の 2 つに限定した
- `multi-review` と `multi-cli-review-orchestration.md` の収束判定へ、上限に達する前に原因を問うトリガーと、上限到達後に方針転換で再開する場合は追加の上限を宣言する規定を追加した。追加上限は Critical / Warning の解消義務を免除せず、未対応のまま残せるのは Suggestion 以下だけ
- PR テンプレート雛形の Cross-Model Review Results へ、前巡の fix 由来または同型 2 回目の指摘が出た場合の系統・原因・構造 Issue の要否の記入欄を追加した

## [0.99.0] - 2026-09-11

### 追加

- 公開リポジトリへ同期されるファイルについて、比較対象ブランチとの差分で**新しく追加された行**にだけ、参照先が文面だけでは一意にならない番号短縮形が混ざっていないかを検査するスクリプトを追加しました。既存行の同型はそのまま許容するため、公開前の同期を止めずに新しい混入だけを機械的に弾けます。
- この検査の許可規則は Issue 本文向けの既存走査器をそのまま使います。コードフェンス内・インラインコード内・markdown リンク内の記述と、完全な公開 URL 形式での参照は従来どおり通ります。
- まだ版管理に追加されていない新規ファイルは差分に現れないため、公開対象の配下にあるものはファイル全体を新しい追加として走査します。新しく書き起こしたファイルこそ混入の主な発生源なので、素通しにしません。
- コードフェンスが閉じられていないファイルでも走査を諦めません。このとき無効化するのは閉じられなかった開始行だけで、同じファイル内で正しく閉じているコードフェンスの内側は従来どおり通ります。
- 公開対象ディレクトリを持たないチェックアウトでは「対象 0 件」として正常終了し、比較対象ブランチを解決できない場合は黙って通さず検査不成立として停止します。
- multi-implement スキルの委譲プロンプト標準文言に、事実主張の確認義務をオーケストレータ（委譲する側）の規定として追加した。委譲プロンプトへ載せる事実主張は、レビュー指摘由来かオーケストレータ自身の観察かを問わず、実体の grep やスクリプトの終了コード契約、コミット済みブランチの内容確認といった一次情報で確認してから渡す
- この規定は、既存の「委譲先が指示からの逸脱を根拠つきで報告してよい」という委譲先側の規定とは別のものであると明記した。委譲先が逸脱を報告できることは、オーケストレータが事実確認を省略してよい理由にはならない
- PreToolUse（Bash）へ工数実績の未記入マージガードを追加した。閉じる Issue の本文に `ff-effort` ブロックがあるのに `effort_ai_actual` が未記入のままの `gh pr merge` を実行前に止め、未記入の Issue 番号・PR 番号つきの `/close-issue` を実行する案内・迂回手段を同じメッセージで示す。ブロックが無い Issue は従来どおり対象外で、遡及付与は強制しない
- 迂回は対象コマンドの先頭へ `FF_EFFORT_ACTUAL_ACK=1` を置いた場合のみ有効。ガード自体は `FF_DEV_TOOLKIT_SKIP_EFFORT_ACTUAL_GUARD=1` で無効化できる。`gh` / `jq` が無い環境、`gh` の呼び出しが失敗する環境では停止しない
- 対象は「この PR が閉じる Issue」。GitHub が解決した closing 参照を正本とし、それが取れないとき（closing keyword を書かない `Refs` 運用の PR）だけ本文を走査するので、長命の tracking Issue を `Refs` で参照する sub-PR は止まらない。既に閉じている Issue も対象外
- 未記入の判定は集計器 `effort-report.sh` の分類と同形。マーカー行は完全一致で見て、字下げ・行末空白・未閉鎖・複数ブロック・工数キーの書式不正（単位違い等）は集計器が別扱いする状態なので停止しない。本文が CRLF でも同じ結論になる
- 文字列やヒアドキュメント本文の中に書かれた `gh pr merge` では発火しない
- 起票ラベル契約ガード（PreToolUse / Bash）を追加した。起票スキルを経由せずに `gh issue create` を直接実行したとき、種別ラベルと優先度ラベル（`priority:` 名前空間）の欠落を実行前に止め、同じメッセージで迂回手段（起票スキル経由・不足ラベルの追加・環境変数 `FF_DEV_TOOLKIT_SKIP_ISSUE_LABEL_GUARD=1`）を案内する
- `follow-up` ラベルの欠落は止めない。起票が PR レビュー・実装から派生した発見かどうかはコマンド文字列から機械的に判定できないため、表示のみの案内に留める
- ラベル名の正本は推奨ラベル構成の軸別表（`setup-github-labels` スキル）で、ガード側は名前を持たない。対象リポジトリのラベル一覧を信用できないとき（照会の失敗・空の一覧・取得上限到達）は「不在」と断定せず素通しする
- 判定はコマンド文字列を行継続（行末のバックスラッシュ）と引用符を解いたうえで読む。複数行に分けた起票コマンドや、タイトル・本文にパイプ・セミコロン・アンパサンドを含む起票を誤って止めない。GitHub 既定のラベルのうち種別として使わない名前（`wontfix` や `question` など）は種別の充足として数えない

### 変更

- create-issue の受け入れ条件（AC）粒度チェックに項目を追加した。AC・参照文書・順序制約が既存の文言・アンカー・パス・行番号を引用している場合のみ、起票時点の作業リポジトリで grep（文言・アンカー）/ ls（パス）により実在を確認し、見つからなければ引用せず「新設」または「最近傍の節に置く」と実在に基づいて書くよう求める。改善提案の出力例にも該当パターンを追加した
- create-issue の完了報告に「引用実在確認: 済（対象 N 件）」または「引用実在確認: 対象なし」の1行を残す規定を追加した
- refine-issue の対応表にも同じ項目名の行を追加し、非継承（引用の実在確認は起票時点の責務であって、既に立った Issue の記述の曖昧さを扱う refine の対象外）とした
- Git ワークフローのコミット手順に、コミット前に staged なシェルファイルを単体チェックする案内を追加した。マルチバイト文字の直付け展開など、後段のレビューでは検出できない種類の違反をコミット前に見つけられるようにするための変更

### 修正

- 固定 root 経由で呼ばれる同梱スクリプトが、起動時に自分の実体位置とホストから渡された plugin root（`FF_DEV_TOOLKIT_ROOT` / `CLAUDE_PLUGIN_ROOT` / `GROK_PLUGIN_ROOT`）の整合を確認するようになった。食い違っていれば案内を出して非 0 で中断する。plugin root が消えた状態で、別インストール領域にある同名スクリプトへ切り替えて実行してしまう事故を、skill 本文の契約が届かなかった呼び出し元でも止められる（契約が散文だけにあると、その契約が守ろうとしている失敗と契約が手元から失われる条件が同じになる）
- スキル本文とドキュメントテンプレートの実行部を `FF_DEV_TOOLKIT_ROOT="${FF_DEV_TOOLKIT_ROOT}" bash "${FF_DEV_TOOLKIT_ROOT}/scripts/merge-cleanup.sh"` の形に揃えた。ホストは plugin root を Bash ツールの環境変数として渡さず、実行部のテキストへ解決済みの絶対パスを差し込むだけなので、handoff を運ぶのが本文の resolver だけだと、本文が劣化して届かなかったちょうどそのときに handoff も消えてガードが素通しになる。代入を実行部と同じ 1 行に載せておくと、実行するスクリプトのパスだけを別領域へ書き換える操作がその行の中の不一致として検出される
- 中断の案内には、別インストール領域へ切り替えてはいけない理由（version 混在と未リリース WIP の実行）と、cache や別 checkout を探さずに plugin を再導入してから skill を呼び直す手順を含める
- ガードは候補を探索しない。cache / marketplace / 旧インストール領域の走査も、version 名の並べ替えによる選び直しも行わず、渡された値を正規化して自分の位置と比べるだけで判定する。handoff が 1 つも渡されていない直接起動（端末・テスト・pin した CI 実行）は従来どおり動く
- 判定できない状態は素通しではなく中断へ倒す。渡された root が実在しても、その plugin manifest を通常ファイルとして読めなければ「別 plugin だから自分宛てではない」とは言えないので中断する（更新中の部分的な消失では、判定材料そのものが壊れた領域の内側にある）。起動したスクリプト自身が symlink の場合も、実体位置の照合が成立しないため中断する
- 「別 plugin なので自分宛てではない」の判定に使う名前は、manifest の最上位オブジェクト直下にある `name` だけを読む。最初に現れる `name` 行を採る読み方だと、`author` のような入れ子オブジェクトが最上位の `name` より前にある manifest で作者名を掴んでしまい、同じ plugin の別インストール領域を別 plugin と誤読して照合ごと飛ばすことになる
- ガードは外部コマンドを使わず bash の組み込みだけで判定する。PATH が壊れた環境はこのガードが最後の砦になる場面そのもので、`dirname` / `grep` / `sed` に依存すると自分の位置を判定できないまま素通しになる。その代わり組み込み側の環境依存はガード自身で閉じており、パスの正規化は `CDPATH` を無効化して行う（`CDPATH` が効くと `cd` は別のディレクトリへ移動したうえで移動先を標準出力へ出すため、相対パスでの直接起動が自分の位置を取り違え、止めないと明言している端末・テスト・CI の起動まで中断していた）
- 起動時に、実行しているスクリプトの絶対パスと plugin の version を 1 行出すようにした。照合を飛ばした handoff がある場合はその変数名と理由も同じ行に出す。出力そのものが呼び出し側との契約になっているスクリプトでは、この 1 行だけを抑止する（抑止はスクリプト内部の名前リストに閉じており、環境変数では抑止できない。中断の判定と案内は抑止されない）
- 対象は固定 root 経由で呼ばれる同梱スクリプト 14 本（merge-cleanup / multi-agent / multi-review / setup-multi-agent / sweep-orphan-transcripts / workflow-tier / check-closing-keywords / check-merge-freshness / check-version-claims / check-plugin-versions / update-version-claim / ace-run-ts / check-issue-body-diff / effort-report）。呼び出し側からの名簿の導出は `${VAR:?案内}` のような修飾付きの綴りと brace 無しの綴りも拾う。複製の byte 一致・配置・中断コード・実際の停止は回帰テストが固定する
...
[Unreleased]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.134.0...HEAD
[0.134.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.133.1...v0.134.0
[0.133.1]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.133.0...v0.133.1
[0.133.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.132.0...v0.133.0
[0.132.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.131.0...v0.132.0
[0.131.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.130.0...v0.131.0
[0.130.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.129.0...v0.130.0
[0.129.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.128.0...v0.129.0
[0.128.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.127.0...v0.128.0
[0.127.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.126.0...v0.127.0
[0.126.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.125.0...v0.126.0
[0.125.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.124.0...v0.125.0
[0.124.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.123.0...v0.124.0
[0.123.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.122.0...v0.123.0
[0.122.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.121.1...v0.122.0
[0.121.1]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.121.0...v0.121.1
[0.121.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.120.0...v0.121.0
[0.120.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.119.0...v0.120.0
[0.119.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.118.0...v0.119.0
[0.118.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.117.0...v0.118.0
[0.117.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.116.1...v0.117.0
[0.116.1]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.115.2...v0.116.1
[0.115.2]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.115.1...v0.115.2
[0.115.1]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.115.0...v0.115.1
[0.115.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.114.0...v0.115.0
[0.114.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.113.0...v0.114.0
[0.113.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.112.1...v0.113.0
[0.112.1]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.112.0...v0.112.1
[0.112.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.111.0...v0.112.0
...
[0.108.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.107.0...v0.108.0
[0.107.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.106.0...v0.107.0
[0.106.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.105.0...v0.106.0
[0.105.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.104.0...v0.105.0
[0.104.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.103.0...v0.104.0
[0.103.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.102.0...v0.103.0
[0.102.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.101.0...v0.102.0
[0.101.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.100.0...v0.101.0
[0.100.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.99.0...v0.100.0
[0.99.0]: https://github.com/feel-flow/ff-dev-toolkit/compare/v0.98.0...v0.99.0
